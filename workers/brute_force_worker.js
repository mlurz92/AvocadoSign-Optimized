...
function applyT2CriteriaToPatientWorker(patient, criteria, logic) {
     ...
     if (activeKeys.length === 0) return null; // No active criteria means no T2 status can be determined based on them
     if (lymphNodes.length === 0) return '-'; // No lymph nodes to evaluate, so cannot be T2 positive
...
function calculateMetric(data, criteria, logic, metricName) {
    ...
    return isNaN(result) ? -Infinity : result; // Return -Infinity for non-calculable metrics to ensure they are not chosen as best
}

function generateCriteriaCombinations() {
    const CRITERIA_KEYS = ['size', 'form', 'kontur', 'homogenitaet', 'signal'];
    const VALUE_OPTIONS = {
        size: [], // Will be populated based on t2SizeRange
        form: ['rund', 'oval'],
    ...
    if (min !== undefined && max !== undefined && step !== undefined && step > 0) {
        ...
        // Ensure min and max are included if not generated by step
    ...
    for (let i = 1; i < (1 << numCriteria); i++) { // Start from 1 to exclude empty set
        const baseTemplate = {};
        const currentActive = [];
        ...
        function generateValues(keyIndex, currentCombo) {
            if (keyIndex === currentActive.length) { // All active criteria have been assigned a value
            ...
            if (!options || options.length === 0) { // Should not happen for defined keys
            ...
            options.forEach(value => {
                ...
                if (currentKey === 'size') {
                    ...
                    nextCombo[currentKey].condition = '>='; // Assuming default condition
                } else {
...
function runBruteForce() {
    ...
    for (let i = 0; i < allCombinations.length; i++) {
        ...
        try {
            ...
            metricValue = -Infinity; // Treat errors in calculation as a very bad metric
        }
    ...
    if(isRunning) { // Only send results if not cancelled
        ...
        for(let i = 0; i < validResults.length; i++) {
            ...
            // Add to topResults if rank is within top 10
            // or if it's rank 11 but has the same score as the 10th (or last in topResults if less than 10)
            if (rank <= 10) {
                topResults.push(validResults[i]);
            } else {
                // If rank is 11 and score is same as last element of topResults (which would be rank 10 or less if topResults < 10)
...
self.onerror = function(error) {
    ...
    isRunning = false; // Ensure worker stops on unhandled errors
};
