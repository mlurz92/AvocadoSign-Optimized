const dataProcessor = (() => {
    ...
    function calculateROCLikeData(data, predictionKey, referenceKey) {
        if (!Array.isArray(data) || data.length === 0) {
            return { rocPoints: [{ fpr: 0, tpr: 0 }, { fpr: 1, tpr: 1 }], auc: 0.5 };
        }

        const matrix = statisticsService.calculateConfusionMatrix(data, predictionKey, referenceKey);
        const { rp, fp, fn, rn } = matrix;
        const totalPos = rp + fn;
        const totalNeg = fp + rn;

        let tpr = 0, fpr = 0;

        if (totalPos > 0) tpr = rp / totalPos;
        else if (rp === 0 && fn === 0) tpr = 1;

        if (totalNeg > 0) fpr = fp / totalNeg;
        else if (fp === 0 && rn === 0) fpr = 0;


        if (isNaN(tpr) || !isFinite(tpr)) tpr = (rp > 0 ? 1 : 0);
        if (isNaN(fpr) || !isFinite(fpr)) fpr = (fp > 0 ? 1 : 0);
        ...
        const auc = (tpr - fpr + 1) / 2;


        return {
            rocPoints: rocPoints,
            auc: isNaN(auc) ? 0.5 : Math.max(0, Math.min(1, auc))
        };
    }
