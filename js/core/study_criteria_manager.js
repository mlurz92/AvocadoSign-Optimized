const studyT2CriteriaManager = (() => {
    ...
    const studyT2CriteriaSets = Object.freeze([
        Object.freeze({
            ...
            criteria: Object.freeze({
                size: Object.freeze({ active: true, threshold: 9.0, condition: '>=' }), // Nicht direkt für die Kombi-Logik verwendet, aber informativ
    ...
    function formatCriteriaForDisplay(criteria, logic = null, shortFormat = false, lang = 'de') {
        ...
        const formatValue = (key, criterion, isShort) => {
            ...
            if (key === 'size') {
                const formattedThreshold = formatNumber(criterion.threshold, 1, '?', false, langKey);
            ...
            } else if (valueTranslations[valueToDisplay] && valueTranslations[valueToDisplay].de) {
                valueToDisplay = valueTranslations[valueToDisplay].de;
            }


            let prefix = currentPrefixes[key]?.[langKey] || currentPrefixes[key]?.['de'] || (key + (isShort ? '=' : ' = '));
        ...
        if (effectiveLogic === 'KOMBINIERT') {
             const studySet = studyT2CriteriaSets.find(s => {
                ...
                const inputCriteriaKeys = Object.keys(criteria).filter(k => k !== 'logic' && criteria[k]?.active === true);
                const studyActiveKeys = Object.keys(s.criteria).filter(k => k !== 'logic' && s.criteria[k]?.active === true);

                if (inputCriteriaKeys.length !== studyActiveKeys.length) return false;

                return inputCriteriaKeys.every(k => {
                    if (!s.criteria[k]) return false;
                    return s.criteria[k]?.active === criteria[k]?.active &&
             ...
             if (studySet && studySet.studyInfo) {
                 return shortFormat ? (studySet.studyInfo.keyCriteriaSummary || studySet.name) : studySet.description;
             }
             return criteria.note || (langKey === 'de' ? 'Kombinierte Logik (Details siehe Studie)' : 'Combined Logic (see study for details)');
        }
    ...
    function _checkSingleNodeESGAR(lymphNode, criteria) {
        ...
        if (nodeSize >= 9.0) {
            ...
            checkResult.size_crit_met = (morphologyCount >=2);
             ...
             checkResult.esgarCategory = 'N/A (Größe außerhalb der Regeln)';
             checkResult.isPositive = false;
             checkResult.size_crit_met = false;
        }
    ...
    function applyStudyT2CriteriaToPatient(patient, studyCriteriaSet) {
        ...
        if (lymphNodes.length === 0) {
            const noLNCriteriaActive = (logic === 'KOMBINIERT' && studyCriteriaSet.id === 'rutegard_et_al_esgar') || activeCriteriaKeys.length > 0;
            return { t2Status: noLNCriteriaActive ? '-' : null, positiveLKCount: 0, bewerteteLK: [] };
        }
        ...
        if ((logic === 'KOMBINIERT' && studyCriteriaSet.id === 'rutegard_et_al_esgar') || activeCriteriaKeys.length > 0) {
    ...
    function formatStudyCriteriaForDisplay(studyCriteriaSet, lang = 'de') {
        ...
        const logic = studyCriteriaSet.logic;
        const langKey = lang === 'en' ? 'en' : 'de';
        const studySetName = UI_TEXTS.literatureSetNames?.[studyCriteriaSet.id]?.[langKey] || UI_TEXTS.literatureSetNames?.[studyCriteriaSet.id]?.['de'] || studyCriteriaSet.name || studyCriteriaSet.id;


        if (logic === 'KOMBINIERT' && studyCriteriaSet.id === 'rutegard_et_al_esgar') {
             return studyCriteriaSet.studyInfo?.keyCriteriaSummary || studyCriteriaSet.description;
        }

        if (studyCriteriaSet.studyInfo?.keyCriteriaSummary && (studyCriteriaSet.id === 'koh_2008_morphology' || studyCriteriaSet.id === 'barbaro_2024_restaging')) {
            if (langKey === 'en') {
                 if (studyCriteriaSet.id === 'koh_2008_morphology') return "Irregular border OR heterogeneous internal signal.";
                 if (studyCriteriaSet.id === 'barbaro_2024_restaging') return "Short-axis diameter ≥ 2.3mm.";
            }
            return studyCriteriaSet.studyInfo.keyCriteriaSummary;
        }

        if (!criteria) return `${studySetName} (${langKey==='de' ? 'Keine Kriterien definiert' : 'No criteria defined'})`;

        const formattedCriteria = formatCriteriaForDisplay(criteria, logic, false, langKey);
        return `${studySetName}: ${formattedCriteria}`;
    }
