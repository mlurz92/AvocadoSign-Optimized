const t2CriteriaManager = (() => {
    ...
    function applyT2CriteriaToPatient(patient, criteria, logic) {
        ...
        if (activeCriteriaKeys.length === 0 && lymphNodes.length > 0) {
             finalT2Status = null;
        } else if (activeCriteriaKeys.length === 0 && lymphNodes.length === 0) {
             finalT2Status = null;
        } else {
    ...
    function evaluateDataset(dataset, criteria, logic) {
        ...
        if (!criteria || (logic !== 'UND' && logic !== 'ODER')) {
             console.warn("evaluateDataset: Ungültige Kriterien oder Logik. Gebe unveränderte Daten zurück.");
