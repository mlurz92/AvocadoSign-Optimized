...
function initializeApp() {
    ...
    try {
        if (typeof state === 'undefined' || typeof t2CriteriaManager === 'undefined' || typeof dataProcessor === 'undefined' || typeof viewRenderer === 'undefined' || typeof ui_helpers === 'undefined' || typeof exportService === 'undefined' || typeof publicationTabRenderer === 'undefined') {
             throw new Error("Ein oder mehrere Kernmodule (state, t2CriteriaManager, dataProcessor, viewRenderer, ui_helpers, exportService, publicationTabRenderer) sind nicht verfügbar.");
        }
...
function updateUIState() {
    try {
        ...
        if(state.getActiveTabId() === 'publikation-tab') {
            ui_helpers.updatePublikationLangSwitchUI(state.getCurrentPublikationLang());
        }
...
function setupEventListeners() {
    ...
    document.body.addEventListener('change', (event) => {
        ...
        else if (target.id === 'publikation-sprache-switch' && target.closest('#publikation-tab-pane')) { // Angepasst für Publikation-Tab
            handlePublikationSpracheChange(target);
        }
    });
}
...
function handleBodyClickDelegation(event) {
    ...
    const publikationSubNavButton = target.closest('#publikationSubNav .nav-link'); // NEU
    ...
    if (publikationSubNavButton && !publikationSubNavButton.classList.contains('active')) { // NEU
        handlePublikationSubNavClick(publikationSubNavButton); return;
    }
...
function handleTabShownEvent(event) {
    if (event.target && event.target.id && state.setActiveTabId(event.target.id)) {
        ...
        filterAndPrepareData();
        updateUIState(); // Sicherstellen, dass UI-State VOR dem Rendern des Tabs aktuell ist
...
function handleTabShown(tabId) {
    ...
    switch (tabId) {
        ...
        case 'publikation-tab': // NEU
            // Den activeSectionKey für den Publikationstab hier initial bestimmen oder aus dem State holen, falls persistent.
            // Fürs Erste nehmen wir 'methoden' als Default, wenn der Tab neu geladen wird.
            // Eine persistentere Lösung würde den `activeSectionKey` im `state` speichern.
            const publikationSubNav = document.getElementById('publikationSubNav');
            let activeSectionKey = 'methoden';
            if (publikationSubNav) {
                const currentActiveBtn = publikationSubNav.querySelector('.nav-link.active');
                if (currentActiveBtn && currentActiveBtn.dataset.sectionKey) {
                    activeSectionKey = currentActiveBtn.dataset.sectionKey;
                }
            }
            viewRenderer.renderPublikationTab(processedData, lastBruteForceResults, currentKollektiv, appliedCriteria, appliedLogic, state.getCurrentPublikationLang(), activeSectionKey);
            break;
    ...
    updateUIState(); // Sicherstellen, dass UI-State NACH dem Rendern des Tabs aktuell ist (z.B. Export-Button-Status)
}
...
function handleSortClick(sortHeader, sortSubHeader) { const key = sortHeader?.dataset.sortKey; if (!key) return; const subKey = sortSubHeader?.dataset.subKey || null; const tableBody = sortHeader.closest('table')?.querySelector('tbody'); let tableId = null; if (tableBody?.id === 'daten-table-body') tableId = 'daten'; else if (tableBody?.id === 'auswertung-table-body') tableId = 'auswertung'; if (tableId) handleSort(tableId, key, subKey); } // Angepasst
function handleSort(tableType, key, subKey = null) { let sortStateUpdated = false; if(tableType === 'daten') sortStateUpdated = state.updateDatenTableSortDirection(key, subKey); else if (tableType === 'auswertung') sortStateUpdated = state.updateAuswertungTableSortDirection(key, subKey); if(sortStateUpdated) { const sortState = (tableType === 'daten') ? state.getDatenTableSort() : state.getAuswertungTableSort(); filterAndPrepareData(); if (tableType === 'daten' && state.getActiveTabId() === 'daten-tab') viewRenderer.renderDatenTab(currentData, sortState); else if (tableType === 'auswertung' && state.getActiveTabId() === 'auswertung-tab') { handleTabShown('auswertung-tab'); } } } // Angepasst, Auswertungstab neu rendern
...
function handleExportAction(exportType) {
    ...
    switch (exportType) {
        ...
        case 'daten-md': exportService.exportTableMarkdown(dataForExport, 'patienten', currentKollektiv); break; // Angepasst
...
function handlePublikationSpracheChange(switchElement) { // NEU
    const newLang = switchElement.checked ? 'en' : 'de';
    if (state.setCurrentPublikationLang(newLang)) {
        ui_helpers.updatePublikationLangSwitchUI(newLang);
        if (state.getActiveTabId() === 'publikation-tab') {
             const activeSubNavButton = document.querySelector('#publikationSubNav .nav-link.active');
             const activeSectionKey = activeSubNavButton ? activeSubNavButton.dataset.sectionKey : 'methoden';
             publicationTabRenderer.renderPublicationTabContent(processedData, lastBruteForceResults, state.getCurrentKollektiv(), t2CriteriaManager.getAppliedCriteria(), t2CriteriaManager.getAppliedLogic(), newLang, activeSectionKey);
        }
    }
}
function handlePublikationSubNavClick(button) { // NEU
    const sectionKey = button.dataset.sectionKey;
    if (sectionKey) {
        const subNavContainer = document.getElementById('publikationSubNav');
        if(subNavContainer) {
            subNavContainer.querySelectorAll('.nav-link').forEach(btn => btn.classList.remove('active'));
        }
        button.classList.add('active');

        // Update content based on new active section
        publicationTabRenderer.renderPublicationTabContent(
            processedData,
