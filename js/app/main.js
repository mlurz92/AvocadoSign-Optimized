...
function initializeApp() {
    ...
    try {
        ...
        lastBruteForceResults = state.getAggregatedBruteForceResults();
        ...
        const initialTabElement = document.getElementById(initialTabId);
         if(initialTabElement && typeof bootstrap !== 'undefined' && bootstrap.Tab) {
            ...
            if(tab) tab.show();
             ...
             if(fallbackTabElement && typeof bootstrap !== 'undefined' && bootstrap.Tab) {
                const fallbackTab = bootstrap.Tab.getOrCreateInstance(fallbackTabElement);
                if(fallbackTab) fallbackTab.show();
             }
         }
        ...
        console.log("App Initialisierung abgeschlossen.");
         ...
         ui_helpers.updateElementHTML('app-container', `<div class="alert alert-danger m-5">Initialisierungsfehler: ${error.message}.<br><pre>${error.stack || ''}</pre> Stellen Sie sicher, dass alle Skripte korrekt geladen wurden und die Dateipfade in index.html aktuell sind.</div>`);
    }
}
...
function updateUIState() {
    try {
        const currentKollektiv = state.getCurrentKollektiv();
        const headerStats = dataProcessor.calculateHeaderStats(currentData, currentKollektiv); // currentData is already filtered and evaluated
        ...
        ui_helpers.updateExportButtonStates(state.getActiveTabId(), !!lastBruteForceResults && Object.values(lastBruteForceResults).some(r => r !== null && r.results && r.results.length > 0), currentData && currentData.length > 0);
    } catch (error) {
...
function handleBodyClickDelegation(event) {
    ...
    // Prevent row collapse when clicking interactive elements within a clickable row
    ...
    if (clickableRowParent && target.closest('a, button, input, select, .btn-close, [data-bs-toggle="modal"], .table-download-png-btn, .chart-download-btn')) {
        event.stopPropagation(); // Stop event from bubbling to the row
        // Do not return; allow the specific handler for the button/link to proceed
    }


    if (kollektivButton && kollektivButton.dataset.kollektiv) { handleKollektivChange(kollektivButton.dataset.kollektiv); return; }
    if (closestHeader && (!closestSubHeader || !target.closest('.sortable-sub-header'))) { handleSortClick(closestHeader, null); return; } // Sort by main header if not a sub-header click
    if (closestSubHeader) { handleSortClick(closestHeader, closestSubHeader); return; } // Sort by sub-header
    ...
    if (modalExportBtn && !modalExportBtn.disabled) {
        const currentBfKollektiv = state.getCurrentKollektiv();
        const bfDataForModal = lastBruteForceResults ? lastBruteForceResults[currentBfKollektiv] : null;
        if (bfDataForModal && bfDataForModal.results && bfDataForModal.results.length > 0) {
            exportService.exportBruteForceReport(bfDataForModal, currentBfKollektiv);
        } else {
            ui_helpers.showToast("Keine Brute-Force-Daten für Export aus Modal verfügbar.", "warning");
        }
        return;
    }
...
function handleTabShownEvent(event) {
    if (event.target && event.target.id && state.setActiveTabId(event.target.id)) {
        filterAndPrepareData(); // Prepare data based on new tab's sort state potential
        updateUIState();
...
function handleTabShown(tabId) {
    ...
    switch (tabId) {
        ...
        case 'publikation-tab':
            const aggregatedBfResults = state.getAggregatedBruteForceResults();
            publikationTabLogic.initializeData(localRawData, appliedCriteria, appliedLogic, aggregatedBfResults);
            viewRenderer.renderPublikationTab(state.getCurrentPublikationLang(), state.getCurrentPublikationSection(), currentKollektiv, localRawData, aggregatedBfResults);
            break;
    ...
    updateUIState(); // Call after rendering to ensure UI elements exist for updates
}

function handleKollektivChange(newKollektiv) { if (state.setCurrentKollektiv(newKollektiv)) { filterAndPrepareData(); updateUIState(); handleTabShown(state.getActiveTabId()); ui_helpers.showToast(`Kollektiv '${getKollektivDisplayName(newKollektiv)}' ausgewählt.`, 'info', 2000); return true; } return false; }
...
function handleApplyCriteria() { t2CriteriaManager.applyCriteria(); filterAndPrepareData(); ui_helpers.markCriteriaSavedIndicator(false); updateUIState(); handleTabShown(state.getActiveTabId()); publikationTabLogic.initializeData( localRawData, t2CriteriaManager.getAppliedCriteria(), t2CriteriaManager.getAppliedLogic(), state.getAggregatedBruteForceResults()); ui_helpers.showToast('T2-Kriterien angewendet & gespeichert.', 'success'); }
function handleApplyBestBfCriteria() { const currentBfKollektiv = state.getCurrentKollektiv(); const bfDataForKollektiv = lastBruteForceResults ? lastBruteForceResults[currentBfKollektiv] : null; if (!bfDataForKollektiv?.bestResult?.criteria) { ui_helpers.showToast('Keine gültigen Brute-Force-Ergebnisse für dieses Kollektiv zum Anwenden.', 'warning'); return; } const best = bfDataForKollektiv.bestResult; Object.keys(best.criteria).forEach(key => { if(key === 'logic') return; const criterion = best.criteria[key]; t2CriteriaManager.toggleCriterionActive(key, criterion.active); if(criterion.active) { if(key === 'size') t2CriteriaManager.updateCriterionThreshold(criterion.threshold); else t2CriteriaManager.updateCriterionValue(key, criterion.value); } }); t2CriteriaManager.updateLogic(best.logic); ui_helpers.updateT2CriteriaControlsUI(t2CriteriaManager.getCurrentCriteria(), t2CriteriaManager.getCurrentLogic()); handleApplyCriteria(); ui_helpers.showToast('Beste Brute-Force Kriterien für dieses Kollektiv angewendet & gespeichert.', 'success'); }
...
function handlePresentationDownloadClick(button) { const actionId = button.id; const currentKollektiv = state.getCurrentKollektiv(); let presentationData = null; const filteredData = dataProcessor.filterDataByKollektiv(processedData, currentKollektiv); const allKollektiveProcessedData = processedData; if (filteredData?.length > 0) { const appliedCriteria = t2CriteriaManager.getAppliedCriteria(); const appliedLogic = t2CriteriaManager.getAppliedLogic(); const allKollektivStats = statisticsService.calculateAllStatsForPublication(allKollektiveProcessedData, appliedCriteria, appliedLogic, state.getAggregatedBruteForceResults()); presentationData = { kollektiv: currentKollektiv, patientCount: filteredData.length, statsGesamt: allKollektivStats.Gesamt?.gueteAS, statsDirektOP: allKollektivStats['direkt OP']?.gueteAS, statsNRCT: allKollektivStats.nRCT?.gueteAS, statsCurrentKollektiv: allKollektivStats[currentKollektiv]?.gueteAS, statsAS: allKollektivStats[currentKollektiv]?.gueteAS }; if (state.getCurrentPresentationView() === 'as-vs-t2') { const studyId = state.getCurrentPresentationStudyId(); let studySet = null; const isApplied = studyId === APP_CONFIG.SPECIAL_IDS.APPLIED_CRITERIA_STUDY_ID; if(isApplied) { studySet = { criteria: appliedCriteria, logic: appliedLogic, id: studyId, name: APP_CONFIG.SPECIAL_IDS.APPLIED_CRITERIA_DISPLAY_NAME, displayShortName: 'Angewandt', studyInfo: { reference: "Benutzerdefiniert", patientCohort: `Aktuell: ${getKollektivDisplayName(currentKollektiv)} (N=${presentationData.patientCount})`, investigationType: "N/A", focus: "Benutzereinstellung", keyCriteriaSummary: studyT2CriteriaManager.formatCriteriaForDisplay(appliedCriteria, appliedLogic) || "Keine" } }; presentationData.statsT2 = allKollektivStats[currentKollektiv]?.gueteT2_angewandt; presentationData.vergleich = allKollektivStats[currentKollektiv]?.vergleichASvsT2_angewandt; } else { studySet = studyT2CriteriaManager.getStudyCriteriaSetById(studyId); if(studySet) { const tempEvaluatedData = studyT2CriteriaManager.applyStudyT2CriteriaToDataset(cloneDeep(filteredData), studySet); presentationData.statsT2 = statisticsService.calculateDiagnosticPerformance(tempEvaluatedData, 't2', 'n'); tempEvaluatedData.forEach((p,idx) => { if(filteredData[idx]) p.as = filteredData[idx].as; }); presentationData.vergleich = statisticsService.compareDiagnosticMethods(tempEvaluatedData, 'as', 't2', 'n');} } if (studySet) { presentationData.comparisonCriteriaSet = studySet; presentationData.t2CriteriaLabelShort = studySet.displayShortName || 'T2'; presentationData.t2CriteriaLabelFull = `${isApplied ? 'Aktuell angewandt' : (studySet.name || 'Studie')}: ${studyT2CriteriaManager.formatCriteriaForDisplay(studySet.criteria, studySet.logic)}`; } } } exportService.exportPraesentationData(actionId, presentationData, currentKollektiv); }
function handleExportAction(exportType) { filterAndPrepareData(); const dataForExport = currentData; const currentKollektiv = state.getCurrentKollektiv(); const appliedCriteria = t2CriteriaManager.getAppliedCriteria(); const appliedLogic = t2CriteriaManager.getAppliedLogic(); const aggregatedBfResults = state.getAggregatedBruteForceResults(); const canExportData = Array.isArray(dataForExport) && dataForExport.length > 0; const hasAnyBfResults = !!aggregatedBfResults && Object.values(aggregatedBfResults).some(r => r !== null && r.results && r.results.length > 0); if (!canExportData && !['bruteforce-txt', 'all-zip', 'md-zip', 'html', 'png-zip', 'svg-zip'].includes(exportType) && !(exportType.endsWith('-zip') && hasAnyBfResults) ) { ui_helpers.showToast("Keine Daten für diesen Export verfügbar.", "warning"); return; } if (exportType === 'bruteforce-txt' && (!aggregatedBfResults || !aggregatedBfResults[currentKollektiv] || !aggregatedBfResults[currentKollektiv].results || aggregatedBfResults[currentKollektiv].results.length === 0)) { ui_helpers.showToast("Keine Brute-Force Ergebnisse für dieses Kollektiv zum Exportieren.", "warning"); return; } switch (exportType) { case 'statistik-csv': exportService.exportStatistikCSV(localRawData, currentKollektiv, appliedCriteria, appliedLogic, aggregatedBfResults); break; case 'bruteforce-txt': exportService.exportBruteForceReport(aggregatedBfResults[currentKollektiv], currentKollektiv); break; case 'deskriptiv-md': { const stats = statisticsService.calculateAllStatsForPublication(localRawData, appliedCriteria, appliedLogic, aggregatedBfResults)[currentKollektiv]; exportService.exportTableMarkdown(stats?.deskriptiv, 'deskriptiv', currentKollektiv); break; } case 'daten-md': exportService.exportTableMarkdown(dataForExport, 'daten', currentKollektiv); break; case 'auswertung-md': exportService.exportTableMarkdown(dataForExport, 'auswertung', currentKollektiv, appliedCriteria, appliedLogic); break; case 'filtered-data-csv': exportService.exportFilteredDataCSV(dataForExport, currentKollektiv); break; case 'comprehensive-report-html': exportService.exportComprehensiveReportHTML(localRawData, aggregatedBfResults, currentKollektiv, appliedCriteria, appliedLogic); break; case 'charts-png': case 'png-zip': exportService.exportChartsZip('#app-container', 'PNG_ZIP', currentKollektiv, 'png'); break; case 'charts-svg': case 'svg-zip': exportService.exportChartsZip('#app-container', 'SVG_ZIP', currentKollektiv, 'svg'); break; case 'all-zip': case 'csv-zip': case 'md-zip': exportService.exportCategoryZip(exportType, localRawData, aggregatedBfResults, currentKollektiv, appliedCriteria, appliedLogic); break; default: console.warn(`Unbekannter Export-Typ: ${exportType}`); ui_helpers.showToast(`Export-Typ '${exportType}' nicht implementiert.`, 'warning'); break; } }
...
function initializeBruteForceWorker() { if (!window.Worker) { ui_helpers.showToast("Web Worker nicht unterstützt.", "danger"); ui_helpers.updateBruteForceUI('error', {message: 'Web Worker not supported'}, false, state.getCurrentKollektiv()); return; } try { if(bruteForceWorker) bruteForceWorker.terminate(); bruteForceWorker = new Worker(APP_CONFIG.PATHS.BRUTE_FORCE_WORKER); bruteForceWorker.onmessage = handleBruteForceMessage; bruteForceWorker.onerror = handleBruteForceError; bruteForceWorker.onmessageerror = (e) => console.error("Worker messageerror:", e); ui_helpers.updateBruteForceUI('idle', {kollektiv: state.getCurrentKollektiv()}, true, state.getCurrentKollektiv()); } catch (e) { console.error("Fehler Initialisierung Brute Force Worker:", e); ui_helpers.showToast("Fehler: Worker Start fehlgeschlagen.", "danger"); bruteForceWorker = null; ui_helpers.updateBruteForceUI('error', {message: 'Worker init failed'}, false, state.getCurrentKollektiv()); } }
function handleStartBruteForce() { if (isBruteForceRunning || !bruteForceWorker) return; isBruteForceRunning = true; const metric = document.getElementById('brute-force-metric')?.value || APP_CONFIG.DEFAULT_SETTINGS.BRUTE_FORCE_METRIC; const currentKollektiv = state.getCurrentKollektiv(); const dataForWorker = dataProcessor.filterDataByKollektiv(processedData, currentKollektiv).map(p => ({ nr: p.nr, n: p.n, lymphknoten_t2: p.lymphknoten_t2 })); if (dataForWorker.length === 0) { ui_helpers.showToast("Keine Daten für Optimierung.", "warning"); isBruteForceRunning = false; ui_helpers.updateBruteForceUI('idle', {}, !!bruteForceWorker, currentKollektiv); return; } ui_helpers.updateBruteForceUI('start', { metric: metric, kollektiv: currentKollektiv }, true, currentKollektiv); bruteForceWorker.postMessage({ action: 'start', payload: { data: dataForWorker, metric: metric, kollektiv: currentKollektiv, t2SizeRange: APP_CONFIG.T2_CRITERIA_SETTINGS.SIZE_RANGE } }); updateUIState(); }
function handleCancelBruteForce() { if (!isBruteForceRunning || !bruteForceWorker) return; bruteForceWorker.postMessage({ action: 'cancel' }); isBruteForceRunning = false; ui_helpers.updateBruteForceUI('cancelled', {kollektiv: state.getCurrentKollektiv()}, true, state.getCurrentKollektiv()); ui_helpers.showToast('Optimierung abgebrochen.', 'warning'); updateUIState(); }
function handleBruteForceMessage(event) { if (!event || !event.data) return; const { type, payload } = event.data; const currentKollektivForUI = payload?.kollektiv || state.getCurrentKollektiv(); const currentMetric = payload?.metric || (type === 'progress' ? payload?.currentBest?.metric : null) || document.getElementById('brute-force-metric')?.value || APP_CONFIG.DEFAULT_SETTINGS.BRUTE_FORCE_METRIC; if (!isBruteForceRunning && type !== 'cancelled' && type !== 'error' && type !== 'result') { if(type === 'progress' || type === 'started') { /* Worker might send residual messages if cancelled late */ return; } } switch (type) { case 'started': ui_helpers.updateBruteForceUI('started', { ...payload, metric: currentMetric }, true, currentKollektivForUI); break; case 'progress': ui_helpers.updateBruteForceUI('progress', { ...payload, metric: currentMetric }, true, currentKollektivForUI); break; case 'result': isBruteForceRunning = false; if (payload && payload.kollektiv && payload.results) { state.updateAggregatedBruteForceResultForKollektiv(payload.kollektiv, payload); lastBruteForceResults = state.getAggregatedBruteForceResults(); publikationTabLogic.initializeData( localRawData, t2CriteriaManager.getAppliedCriteria(), t2CriteriaManager.getAppliedLogic(), lastBruteForceResults); } ui_helpers.updateBruteForceUI('result', payload, true, currentKollektivForUI); if(payload?.results?.length > 0 && payload.bestResult && isFinite(payload.bestResult.metricValue)){ const modalBody = document.querySelector('#brute-force-modal .modal-body'); if (modalBody) { modalBody.innerHTML = uiComponents.createBruteForceModalContent(payload.results, payload.metric, payload.kollektiv, payload.duration, payload.totalTested); ui_helpers.initializeTooltips(modalBody); } ui_helpers.showToast('Optimierung abgeschlossen.', 'success'); if (state.getActiveTabId() === 'publikation-tab' || state.getActiveTabId() === 'auswertung-tab' || state.getActiveTabId() === 'statistik-tab') { handleTabShown(state.getActiveTabId()); } } else { ui_helpers.showToast(`Optimierung für ${getKollektivDisplayName(currentKollektivForUI)} ohne valide Ergebnisse für Metrik ${currentMetric}.`, 'warning'); } updateUIState(); break; case 'cancelled': isBruteForceRunning = false; ui_helpers.updateBruteForceUI('cancelled', {kollektiv: currentKollektivForUI}, true, currentKollektivForUI); updateUIState(); break; case 'error': isBruteForceRunning = false; ui_helpers.showToast(`Optimierungsfehler: ${payload?.message || 'Unbekannt'}`, 'danger'); ui_helpers.updateBruteForceUI('error', payload, true, currentKollektivForUI); updateUIState(); break; default: console.warn(`Unbekannter Worker Nachrichtentyp: ${type}`); } }
function handleBruteForceError(error) { console.error("Fehler im Brute Force Worker:", error); isBruteForceRunning = false; const currentKollektiv = state.getCurrentKollektiv(); ui_helpers.showToast("Fehler: Hintergrundverarbeitung gestoppt.", "danger"); ui_helpers.updateBruteForceUI('error', { message: error.message || 'Worker Error', kollektiv: currentKollektiv }, !!bruteForceWorker, currentKollektiv); if (bruteForceWorker) { bruteForceWorker.terminate(); initializeBruteForceWorker(); } updateUIState(); }
