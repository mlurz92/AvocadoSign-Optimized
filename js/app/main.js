...
let lastBruteForceResults = null; // Wird nun die aggregierten Ergebnisse halten
...
function initializeApp() {
    ...
    try {
        ...
        lastBruteForceResults = state.getAggregatedBruteForceResults(); // Initialisiere mit gespeicherten Ergebnissen
        ...
        publikationTabLogic.initializeData(
            localRawData, // Übergib rohe Daten für die Möglichkeit, sie neu zu prozessieren, falls nötig
            t2CriteriaManager.getAppliedCriteria(),
            t2CriteriaManager.getAppliedLogic(),
            lastBruteForceResults // Übergib die aggregierten BF Ergebnisse
        );
...
function updateUIState() {
    try {
        ...
        // lastBruteForceResults wird hier verwendet, um zu prüfen, ob *irgendwelche* BF-Ergebnisse existieren
        ui_helpers.updateExportButtonStates(state.getActiveTabId(), !!lastBruteForceResults && Object.values(lastBruteForceResults).some(r => r !== null), currentData && currentData.length > 0);
    } catch (error) {
...
function handleBodyClickDelegation(event) {
    ...
    if (modalExportBtn && !modalExportBtn.disabled) {
        const currentBfKollektiv = state.getCurrentKollektiv(); // Or get the specific kollektiv for which modal is shown if different
...
function handleTabShown(tabId) {
    ...
    switch (tabId) {
        ...
        case 'publikation-tab':
            // Stelle sicher, dass die aktuellsten aggregierten BF Ergebnisse verwendet werden
...
function handleBruteForceMessage(event) { if (!event || !event.data) return; const { type, payload } = event.data; if (!isBruteForceRunning && type !== 'cancelled' && type !== 'error' && type !== 'result') return; const currentKollektivForUI = payload?.kollektiv || state.getCurrentKollektiv(); const currentMetric = payload?.metric || (type === 'progress' ? payload?.currentBest?.metric : null) || document.getElementById('brute-force-metric')?.value || 'Balanced Accuracy'; switch (type) { case 'started': ui_helpers.updateBruteForceUI('started', { ...payload, metric: currentMetric }, true, currentKollektivForUI); break; case 'progress': ui_helpers.updateBruteForceUI('progress', { ...payload, metric: currentMetric }, true, currentKollektivForUI); break; case 'result': isBruteForceRunning = false; if (payload && payload.kollektiv && payload.results) { state.updateAggregatedBruteForceResultForKollektiv(payload.kollektiv, payload); lastBruteForceResults = state.getAggregatedBruteForceResults(); publikationTabLogic.initializeData( localRawData, t2CriteriaManager.getAppliedCriteria(), t2CriteriaManager.getAppliedLogic(), lastBruteForceResults); } ui_helpers.updateBruteForceUI('result', payload, true, currentKollektivForUI); if(payload?.results?.length > 0){ const modalBody = document.querySelector('#brute-force-modal .modal-body'); if (modalBody) { modalBody.innerHTML = uiComponents.createBruteForceModalContent(payload.results, payload.metric, payload.kollektiv, payload.duration, payload.totalTested); ui_helpers.initializeTooltips(modalBody); } ui_helpers.showToast('Optimierung abgeschlossen.', 'success'); if (state.getActiveTabId() === 'publikation-tab') { handleTabShown('publikation-tab'); } } else { ui_helpers.showToast('Optimierung ohne valide Ergebnisse.', 'warning'); } updateUIState(); break; case 'cancelled': isBruteForceRunning = false; ui_helpers.updateBruteForceUI('cancelled', {}, true, currentKollektivForUI); updateUIState(); break; case 'error': isBruteForceRunning = false; ui_helpers.showToast(`Optimierungsfehler: ${payload?.message || 'Unbekannt'}`, 'danger'); ui_helpers.updateBruteForceUI('error', payload, true, currentKollektivForUI); updateUIState(); break; default: console.warn(`Unbekannter Worker Nachrichtentyp: ${type}`); } }
