...
let lastBruteForceResults = null; // Kann ein einzelnes Ergebnisobjekt oder ein Objekt mit Kollektiv-Schlüsseln sein
...
function initializeApp() {
    ...
    try {
        ...
        // Initial data for publication tab (might be null if no prior BF results)
        // The structure for bfResultsPerKollektiv should be { "Gesamt": bfResultGesamt, "direkt OP": bfResultDirektOP, ... }
        // If lastBruteForceResults is a single run, adapt it. If it's already structured, use as is.
        ...
        if (lastBruteForceResults) {
            if (lastBruteForceResults.kollektiv && !lastBruteForceResults[lastBruteForceResults.kollektiv]) {
                 ...
                 initialBruteForceResultsForPubTab = lastBruteForceResults; // Assumes it's already in the correct format or null
            }
        }


        publikationTabLogic.initializeData(
            processedData, // Publikation sollte auf prozessierten Daten basieren für Konsistenz
...
function handleBodyClickDelegation(event) {
    ...
    if (clickableRowParent && target.closest('a, button, input, select, .btn-close, [data-bs-toggle="modal"], .table-download-png-btn, .chart-download-btn')) {
        event.stopPropagation(); // Prevent row collapse if clicking on an interactive element within the row
        return;
    }
...
function handleTabShownEvent(event) {
    if (event.target && event.target.id && state.setActiveTabId(event.target.id)) {
        filterAndPrepareData(); // Ensure currentData is up-to-date for the new tab
        updateUIState(); // Update general UI elements like header stats
...
function handleTabShown(tabId) {
    ...
    switch (tabId) {
        ...
        case 'publikation-tab':
            ...
            // Re-initialize publication logic data in case BF results changed or criteria changed
            publikationTabLogic.initializeData(
                processedData,
                appliedCriteria,
                appliedLogic,
        ...
        default: const paneId = tabId.replace('-tab', '-tab-pane'); ui_helpers.updateElementHTML(paneId, `<div class="alert alert-warning m-3">Inhalt für Tab '${tabId}' nicht implementiert.</div>`);
    }
    updateUIState(); // Ensure export buttons etc. are updated after tab content is rendered
}
...
function handleExportAction(exportType) { filterAndPrepareData(); const dataForExport = currentData; const currentKollektiv = state.getCurrentKollektiv(); const appliedCriteria = t2CriteriaManager.getAppliedCriteria(); const appliedLogic = t2CriteriaManager.getAppliedLogic(); const canExport = Array.isArray(processedData) && processedData.length > 0; if (!canExport && !['bruteforce-txt', 'all-zip', 'png-zip', 'svg-zip', 'csv-zip', 'md-zip'].includes(exportType) && exportType !== 'comprehensive-report-html') { ui_helpers.showToast("Keine Daten für diesen Export verfügbar.", "warning"); return; } if (exportType === 'bruteforce-txt' && !lastBruteForceResults) { ui_helpers.showToast("Keine Brute-Force Ergebnisse für Export.", "warning"); return; } if (['all-zip', 'png-zip', 'svg-zip', 'csv-zip', 'md-zip'].includes(exportType) && !canExport && !lastBruteForceResults && exportType !== 'md-zip' /* md-zip can contain publication texts */) { ui_helpers.showToast("Keine Daten/Ergebnisse für ZIP-Export.", "warning"); return; } if (exportType === 'comprehensive-report-html' && !canExport) { ui_helpers.showToast("Keine Daten für HTML-Report.", "warning"); return; } switch (exportType) { case 'statistik-csv': exportService.exportStatistikCSV(processedData, currentKollektiv, appliedCriteria, appliedLogic); break; case 'bruteforce-txt': exportService.exportBruteForceReport(lastBruteForceResults, currentKollektiv); break; case 'deskriptiv-md': { const stats = statisticsService.calculateAllStatsForPublication(processedData, appliedCriteria, appliedLogic, lastBruteForceResults ? {[currentKollektiv]: lastBruteForceResults} : null)[currentKollektiv]; exportService.exportTableMarkdown(stats?.deskriptiv, 'deskriptiv', currentKollektiv); break; } case 'daten-md': exportService.exportTableMarkdown(dataForExport, 'daten', currentKollektiv); break; case 'auswertung-md': exportService.exportTableMarkdown(dataForExport, 'auswertung', currentKollektiv, appliedCriteria, appliedLogic); break; case 'filtered-data-csv': exportService.exportFilteredDataCSV(dataForExport, currentKollektiv); break; case 'comprehensive-report-html': exportService.exportComprehensiveReportHTML(processedData, lastBruteForceResults, currentKollektiv, appliedCriteria, appliedLogic); break; case 'charts-png': exportService.exportChartsZip('#app-container', 'PNG_ZIP', currentKollektiv, 'png'); break; case 'charts-svg': exportService.exportChartsZip('#app-container', 'SVG_ZIP', currentKollektiv, 'svg'); break; case 'all-zip': case 'csv-zip': case 'md-zip': exportService.exportCategoryZip(exportType, processedData, lastBruteForceResults, currentKollektiv, appliedCriteria, appliedLogic); break; default: ui_helpers.showToast(`Export-Typ '${exportType}' nicht implementiert.`, 'warning'); break; } }
