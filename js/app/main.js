...
function handleBodyClickDelegation(event) {
    ...
    if (modalExportBtn && !modalExportBtn.disabled) { exportService.exportBruteForceReport(lastBruteForceResults[state.getCurrentKollektiv()], state.getCurrentKollektiv()); return; }
...
function handleTabShown(tabId) {
    ...
    switch (tabId) {
        ...
        case 'publikation-tab':
            ...
            if (lastBruteForceResults) {
                bfResultsForPubTab = lastBruteForceResults; // Pass the whole object
            }
            publikationTabLogic.initializeData(
                processedData,
                appliedCriteria,
...
function handleApplyBestBfCriteria() {
    ...
    if (!relevantBFPayload?.bestResult?.criteria) {
        ui_helpers.showToast('Keine gültigen Brute-Force-Ergebnisse für das aktuelle Kollektiv zum Anwenden.', 'warning');
        return;
    }
    ...
    handleApplyCriteria(); // This already saves and re-renders
    ui_helpers.showToast('Beste Brute-Force Kriterien für Kollektiv "' + getKollektivDisplayName(currentActiveKollektiv) + '" angewendet & gespeichert.', 'success');
}
