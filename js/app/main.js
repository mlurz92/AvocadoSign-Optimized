class App {
    ...
    async init() {
        try {
            ...
            Object.entries(this.libraryStatus).forEach(([lib, status]) => {
                if (!status && ['JSZip', 'docx', 'canvg'].includes(lib)) {
    ...
    checkDependencies() {
        return new Promise((resolve) => {
            const internalModules = { 
                ...
                stardGenerator: window.stardGenerator, publicationService: window.publicationService, exportService: window.exportService,
                ...
                statisticsTab: window.statisticsTab, comparisonTab: window.comparisonTab, publicationTab: window.publicationTab, exportTab: window.exportTab,
            ...
            const intervalId = setInterval(() => {
                const allCoreLoaded = !!window.d3 && !!window.tippy;

                if (allCoreLoaded || elapsedTime >= timeout) {
    ...
    handleExportRequest(exportType) {
        const appData = {
            currentCohortData: this.currentCohortData,
            allPublicationStats: this.allPublicationStats,
            bruteForceResults: window.bruteForceManager.getAllResults(),
            publicationHTML: this.preRenderedPublicationHTML,
            currentCohortId: window.state.getCurrentCohort()
        };
        window.exportService.triggerExport(exportType, appData);
    }
