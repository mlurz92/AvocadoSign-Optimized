const statisticsService = (() => {
    ...
    function normalCDF(x, mean = 0, stdDev = 1) {
        ...
        if (stdDev === 0) return x < mean ? 0.0 : (x > mean ? 1.0 : 0.5) ;
    ...
    function inverseNormalCDF(p, mean = 0, stdDev = 1) {
        ...
        const c_const = [-7.784894002430293e-03, -3.223964580411365e-01, -2.400758277161838e+00, -2.549732539343734e+00, 4.374664141464968e+00, 2.938163982698783e+00];
        const d_const = [7.784695709041462e-03, 3.224671290700398e-01, 2.445134137142996e+00, 3.754408661907416e+00];
        const p_low = 0.02425, p_high = 1 - p_low;
        let q_val, r_val, x_val;
        if (p < p_low) {
            q_val = Math.sqrt(-2 * Math.log(p));
            x_val = (((((c_const[0] * q_val + c_const[1]) * q_val + c_const[2]) * q_val + c_const[3]) * q_val + c_const[4]) * q_val + c_const[5]) / ((((d_const[0] * q_val + d_const[1]) * q_val + d_const[2]) * q_val + d_const[3]) * q_val + 1);
        } else if (p <= p_high) {
            q_val = p - 0.5; r_val = q_val * q_val;
            x_val = (((((a[0] * r_val + a[1]) * r_val + a[2]) * r_val + a[3]) * r_val + a[4]) * r_val + a[5]) * q_val / (((((b[0] * r_val + b[1]) * r_val + b[2]) * r_val + b[3]) * r_val + b[4]) * r_val + 1);
        } else {
            q_val = Math.sqrt(-2 * Math.log(1 - p));
            x_val = -(((((c_const[0] * q_val + c_const[1]) * q_val + c_const[2]) * q_val + c_const[3]) * q_val + c_const[4]) * q_val + c_const[5]) / ((((d_const[0] * q_val + d_const[1]) * q_val + d_const[2]) * q_val + d_const[3]) * q_val + 1);
        }
    ...
    function logGamma(xx) {
        ...
        let x_val = xx, y_val = x_val, tmp = x_val + 5.5;
        tmp -= (x_val + 0.5) * Math.log(tmp);
        let ser = 1.000000000190015;
        for (let j = 0; j <= 5; j++) ser += cof[j] / ++y_val;
        const result = -tmp + Math.log(2.5066282746310005 * ser / x_val);
    ...
    function regularizedGammaIncomplete(a, x) {
        ...
        if (x < a + 1.0) {
            ...
            const expVal = Math.exp(a * Math.log(x) - x - logGammaA);
            return isFinite(expVal) ? Math.max(0.0, Math.min(1.0, sum * expVal)) : NaN;
        } else {
            let b_val = x + 1.0 - a, c_val = 1.0 / epsilon, d_val = 1.0 / b_val, h_val = d_val, an_val, del_val;
            for (let k = 1; k <= maxIterations; k++) {
                an_val = -k * (k - a); b_val += 2.0; d_val = an_val * d_val + b_val;
                if (Math.abs(d_val) < epsilon) d_val = epsilon;
                c_val = b_val + an_val / c_val; if (Math.abs(c_val) < epsilon) c_val = epsilon;
                d_val = 1.0 / d_val; del_val = d_val * c_val; h_val *= del_val;
                if (Math.abs(del_val - 1.0) < epsilon) break;
            }
            const expVal = Math.exp(a * Math.log(x) - x - logGammaA);
            return isFinite(expVal) ? Math.max(0.0, Math.min(1.0, 1.0 - (expVal * h_val))) : NaN;
        }
    }
    ...
    function calculateORCI(a, b_val, c_val, d_val, alpha = APP_CONFIG.STATISTICAL_CONSTANTS.BOOTSTRAP_CI_ALPHA) {
        const defaultReturn = { value: NaN, ci: null, method: 'Woolf Logit (Adjusted)' };
        if (isNaN(a) || isNaN(b_val) || isNaN(c_val) || isNaN(d_val) || a < 0 || b_val < 0 || c_val < 0 || d_val < 0) return defaultReturn;
        const or_raw = (b_val === 0 || c_val === 0) ? ((a > 0 && d_val > 0 && (b_val + c_val === 0)) ? Infinity : NaN) : (a * d_val) / (b_val * c_val);
        const a_adj = a + 0.5, b_adj = b_val + 0.5, c_adj = c_val + 0.5, d_adj = d_val + 0.5;
    ...
    function calculateRDCI(a, b_val, c_val, d_val, alpha = APP_CONFIG.STATISTICAL_CONSTANTS.BOOTSTRAP_CI_ALPHA) {
        const defaultReturn = { value: NaN, ci: null, method: 'Wald' };
        if (isNaN(a) || isNaN(b_val) || isNaN(c_val) || isNaN(d_val) || a < 0 || b_val < 0 || c_val < 0 || d_val < 0) return defaultReturn;
        const n1 = a + b_val, n2 = c_val + d_val;
        if (n1 === 0 || n2 === 0) return defaultReturn;
        const p1 = a / n1, p2 = c_val / n2; const rd = p1 - p2;
    ...
    function bootstrapCI(data, statisticFn, nBoot = APP_CONFIG.STATISTICAL_CONSTANTS.BOOTSTRAP_CI_REPLICATIONS, alpha = APP_CONFIG.STATISTICAL_CONSTANTS.BOOTSTRAP_CI_ALPHA) {
        ...
        bootStats.sort((a_val, b_val) => a_val - b_val);
    ...
    function manualMcNemarTest(b_val, c_val) {
        if (isNaN(b_val) || isNaN(c_val) || b_val < 0 || c_val < 0) return { pValue: NaN, statistic: NaN, df: 1, method: "McNemar's Test (Invalid Input)" };
        const n_discordant = b_val + c_val; if (n_discordant === 0) return { pValue: 1.0, statistic: 0, df: 1, method: "McNemar's Test (No Discordance)" };
        const diff = Math.abs(b_val - c_val);
        let continuityCorrection = 0;
        if (n_discordant < APP_CONFIG.STATISTICAL_CONSTANTS.FISHER_EXACT_THRESHOLD * 4) {
            continuityCorrection = 1;
        }
        let statistic = Math.pow(diff - continuityCorrection, 2) / n_discordant;
        if (isNaN(statistic) || !isFinite(statistic)) statistic = 0;
        const pValue = 1.0 - chiSquareCDF(statistic, 1);
        return { pValue: Math.max(0.0, Math.min(1.0, pValue)), statistic: statistic, df: 1, method: `McNemar's Test${continuityCorrection > 0 ? " (Yates' corrected)" : ''}` };
    }

    function logProbHypergeometric(k_val, N_pop, K_success, n_draw) {
        if (k_val < 0 || n_draw < 0 || K_success < 0 || N_pop < 0 || k_val > n_draw || k_val > K_success || n_draw - k_val > N_pop - K_success || n_draw > N_pop) return -Infinity;
        try {
            const logC_K_k = logFactorial(K_success) - logFactorial(k_val) - logFactorial(K_success - k_val);
            const logC_NK_nk = logFactorial(N_pop - K_success) - logFactorial(n_draw - k_val) - logFactorial((N_pop - K_success) - (n_draw - k_val));
    ...
    function manualFisherExactTest(a_val, b_val, c_val, d_val) {
        if (isNaN(a_val) || isNaN(b_val) || isNaN(c_val) || isNaN(d_val) || a_val < 0 || b_val < 0 || c_val < 0 || d_val < 0) return { pValue: NaN, method: "Fisher's Exact Test (Invalid Input)" };
        const n1_dot = a_val + b_val, k1_dot = a_val + c_val, N_total = a_val + b_val + c_val + d_val;
        if (N_total === 0) return { pValue: 1.0, method: "Fisher's Exact Test (No Data)" };
        const pObservedLog = logProbHypergeometric(a_val, N_total, k1_dot, n1_dot);
        if (!isFinite(pObservedLog)) return { pValue: NaN, method: "Fisher's Exact Test (Numerical Issue in Observed Prob)" };
        let pValue = 0.0; const min_val = Math.max(0, k1_dot - (N_total - n1_dot)), max_val = Math.min(k1_dot, n1_dot); const tolerance = 1e-9;
        for (let i = min_val; i <= max_val; i++) {
            const pCurrentLog = logProbHypergeometric(i, N_total, k1_dot, n1_dot);
    ...
    function rankData(arr) {
        const sorted = arr.map((value, index) => ({ value: parseFloat(value), originalIndex: index })).filter(item => !isNaN(item.value) && isFinite(item.value)).sort((a_val, b_val) => a_val.value - b_val.value);
    ...
    function manualMannWhitneyUTest(sample1, sample2) {
        ...
        const R1 = ranks1.reduce((sum, r) => sum + r, 0); const U1 = n1 * n2 + (n1 * (n1 + 1)) / 2.0 - R1; const U_stat = Math.min(U1, n1 * n2 - U1);
        const meanU = (n1 * n2) / 2.0; const N_total = n1 + n2; const tieGroups = {}; const validRanks = ranks.filter(r => !isNaN(r)); validRanks.forEach(r => { tieGroups[r] = (tieGroups[r] || 0) + 1; });
        const tieCorrectionFactor = Object.values(tieGroups).reduce((sum, t) => t > 1 ? sum + (t * t * t - t) : sum, 0);
        const varU_numerator = n1 * n2 * ((N_total * N_total * N_total - N_total) - tieCorrectionFactor); const varU_denominator = 12.0 * N_total * (N_total - 1.0);
        if (varU_denominator <= 0 || N_total <= 1) return { pValue: 1.0, U: U_stat, Z: 0, testName: "Mann-Whitney U (No Variance)" };
        const varU = varU_numerator / varU_denominator; if (varU < 0 || isNaN(varU)) return { ...defaultReturn, U: U_stat, testName: "Mann-Whitney U (Variance Error)" };
        if (varU === 0) return { pValue: 1.0, U: U_stat, Z: 0, testName: "Mann-Whitney U (Zero Variance)" };
        const stdDevU = Math.sqrt(varU); const correction = (U_stat < meanU) ? 0.5 : (U_stat > meanU) ? -0.5 : 0;
        const z_val = (U_stat - meanU + correction) / stdDevU; if (isNaN(z_val) || !isFinite(z_val)) return { ...defaultReturn, U: U_stat, testName: "Mann-Whitney U (Z Calculation Error)" };
        const pValue = 2.0 * normalCDF(-Math.abs(z_val));
        return { pValue: Math.max(0.0, Math.min(1.0, pValue)), U: U_stat, Z: z_val, testName: "Mann-Whitney U (Normal Approx.)" };
    }
    ...
    function manualDeLongTest(data, key1, key2, referenceKey) {
        ...
        try {
            ...
            const calculateVariance = (V, mean, n_items) => { if (n_items <= 1) return NaN; let sumSqDiff = 0, validCount = 0; for (let i = 0; i < V.length; i++) { if(!isNaN(V[i])) { sumSqDiff += Math.pow(V[i] - mean, 2); validCount++; } } return validCount > 1 ? sumSqDiff / (validCount - 1) : NaN; };
            ...
            const calculateCovariance = (V_X, V_Y, meanX, meanY, n_items) => { if (n_items <= 1) return NaN; let sumProdDiff = 0, validCount = 0; for (let i = 0; i < n_items; i++) { if(i < V_X.length && i < V_Y.length && !isNaN(V_X[i]) && !isNaN(V_Y[i])) { sumProdDiff += (V_X[i] - meanX) * (V_Y[i] - meanY); validCount++; } } return validCount > 1 ? sumProdDiff / (validCount - 1) : NaN; };
            ...
            const seDiff = Math.sqrt(varDiff); const z_val = (auc1 - auc2) / seDiff; if (isNaN(z_val) || !isFinite(z_val)) return { ...defaultReturn, diffAUC: auc1 - auc2, method: "DeLong Test (Z Calculation Error)" };
            const pValue = 2.0 * normalCDF(-Math.abs(z_val));
            return { pValue: Math.max(0.0, Math.min(1.0, pValue)), Z: z_val, diffAUC: auc1 - auc2, method: "DeLong Test" };
        } catch (error) { return { ...defaultReturn, method: "DeLong Test (Error)" }; }
    }

    function calculateZTestForAUCComparison(auc1, se1, n1_val, auc2, se2, n2_val) {
        const defaultReturn = { pValue: NaN, Z: NaN, method: "Z-Test (AUC - Invalid Input)" };
        if (auc1 === null || auc2 === null || se1 === null || se2 === null || isNaN(auc1) || isNaN(auc2) || isNaN(se1) || isNaN(se2) || isNaN(n1_val) || isNaN(n2_val) || se1 < 0 || se2 < 0 || n1_val < 2 || n2_val < 2) return defaultReturn;
        ...
        const seDiff = Math.sqrt(varDiff); const z_val = (auc1 - auc2) / seDiff; if (isNaN(z_val) || !isFinite(z_val)) return { ...defaultReturn, method: "Z-Test (AUC - Z Calculation Error)" };
        const pValue = 2.0 * (1.0 - normalCDF(Math.abs(z_val)));
        return { pValue: Math.max(0.0, Math.min(1.0, pValue)), Z: z_val, method: "Z-Test (AUC)" };
    }
    ...
    function calculatePhi(a_val, b_val, c_val, d_val) {
        if (isNaN(a_val) || isNaN(b_val) || isNaN(c_val) || isNaN(d_val) || a_val < 0 || b_val < 0 || c_val < 0 || d_val < 0) return NaN;
        const n_total = a_val + b_val + c_val + d_val; if (n_total === 0) return NaN;
        const row1 = a_val + b_val, row2 = c_val + d_val, col1 = a_val + c_val, col2 = b_val + d_val;
        const denominator = Math.sqrt(row1 * row2 * col1 * col2); if (denominator === 0 || isNaN(denominator)) return NaN;
        const phi = (a_val * d_val - b_val * c_val) / denominator; return isFinite(phi) ? phi : NaN;
    }

    function calculateDiagnosticPerformance(data, predictionKey, referenceKey) {
        if (!Array.isArray(data) || data.length === 0) {
             return { matrix: {rp:0,fp:0,fn:0,rn:0}, sens: null, spez: null, ppv: null, npv: null, acc: null, balAcc: null, f1: null, auc: null };
        }
        ...
        const bootstrapStatFnFactory = (pKey, rKey, metric) => (sample) => { const m = calculateConfusionMatrix(sample, pKey, rKey); const t = m.rp + m.fp + m.fn + m.rn; if (t === 0) return NaN; const se_val = (m.rp + m.fn) > 0 ? m.rp / (m.rp + m.fn) : NaN; const sp_val = (m.fp + m.rn) > 0 ? m.rn / (m.fp + m.rn) : NaN; if (isNaN(se_val) || isNaN(sp_val)) return NaN; if (metric === 'balAcc' || metric === 'auc') return (se_val + sp_val) / 2.0; if (metric === 'f1') { const pv_val = (m.rp + m.fp) > 0 ? m.rp / (m.rp + m.fp) : NaN; return (isNaN(pv_val) || (pv_val + se_val) <= 1e-9) ? NaN : 2.0 * (pv_val * se_val) / (pv_val + se_val); } return NaN; };
    ...
    function compareDiagnosticMethods(data, key1, key2, referenceKey) {
        ...
        let b = 0, c = 0;
        data.forEach(p => { if (p && typeof p === 'object') { const p1_val = p[key1] === '+'; const p2_val = p[key2] === '+'; const v1 = p[key1] === '+' || p[key1] === '-'; const v2 = p[key2] === '+' || p[key2] === '-'; const vr = p[referenceKey] === '+' || p[referenceKey] === '-'; if (v1 && v2 && vr) { if (p1_val && !p2_val) b++; if (!p1_val && p2_val) c++; } } });
    ...
    function calculateAssociations(data, t2Criteria) {
        ...
        else { results.as = {...nullReturn, testName: "Fisher's Exact Test (Keine Daten für AS)"}; }
        ...
        t2Features.forEach(feature => {
            let featureValueForTest = null, thresholdForTest = null, conditionForTest = '>='; let featureNameForDisplay = `T2: ${feature}`;
            if (t2Criteria[feature]) { if (feature === 'size') { thresholdForTest = t2Criteria[feature].threshold; conditionForTest = t2Criteria[feature].condition || '>='; if (thresholdForTest !== null && !isNaN(thresholdForTest)) featureNameForDisplay = `T2 Größe ${conditionForTest} ${formatNumber(thresholdForTest, 1)}`; } else { featureValueForTest = t2Criteria[feature].value; if (featureValueForTest) featureNameForDisplay = `T2 ${feature}=${featureValueForTest}`; } }
            if ((feature !== 'size' && !featureValueForTest && t2Criteria[feature]?.active) || (feature === 'size' && (thresholdForTest === null || thresholdForTest === undefined || isNaN(thresholdForTest)) && t2Criteria[feature]?.active)) { results[feature] = { ...nullReturn, testName: `Merkmal ${feature} aktiv aber Wert/Schwelle nicht definiert`, featureName: featureNameForDisplay }; return; }
            else if(!t2Criteria[feature]?.active && feature !== 'size') { results[feature] = { ...nullReturn, testName: `Merkmal ${feature} nicht aktiv`, featureName: featureNameForDisplay }; return; }
            else if(feature === 'size' && !t2Criteria[feature]?.active) { results[feature] = { ...nullReturn, testName: `Merkmal ${feature} nicht aktiv`, featureName: featureNameForDisplay }; return; }
            ...
            data.forEach(p => { const validN = p?.[referenceKey] === '+' || p?.[referenceKey] === '-'; if (!validN || !Array.isArray(p.lymphknoten_t2)) return; const actualN = p[referenceKey] === '+'; let patientHasFeature = false;
                if (feature === 'size') { patientHasFeature = p.lymphknoten_t2.some(lk => { if (lk && typeof lk.groesse === 'number' && !isNaN(lk.groesse)) { switch(conditionForTest) { case '>=': return lk.groesse >= thresholdForTest; case '>': return lk.groesse > thresholdForTest; case '<=': return lk.groesse <= thresholdForTest; case '<': return lk.groesse < thresholdForTest; case '==': return lk.groesse === thresholdForTest; default: return false; } } return false; }); }
                else { patientHasFeature = p.lymphknoten_t2.some(lk => lk && lk[feature] === featureValueForTest); }
                if (patientHasFeature && actualN) a_val++; else if (patientHasFeature && !actualN) b_val++; else if (!patientHasFeature && actualN) c_val++; else if (!patientHasFeature && !actualN) d_val++;
            });
            const n_feature_total = a_val + b_val + c_val + d_val;
            if (n_feature_total > 0) { const fisherFeature = manualFisherExactTest(a_val, b_val, c_val, d_val); results[feature] = { matrix: { rp: a_val, fp: b_val, fn: c_val, rn: d_val }, testName: fisherFeature.method, pValue: fisherFeature.pValue, statistic: NaN, or: calculateORCI(a_val, b_val, c_val, d_val), rd: calculateRDCI(a_val, b_val, c_val, d_val), phi: { value: calculatePhi(a_val, b_val, c_val, d_val), ci: null, method: 'Phi Coefficient' }, featureName: featureNameForDisplay }; }
    ...
    function compareCohorts(data1, data2, t2Criteria, t2Logic) {
        ...
        if (t2Criteria && t2Logic && typeof t2CriteriaManager !== 'undefined' && typeof t2CriteriaManager.evaluateDataset === 'function') { evaluatedData1 = t2CriteriaManager.evaluateDataset(cloneDeep(data1), t2Criteria, t2Logic); evaluatedData2 = t2CriteriaManager.evaluateDataset(cloneDeep(data2), t2Criteria, t2Logic); t2Evaluated = true; }
    ...
    function calculateDescriptiveStats(data) {
        const n_total = data?.length ?? 0; const nullMetric = { median: NaN, min: NaN, max: NaN, mean: NaN, sd: NaN, n: 0 }; const nullReturn = { anzahlPatienten: 0, alter: nullMetric, geschlecht: null, therapie: null, nStatus: null, asStatus: null, t2Status: null, lkAnzahlen: null, alterData: [] };
        if (!Array.isArray(data) || n_total === 0) return nullReturn;
        const alterData = data.map(p => p?.alter).filter(a => a !== null && a !== undefined && !isNaN(a) && isFinite(a)).sort((a_val, b_val) => a_val - b_val);
        ...
        const getLKStats = (keyTotal, keyPlus = null, statusKeyForPlusFilter = null) => {
            const getCounts = (arr, prop) => arr.map(p => p?.[prop]).filter(c => c !== null && c !== undefined && !isNaN(c) && isFinite(c) && c >= 0);
            const totalCounts = getCounts(data, keyTotal).sort((a_val, b_val) => a_val - b_val); let plusCounts = [];
            if (keyPlus && statusKeyForPlusFilter) { plusCounts = getCounts(data.filter(p => p?.[statusKeyForPlusFilter] === '+'), keyPlus).sort((a_val, b_val) => a_val - b_val); }
            ...
            return { total: getStatsFromCounts(totalCounts), plus: (keyPlus && statusKeyForPlusFilter) ? getStatsFromCounts(plusCounts) : { ...nullMetric, n: 0 } };
        };
        ...
        return { anzahlPatienten: n_total, alter: alterStats, geschlecht: geschlecht, therapie: therapie, nStatus: nStatus, asStatus: asStatus, t2Status: t2Status, lkAnzahlen: lkAnzahlen, alterData: alterData };
    }
     ...
     function calculateStatsForExport(data, criteria, logic) {
         ...
         try {
            ...
            return {
                ...
                gueteT2: calculateDiagnosticPerformance(evaluatedData, 't2', 'n'),
                vergleichASvsT2: compareDiagnosticMethods(evaluatedData, 'as', 't2', 'n'),
                assoziation: calculateAssociations(evaluatedData, criteria),
    ...
    function calculateSingleMetricValue(data, predictionKey, referenceKey, metricName) {
        if (!Array.isArray(data) || data.length === 0 || !predictionKey || !referenceKey || !metricName) {
            return NaN;
        }
        const { rp, fp, fn, rn } = calculateConfusionMatrix(data, predictionKey, referenceKey);
        const total = rp + fp + fn + rn;
        ...
        switch (metricName) {
            ...
            case 'PPV': result = ppv; break;
            case 'NPV': result = npv; break;
            case 'Sensitivity': result = sens; break;
            case 'Specificity': result = spez; break;
            default: result = (isNaN(sens) || isNaN(spez)) ? NaN : (sens + spez) / 2.0; break;
        }
        return isNaN(result) ? -Infinity : result;
    }

    function calculateMetricForWorker(data, criteria, logic, metricName) {
        let evaluatedData = data;
        if (typeof t2CriteriaManager !== 'undefined' && typeof t2CriteriaManager.evaluateDataset === 'function') {
             evaluatedData = t2CriteriaManager.evaluateDataset(cloneDeep(data), criteria, logic);
        } else {
            const tempEvaluated = [];
            data.forEach(p => {
                const tempP = cloneDeep(p);
                const res = t2CriteriaManager.applyT2CriteriaToPatient(tempP, criteria, logic);
                tempP.t2 = res.t2Status;
                tempEvaluated.push(tempP);
            });
            evaluatedData = tempEvaluated;
        }
        return calculateSingleMetricValue(evaluatedData, 't2', 'n', metricName);
    }
    ...
    return Object.freeze({
        ...
        calculateSingleMetricValue,
        calculateMetricForWorker,
        manualMcNemarTest,
        manualDeLongTest,
