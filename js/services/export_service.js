const exportService = (() => {
    ...
    function generateStatistikCSVString(stats, kollektiv, criteria, logic) {
        ...
        try {
            ...
            csvData.push(['Geschlecht Männlich (n)', `${d.geschlecht?.m ?? 0}`]); csvData.push(['Geschlecht Männlich (%)', `${fp((d.geschlecht?.m ?? 0) / d.anzahlPatienten, 1)}`]);
            ...
            const addPerfRow = (metricKey, metricName, objAS, objT2) => {
                ...
                const [valAS, ciASL, ciASU] = fCIVal(objAS?.value, objAS?.ci, digits, isRate, true);
                const [valT2, ciT2L, ciT2U] = fCIVal(objT2?.value, objT2?.ci, digits, isRate, true);
                csvData.push([metricName, 'AS', valAS, ciASL, ciASU, fv(objAS?.se, 4, true), objAS?.method || na]);
                csvData.push([metricName, 'T2 (angew.)', valT2, ciT2L, ciT2U, fv(objT2?.se, 4, true), objT2?.method || na]);
            };
            const gAS = stats.gueteAS, gT2 = stats.gueteT2_angewandt;
            addPerfRow('sens', 'Sensitivität', gAS?.sens, gT2?.sens); addPerfRow('spez', 'Spezifität', gAS?.spez, gT2?.spez);
            addPerfRow('ppv', 'PPV', gAS?.ppv, gT2?.ppv); addPerfRow('npv', 'NPV', gAS?.npv, gT2?.npv);
            addPerfRow('acc', 'Accuracy', gAS?.acc, gT2?.acc); addPerfRow('balAcc', 'Balanced Accuracy', gAS?.balAcc, gT2?.balAcc);
            addPerfRow('f1', 'F1-Score', gAS?.f1, gT2?.f1); addPerfRow('auc', 'AUC', gAS?.auc, gT2?.auc);
            addPerfRow('lrPlus', 'LR+', gAS?.lrPlus, gT2?.lrPlus); addPerfRow('lrMinus', 'LR-', gAS?.lrMinus, gT2?.lrMinus);
            csvData.push([]);
            ...
            const [aucDiffVal, aucDiffL, aucDiffU] = fCIVal(v?.delong?.diffAUC, v?.delong, 3, false, true);
    ...
    function generateBruteForceTXTString(resultsData) {
        ...
        try {
            ...
            report += ` (Sens: ${formatPercent(bestResult.sens,1)}, Spez: ${formatPercent(bestResult.spez,1)}, PPV: ${formatPercent(bestResult.ppv,1)}, NPV: ${formatPercent(bestResult.npv,1)}, Acc: ${formatPercent(bestResult.acc,1)}, BalAcc: ${formatNumber(bestResult.balAcc,3,true)})\r\n\r\n`;
    ...
    async function exportChartsZip(scopeSelector, zipTypeKey, kollektiv, format) {
     ...
     async function exportCategoryZip(category, data, bfResults, kollektiv, criteria, logic, optionalLang, optionalBfMetric) {
         ...
         try {
                 ...
                 if (typeof publicationTextGenerator !== 'undefined' && statsDataForAllKollektive && state) {
                     ...
                     PUBLICATION_CONFIG.sections.forEach(mainSection => {
                         mainSection.subSections.forEach(subSection => {
                            ...
                            const typeKey = subSection.id.startsWith('methoden_') ? 'PUBLIKATION_METHODEN_MD' : (subSection.id.startsWith('ergebnisse_') ? 'PUBLIKATION_ERGEBNISSE_MD' : `PUBLIKATION_${mainSection.id.toUpperCase()}_MD`);
                            addFile(generateFilename(typeKey, kollektiv, 'md', {sectionName: subSection.id.replace(/^(methoden_|ergebnisse_|abstract_|einleitung_|diskussion_|referenzen_)/, '')}), mdContent);
                         });
                     });
                 }
             }
             ...
             if (['all-zip', 'konfiguration_json_export_unused_category'].includes(category)) { // konfiguration_json ist kein ZIP-Category, sondern ein Einzel-Export
                exportConfiguration(kollektiv, criteria, logic, currentKollektivBfResult); // wird direkt heruntergeladen
             }
     ...
     function exportPraesentationData(actionId, presentationData, kollektiv) {
            ...
            try {
                ...
                } else if (actionId === 'download-performance-as-vs-t2-csv') { if (!statsAS || !statsT2) { ui_helpers.showToast("Vergleichsdaten für CSV fehlen.", "warning"); return; } const headers = ['Metrik', 'AS (Wert)', 'AS (95% CI)', 'T2 (Wert)', 'T2 (95% CI)', 'CI Methode AS', 'CI Methode T2']; const fRow = (mKey, nm, isP = true, d = 1) => { const mAS = statsAS[mKey]; const mT2 = statsT2[mKey]; const dig = (mKey === 'auc' || mKey === 'f1' || mKey === 'lrPlus' || mKey === 'lrMinus') ? 3 : d; const ciAS = `(${formatNumber(mAS?.ci?.lower, dig, na, true)} - ${formatNumber(mAS?.ci?.upper, dig, na, true)})`; const ciT2 = `(${formatNumber(mT2?.ci?.lower, dig, na, true)} - ${formatNumber(mT2?.ci?.upper, dig, na, true)})`; const valAS = formatNumber(mAS?.value, dig, na, true); const valT2 = formatNumber(mT2?.value, dig, na, true); return [nm, valAS, ciAS, valT2, ciT2, mAS?.method || na, mT2?.method || na]; }; const rows = [ fRow('sens', 'Sensitivität'), fRow('spez', 'Spezifität'), fRow('ppv', 'PPV'), fRow('npv', 'NPV'), fRow('acc', 'Accuracy'), fRow('balAcc', 'Balanced Accuracy'), fRow('f1', 'F1-Score', false, 3), fRow('auc', 'AUC', false, 3), fRow('lrPlus', 'LR+', false, 3), fRow('lrMinus', 'LR-', false, 3) ]; content = Papa.unparse([headers, ...rows], { delimiter: APP_CONFIG.EXPORT_SETTINGS.CSV_DELIMITER || ";" }); filenameKey = 'PRAES_AS_VS_T2_PERF_CSV'; extension = 'csv'; mimeType = 'text/csv;charset=utf-8;';
