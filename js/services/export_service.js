const exportService = (() => {
    ...
    async function convertSvgToPngBlob(svgElement, targetWidth = 800) {
        return new Promise((resolve, reject) => {
            ...
            try {
                ...
                let sourceWidth = parseFloat(svgClone.getAttribute('width')) || svgElement.width?.baseVal?.value || viewBox?.width || targetWidth;
                let sourceHeight = parseFloat(svgClone.getAttribute('height')) || svgElement.height?.baseVal?.value || viewBox?.height || (targetWidth * 0.75);
         ...
         return new Promise((resolve, reject) => {
             if (!svgElement || typeof svgElement.cloneNode !== 'function') return reject(new Error("Ungültiges SVG Element für SVG Export."));
              try {
                 ...
                 svgClone.setAttribute('xmlns', 'http://www.w3.org/2000/svg');
                 svgClone.setAttribute('version', '1.1');
                 svgClone.style.backgroundColor = APP_CONFIG.CHART_SETTINGS.PLOT_BACKGROUND_COLOR || '#ffffff'; // Ensure background for viewers that don't assume white
                 ...
                 const svgXml = new XMLSerializer().serializeToString(svgClone);
                 const blob = new Blob([`<?xml version="1.0" standalone="no"?>\r\n`, svgXml], { type: 'image/svg+xml;charset=utf-8' });
                 resolve(blob);
              } catch(error) {
    ...
    function generateStatistikCSVString(stats, kollektiv, criteria, logic) {
        ...
        const csvData = []; const na = 'N/A'; const fv = (v, d=1, useStd = false) => formatNumber(v, d, na, useStd); const fp = (v, d=1) => formatPercent(v, d, na);
        const fCI_val = (o, d=1, isP = true) => !o || typeof o !== 'object' || o.value === null || isNaN(o.value) ? na : (isP ? formatPercent(o.value, d, na, true) : formatNumber(o.value, d, na, true));
        const fCI_low = (o, d=1, isP = true) => !o || typeof o !== 'object' || o.ci?.lower === null || isNaN(o.ci?.lower) ? na : (isP ? formatPercent(o.ci.lower, d, na, true) : formatNumber(o.ci.lower, d, na, true));
        const fCI_upp = (o, d=1, isP = true) => !o || typeof o !== 'object' || o.ci?.upper === null || isNaN(o.ci?.upper) ? na : (isP ? formatPercent(o.ci.upper, d, na, true) : formatNumber(o.ci.upper, d, na, true));
        const fPVal = (p) => (p !== null && !isNaN(p)) ? (p < 0.0001 ? '<0.0001' : fv(p, 4, true)) : na;

        try {
            csvData.push(['Parameter', 'Wert']); csvData.push(['Kollektiv', getKollektivDisplayName(kollektiv)]); csvData.push(['Angewandte T2 Logik', logic]); csvData.push(['Angewandte T2 Kriterien', formatCriteriaFunc(criteria, logic, false)]); csvData.push(['Anzahl Patienten', stats.deskriptiv.anzahlPatienten]); csvData.push([]);
            ...
            csvData.push(['Therapie direkt OP (n)', `${d.therapie?.['direkt OP'] ?? 0}`]); csvData.push(['Therapie direkt OP (%)', `${fp(d.anzahlPatienten > 0 ? (d.therapie?.['direkt OP'] ?? 0) / d.anzahlPatienten : NaN, 1)}`]); csvData.push(['Therapie nRCT (n)', `${d.therapie?.nRCT ?? 0}`]); csvData.push(['Therapie nRCT (%)', `${fp(d.anzahlPatienten > 0 ? (d.therapie?.nRCT ?? 0) / d.anzahlPatienten : NaN, 1)}`]);
            csvData.push(['N Status (+) (n)', `${d.nStatus?.plus ?? 0}`]); csvData.push(['N Status (+) (%)', `${fp(d.anzahlPatienten > 0 ? (d.nStatus?.plus ?? 0) / d.anzahlPatienten : NaN, 1)}`]);
            csvData.push(['AS Status (+) (n)', `${d.asStatus?.plus ?? 0}`]); csvData.push(['AS Status (+) (%)', `${fp(d.anzahlPatienten > 0 ? (d.asStatus?.plus ?? 0) / d.anzahlPatienten : NaN, 1)}`]);
            csvData.push(['T2 Status (angew.) (+) (n)', `${d.t2Status?.plus ?? 0}`]); csvData.push(['T2 Status (angew.) (+) (%)', `${fp(d.anzahlPatienten > 0 ? (d.t2Status?.plus ?? 0) / d.anzahlPatienten : NaN, 1)}`]);
            const fLKRow = (lk) => [fv(lk?.median, 1, true), fv(lk?.mean, 1, true), fv(lk?.sd, 1, true), fv(lk?.min, 0, true), fv(lk?.max, 0, true)];
            csvData.push(['LK N gesamt', ...fLKRow(d.lkAnzahlen?.n?.total)]); csvData.push(['LK N+ (nur N+ Pat.)', ...fLKRow(d.lkAnzahlen?.n?.plus)]);
            csvData.push(['LK AS gesamt', ...fLKRow(d.lkAnzahlen?.as?.total)]); csvData.push(['LK AS+ (nur AS+ Pat.)', ...fLKRow(d.lkAnzahlen?.as?.plus)]);
            csvData.push(['LK T2 (angew.) gesamt', ...fLKRow(d.lkAnzahlen?.t2?.total)]); csvData.push(['LK T2+ (angew., nur T2+ Pat.)', ...fLKRow(d.lkAnzahlen?.t2?.plus)]); csvData.push([]);
            csvData.push(['Metrik (Diagnostik)', 'Methode', 'Wert', '95% KI Lower', '95% KI Upper', 'SE (Bootstrap)', 'CI Methode']);
            const addPerfRow = (metricKey, metricName, objAS, objT2_angew) => {
                const isRate = !['auc', 'f1', 'lr_plus', 'lr_minus'].includes(metricKey);
                const digits = (metricKey === 'auc' || metricKey === 'f1' || metricKey === 'lr_minus') ? 3 : (metricKey === 'lr_plus' ? 2 : 1);
                csvData.push([metricName, 'Avocado Sign', fCI_val(objAS, digits, isRate), fCI_low(objAS, digits, isRate), fCI_upp(objAS, digits, isRate), fv(objAS?.se, 4, true), objAS?.method || na]);
                csvData.push([metricName, 'T2 (angewandt)', fCI_val(objT2_angew, digits, isRate), fCI_low(objT2_angew, digits, isRate), fCI_upp(objT2_angew, digits, isRate), fv(objT2_angew?.se, 4, true), objT2_angew?.method || na]);
            };
            const gAS = stats.gueteAS, gT2_angewandt = stats.gueteT2_angewandt;
            ['sens', 'spez', 'ppv', 'npv', 'lr_plus', 'lr_minus', 'acc', 'balAcc', 'f1', 'auc'].forEach(key => {
                addPerfRow(key, TOOLTIP_CONTENT.statMetrics[key]?.name || key.toUpperCase(), gAS?.[key], gT2_angewandt?.[key]);
            });
            csvData.push([]);
            csvData.push(['Vergleichstest (AS vs. T2 angewandt)', 'Test Statistik', 'p-Wert', 'Methode']); const v = stats.vergleichASvsT2_angewandt; csvData.push(['Accuracy (McNemar)', fv(v?.mcnemar?.statistic, 3, true) + (v?.mcnemar?.df ? ` (df=${v.mcnemar.df})` : ''), fPVal(v?.mcnemar?.pValue), v?.mcnemar?.method || na]); csvData.push(['AUC (DeLong)', `Z=${fv(v?.delong?.Z, 3, true)} (Diff: ${fv(v?.delong?.diffAUC,3,true)})`, fPVal(v?.delong?.pValue), v?.delong?.method || na]); csvData.push([]);
            csvData.push(['Assoziation mit N-Status (T2-Merkmale basierend auf angewandten Kriterien)', 'Merkmal', 'OR', 'OR CI Lower', 'OR CI Upper', 'RD (%)', 'RD CI Lower (%)', 'RD CI Upper (%)', 'Phi', 'Test Statistik', 'p-Wert', 'Test Methode']); const a = stats.assoziation_angewandt;
            const addAssocRow = (obj) => { if (!obj) return; const orCI = obj.or?.ci; const rdCI = obj.rd?.ci; csvData.push([ obj.featureName || 'N/A', fv(obj.or?.value, 2, true), fv(orCI?.lower, 2, true), fv(orCI?.upper, 2, true), fv((obj.rd?.value ?? NaN)*100, 1, true), fv((rdCI?.lower ?? NaN)*100, 1, true), fv((rdCI?.upper ?? NaN)*100, 1, true), fv(obj.phi?.value, 2, true), fv(obj.statistic ?? (obj.Z ?? NaN), 2, true), fPVal(obj.pValue), obj.testName || na ]); };
            if(a?.as) addAssocRow(a.as); if(a?.size_mwu) addAssocRow(a.size_mwu);
            ['size', 'form', 'kontur', 'homogenitaet', 'signal'].forEach(fKey => { if(a && a[fKey]) { addAssocRow(a[fKey]); } });
    ...
    function generateBruteForceTXTString(resultsData) {
        ...
        try {
            ...
            report += `Kriterien: ${formatCriteriaFunc(bestResult.criteria, bestResult.logic, false)}\r\n`;
            ...
            for (let i = 0; i < results.length; i++) {
                ...
                if (rank > 10 && isNewRank && displayedCount >=10 ) break; // Ensure at least 10 distinct ranks or more items if scores are tied

                // Re-calculate performance metrics for this specific combination for detailed output
                // This assumes 'currentData' is available and represents the data fed to the worker for this kollektiv.
                // For a pure export service, this data would ideally be passed in or the worker would return these metrics.
                // Fallback to N/A if full recalculation isn't feasible here.
                let sens = 'N/A', spez = 'N/A', ppv = 'N/A', npv = 'N/A';
                if (typeof currentData !== 'undefined' && currentData && currentData.length > 0 && typeof t2CriteriaManager !== 'undefined' && typeof statisticsService !== 'undefined') {
                     const tempEvalData = t2CriteriaManager.evaluateDataset(cloneDeep(currentData.filter(p => p.kollektiv === kollektiv || kollektiv === "Gesamt")), result.criteria, result.logic);
                     const perf = statisticsService.calculateDiagnosticPerformance(tempEvalData, 't2', 'n');
                     if (perf) {
                         sens = formatPercent(perf.sens?.value, 1, 'N/A');
                         spez = formatPercent(perf.spez?.value, 1, 'N/A');
                         ppv = formatPercent(perf.ppv?.value, 1, 'N/A');
                         npv = formatPercent(perf.npv?.value, 1, 'N/A');
                     }
                }

                report += `${String(currentRank).padEnd(4)} | ${formatNumber(result.metricValue, 4, true).padEnd(12)} | ${sens.padEnd(6)} | ${spez.padEnd(6)} | ${ppv.padEnd(6)} | ${npv.padEnd(6)} | ${result.logic.toUpperCase().padEnd(5)} | ${formatCriteriaFunc(result.criteria, result.logic, false)}\r\n`;
    ...
    function _generatePublicationDraftText(type, data, bfResults, kollektiv, criteria, logic, lang) {
        let fullText = "";
        const sectionsToInclude = PUBLICATION_CONFIG.sections.filter(s => s.id !== 'referenzen'); // Referenzen am Ende
        
        const allStats = statisticsService.calculateAllStatsForPublication(data, criteria, logic, bfResults, state.getCurrentPublikationBruteForceMetric());
         const commonData = {
            ...
            ETHICS_VOTE_SAXONY: { shortCitation: "Ethikvotum Nr. 2023-101 (Sächsische Landesärztekammer)"},
            LURZ_SCHAEFER_2025_ADDITIONAL: { studyPeriod: "Januar 2020 und November 2023", mriSystem: "einem 3.0-T System (MAGNETOM Prisma Fit; Siemens Healthineers, Erlangen, Deutschland)", contrastAgent: "Gadoteridol (ProHance; Bracco Imaging, Konstanz, Deutschland)", t2SliceThickness: "2–3 mm", radiologistExperience: ["29", "7", "19"],}
        };
        const options = { currentKollektiv: kollektiv, publicationBfMetric: state.getCurrentPublikationBruteForceMetric() };

        referenceManager.resetUsedReferences();

        sectionsToInclude.forEach(mainSection => {
            if (type === 'md') {
                fullText += `## ${UI_TEXTS.publikationTab.sectionLabels[mainSection.labelKey] || mainSection.labelKey}\n\n`;
            } else { // html
                fullText += `<section><h2>${UI_TEXTS.publikationTab.sectionLabels[mainSection.labelKey] || mainSection.labelKey}</h2>`;
            }
            mainSection.subSections.forEach(subSection => {
                if (type === 'md') {
                    fullText += `### ${subSection.label}\n`;
                    fullText += publicationTextGenerator.getSectionTextAsMarkdown(subSection.id, lang, allStats, commonData, options) + "\n\n";
                } else {
                    fullText += `<h3>${subSection.label}</h3>`;
                    fullText += publicationTextGenerator.getSectionText(subSection.id, lang, allStats, commonData, options);
                }
            });
            if (type === 'html') fullText += `</section>`;
        });

        // Add References
        if (type === 'md') {
            fullText += `## ${UI_TEXTS.publikationTab.sectionLabels.referenzen || 'Referenzen'}\n\n`;
            fullText += publicationTextGenerator.getSectionTextAsMarkdown('referenzen_list', lang, allStats, commonData, options);
        } else {
            fullText += `<section><h2>${UI_TEXTS.publikationTab.sectionLabels.referenzen || 'References'}</h2>`;
            fullText += publicationTextGenerator.getSectionText('referenzen_list', lang, allStats, commonData, options);
            fullText += `</section>`;
        }
        return fullText;
    }
    ...
    function generateComprehensiveReportHTML(data, bfResultsForCurrentKollektiv, kollektiv, criteria, logic) {
        try {
            ...
            const statsDataForAllKollektive = statisticsService.calculateAllStatsForPublication(data, criteria, logic, allBruteForceResults, state.getCurrentPublikationBruteForceMetric());
            ...
            if (config.INCLUDE_DESCRIPTIVES_CHARTS) { ['chart-stat-age-0', 'chart-stat-gender-0'].forEach(id => { if (document.getElementById(id)) chartIdsToCapture.push(id); }); }
            ...
            if (config.INCLUDE_DESCRIPTIVES_TABLE && statsDataForCurrentKollektiv?.deskriptiv) { html += `<h2>Deskriptive Statistik</h2>`; html += `<table><thead><tr><th>Metrik</th><th>Wert (Median)</th><th>Mean</th><th>SD</th><th>Min</th><th>Max</th></tr></thead><tbody>`; const d = statsDataForCurrentKollektiv.deskriptiv; const na = '--'; const fv = (v, dig = 1, useStd = false) => formatNumber(v, dig, na, useStd); const fP = (v, dig = 1) => formatPercent(v, dig, na); const addRowHTML = (l, vl=na, m=na, s=na, mn=na, mx=na) => `<tr><td>${l}</td><td>${vl}</td><td>${m}</td><td>${s}</td><td>${mn}</td><td>${mx}</td></tr>`; html += addRowHTML('Alter (Jahre)', fv(d.alter?.median, 1), fv(d.alter?.mean, 1), fv(d.alter?.sd, 1), fv(d.alter?.min, 0), fv(d.alter?.max, 0)); html += addRowHTML('Geschlecht Männlich (n / %)', `${d.geschlecht?.m ?? 0} / ${fP(d.anzahlPatienten > 0 ? (d.geschlecht?.m ?? 0) / d.anzahlPatienten : NaN, 1)}`); html += addRowHTML('Therapie direkt OP (n / %)', `${d.therapie?.['direkt OP'] ?? 0} / ${fP(d.anzahlPatienten > 0 ? (d.therapie?.['direkt OP'] ?? 0) / d.anzahlPatienten : NaN, 1)}`); html += addRowHTML('Therapie nRCT (n / %)', `${d.therapie?.nRCT ?? 0} / ${fP(d.anzahlPatienten > 0 ? (d.therapie?.nRCT ?? 0) / d.anzahlPatienten : NaN, 1)}`); html += addRowHTML('N Status (+ / %)', `${d.nStatus?.plus ?? 0} / ${fP(d.anzahlPatienten > 0 ? (d.nStatus?.plus ?? 0) / d.anzahlPatienten : NaN, 1)}`); html += addRowHTML('AS Status (+ / %)', `${d.asStatus?.plus ?? 0} / ${fP(d.anzahlPatienten > 0 ? (d.asStatus?.plus ?? 0) / d.anzahlPatienten : NaN, 1)}`); html += addRowHTML('T2 Status (angew.) (+ / %)', `${d.t2Status?.plus ?? 0} / ${fP(d.anzahlPatienten > 0 ? (d.t2Status?.plus ?? 0) / d.anzahlPatienten : NaN, 1)}`); const fLK = (lk) => `${fv(lk?.median,1)} (${fv(lk?.min,0)}-${fv(lk?.max,0)})`; html += addRowHTML('LK N gesamt (Median (Min-Max))', fLK(d.lkAnzahlen?.n?.total), fv(d.lkAnzahlen?.n?.total?.mean,1), fv(d.lkAnzahlen?.n?.total?.sd,1),fv(d.lkAnzahlen?.n?.total?.min,0), fv(d.lkAnzahlen?.n?.total?.max,0)); html += addRowHTML('LK AS gesamt (Median (Min-Max))', fLK(d.lkAnzahlen?.as?.total), fv(d.lkAnzahlen?.as?.total?.mean,1), fv(d.lkAnzahlen?.as?.total?.sd,1),fv(d.lkAnzahlen?.as?.total?.min,0), fv(d.lkAnzahlen?.as?.total?.max,0)); html += addRowHTML('LK T2 (angew.) gesamt (Median (Min-Max))', fLK(d.lkAnzahlen?.t2?.total), fv(d.lkAnzahlen?.t2?.total?.mean,1), fv(d.lkAnzahlen?.t2?.total?.sd,1),fv(d.lkAnzahlen?.t2?.total?.min,0), fv(d.lkAnzahlen?.t2?.total?.max,0)); html += `</tbody></table>`; }
            if (config.INCLUDE_DESCRIPTIVES_CHARTS) { html += `<div style="display: flex; flex-wrap: wrap; gap: 10px; margin-top: 1em; justify-content: space-around;">`; Object.keys(chartSVGs).forEach(key => { if(key.includes('stat-age') || key.includes('stat-gender')) html += `<div class="chart-container" style="flex: 1 1 45%; min-width: 300px;"><h3>${key.includes('age')?'Altersverteilung':'Geschlechterverteilung'}</h3>${chartSVGs[key]}</div>`; }); html += `</div>`; }
            const addPerfSectionHTML = (title, statsObj) => { if (!statsObj) return ''; let sHtml = `<h2>${title}</h2><table><thead><tr><th>Metrik</th><th>Wert (95% CI)</th><th>CI Methode</th></tr></thead><tbody>`; const fCI_local = (m, d=1, p=true) => formatCI(m?.value, m?.ci?.lower, m?.ci?.upper, d, p, '--'); const na = '--'; ['sens', 'spez', 'ppv', 'npv', 'lr_plus', 'lr_minus', 'acc', 'balAcc', 'f1', 'auc'].forEach(k => { const d = (k==='f1'||k==='auc'||k==='lr_minus')?3:((k==='lr_plus')?2:1); const p = !['f1','auc','lr_plus','lr_minus'].includes(k); sHtml += `<tr><td>${TOOLTIP_CONTENT.statMetrics[k]?.name || k.toUpperCase()}</td><td>${fCI_local(statsObj[k], d, p)}</td><td>${statsObj[k]?.method || na}</td></tr>`; }); sHtml += `</tbody></table>`; return sHtml; };
            ...
            if (config.INCLUDE_AS_VS_T2_COMPARISON_TABLE && statsDataForCurrentKollektiv?.vergleichASvsT2_angewandt) { html += `<h2>Statistischer Vergleich: AS vs. T2 ('${appliedCriteriaDisplayName}')</h2><table><thead><tr><th>Test</th><th>Statistik</th><th>p-Wert</th><th>Methode</th></tr></thead><tbody>`; const v = statsDataForCurrentKollektiv.vergleichASvsT2_angewandt; const fP = (p) => (p !== null && !isNaN(p)) ? (p < 0.001 ? '&lt;0.001' : formatNumber(p, 3, '--', true)) : '--'; const na = '--'; html += `<tr><td>Accuracy (McNemar)</td><td>${formatNumber(v?.mcnemar?.statistic, 3, na, true)} (df=${v?.mcnemar?.df || na})</td><td>${fP(v?.mcnemar?.pValue)} ${getStatisticalSignificanceSymbol(v?.mcnemar?.pValue)}</td><td>${v?.mcnemar?.method || na}</td></tr>`; html += `<tr><td>AUC (DeLong)</td><td>Z=${formatNumber(v?.delong?.Z, 3, na, true)} (Diff: ${formatNumber(v?.delong?.diffAUC,3,na,true)})</td><td>${fP(v?.delong?.pValue)} ${getStatisticalSignificanceSymbol(v?.delong?.pValue)}</td><td>${v?.delong?.method || na}</td></tr>`; html += `</tbody></table>`; }
            ...
            if (config.INCLUDE_ASSOCIATIONS_TABLE && statsDataForCurrentKollektiv?.assoziation_angewandt && Object.keys(statsDataForCurrentKollektiv.assoziation_angewandt).length > 0) { html += `<h2>Assoziation mit N-Status</h2><table><thead><tr><th>Merkmal</th><th>OR (95% CI)</th><th>RD (%) (95% CI)</th><th>Phi</th><th>p-Wert</th><th>Test</th></tr></thead><tbody>`; const a = statsDataForCurrentKollektiv.assoziation_angewandt; const na = '--'; const fP = (p) => (p !== null && !isNaN(p)) ? (p < 0.001 ? '&lt;0.001' : formatNumber(p, 3, na, true)) : na; const fRowAssoc = (obj) => { if (!obj) return ''; const orS = formatCI(obj.or?.value, obj.or?.ci?.lower, obj.or?.ci?.upper, 2, false, na); const rdV = formatNumber(obj.rd?.value !== null && !isNaN(obj.rd?.value) ? obj.rd.value * 100 : NaN, 1, na, true); const rdL = formatNumber(obj.rd?.ci?.lower !== null && !isNaN(obj.rd?.ci?.lower) ? obj.rd.ci.lower * 100 : NaN, 1, na, true); const rdU = formatNumber(obj.rd?.ci?.upper !== null && !isNaN(obj.rd?.ci?.upper) ? obj.rd.ci.upper * 100 : NaN, 1, na, true); const rdS = rdV !== na ? `${rdV}% (${rdL}% - ${rdU}%)` : na; const phiS = formatNumber(obj.phi?.value, 2, na, true); const pS = fP(obj.pValue) + ' ' + getStatisticalSignificanceSymbol(obj.pValue); const tN = obj.testName || na; return `<tr><td>${obj.featureName}</td><td>${orS}</td><td>${rdS}</td><td>${phiS}</td><td>${pS}</td><td>${tN}</td></tr>`; }; if (a?.as) html += fRowAssoc(a.as); if (a?.size_mwu) html += `<tr><td>${a.size_mwu.featureName || 'LK Größe (Median Vgl.)'}</td><td>${na}</td><td>${na}</td><td>${na}</td><td>${fP(a.size_mwu.pValue)} ${getStatisticalSignificanceSymbol(a.size_mwu.pValue)}</td><td>${a.size_mwu.testName || na}</td></tr>`; ['size', 'form', 'kontur', 'homogenitaet', 'signal'].forEach(k => { if (a && a[k]) { html += fRowAssoc(a[k]); } }); html += `</tbody></table>`; }
            const currentKollektivBfResult = allBruteForceResults ? allBruteForceResults[kollektiv] : null;
            if (config.INCLUDE_BRUTEFORCE_BEST_RESULT && currentKollektivBfResult?.results && currentKollektivBfResult.results.length > 0 && currentKollektivBfResult.bestResult) { html += `<h2>Bestes Brute-Force Ergebnis (für Kollektiv: ${kollektivName})</h2><div class="meta-info"><ul>`; const best = currentKollektivBfResult.bestResult; html += `<li><strong>Optimierte Metrik:</strong> ${currentKollektivBfResult.metric}</li><li><strong>Bester Wert:</strong> ${formatNumber(best.metricValue, 4, '--', true)}</li><li><strong>Logik:</strong> ${best.logic?.toUpperCase()}</li><li><strong>Kriterien:</strong> ${formatCriteriaFunc(best.criteria, best.logic, false)}</li></ul><p class="small text-muted">Kollektiv N=${formatNumber(currentKollektivBfResult.nGesamt, 0, 'N/A')} (N+: ${formatNumber(currentKollektivBfResult.nPlus, 0, 'N/A')}, N-: ${formatNumber(currentKollektivBfResult.nMinus, 0, 'N/A')})</p></div>`; }
            html += `<div class="report-footer">${config.REPORT_AUTHOR} - ${timestamp}</div></body></html>`; return html;
             ...
             return '<html><head><title>Fehler</title></head><body>Fehler bei der Reporterstellung. Details siehe Konsole.</body></html>';
        }
    }
        ...
        return new Promise((resolve, reject) => {
            ...
            try {
                const options = {
                    ...
                    useCORS: true, // May be needed if table contains external images/fonts, though unlikely here
                    width: table.offsetWidth,
                    height: table.offsetHeight,
                    windowWidth: table.scrollWidth, // Use scrollWidth/Height to capture full table
                    windowHeight: table.scrollHeight
                };
                 if (typeof html2canvas === 'undefined') {
                     return reject(new Error("html2canvas Bibliothek nicht geladen. Tabellen-PNG-Export nicht möglich."));
                 }
                ...
                }).catch(err => reject(new Error(`html2canvas Fehler für Tabelle ${tableElementId}: ${err.message}`)));
    ...
    function exportStatistikCSV(data, kollektiv, criteria, logic) {
        ...
        try { stats = statisticsService.calculateAllStatsForPublication(data, criteria, logic, allBruteForceResults, state.getCurrentPublikationBruteForceMetric())[kollektiv]; } catch(e) { console.error("Fehler bei Statistikberechnung für CSV.", e); ui_helpers.showToast("Fehler bei Statistikberechnung für CSV.", "danger"); return; }
        if (!stats) { ui_helpers.showToast("Keine Statistikdaten zum Exportieren für dieses Kollektiv.", "warning"); return; }
        try { csvString = generateStatistikCSVString(stats, kollektiv, criteria, logic); } catch(e) { console.error("Fehler bei CSV-Erstellung.", e); ui_helpers.showToast("Fehler bei CSV-Erstellung.", "danger"); return; }
    ...
    function exportBruteForceReport(resultsData) {
        ...
        try {
            ...
            console.error("Fehler bei TXT-Erstellung (BruteForce).", e);
    ...
    function exportTableMarkdown(dataOrStats, tableType, kollektiv, criteria = null, logic = null, options = {}) {
        ...
        try { mdString = generateMarkdownTableString(dataOrStats, tableType, kollektiv, criteria, logic, options); } catch(e) { console.error(`Fehler bei MD-Erstellung für Typ ${tableType}.`, e); ui_helpers.showToast(`Fehler bei MD-Erstellung (${tableType}).`, "danger"); return; }
        ...
        else if(tableType === 'praes_as_vs_t2_perf' || tableType === 'praes_as_vs_t2_comp') { typeKey = 'PRAES_AS_VS_T2_PERF_MD'; title = 'AS vs T2 Performance'; }
        ...
        else if(tableType.startsWith('publication_')) { typeKey = tableType.toUpperCase() + '_MD'; title = `Publikation ${options.sectionName || tableType.replace('publication_','')}`; }
    ...
    function exportFilteredDataCSV(data, kollektiv) {
       let csvString = null;
       try { csvString = generateFilteredDataCSVString(data); } catch(e) { console.error("Fehler bei Rohdaten-CSV-Erstellung.", e); ui_helpers.showToast("Fehler bei Rohdaten-CSV-Erstellung.", "danger"); return; }
    ...
    function exportComprehensiveReportHTML(data, bfResultsForCurrentKollektiv, kollektiv, criteria, logic) {
        let htmlString = null;
        try { htmlString = generateComprehensiveReportHTML(data, bfResultsForCurrentKollektiv, kollektiv, criteria, logic); } catch(e) { console.error("Fehler bei HTML-Report-Erstellung.", e); ui_helpers.showToast("Fehler bei HTML-Report-Erstellung.", "danger"); return; }
    ...
    async function exportSingleChart(chartElementId, format, kollektiv, options = {}) {
         ...
         try {
             ...
             if (format === 'png') { ui_helpers.showToast(`Generiere PNG für Chart ${chartName}...`, 'info', 1500); blob = await convertSvgToPngBlob(svgElement, APP_CONFIG.CHART_SETTINGS.DEFAULT_WIDTH * 1.5); filenameKey = 'CHART_SINGLE_PNG'; mimeType = 'image/png'; ext = 'png'; }
             ...
             else if (format === 'tiff') { ui_helpers.showToast(`Generiere TIFF (via PNG) für Chart ${chartName}...`, 'info', 2000); blob = await convertSvgToPngBlob(svgElement, APP_CONFIG.EXPORT_SETTINGS.TIFF_EXPORT_DPI ? (APP_CONFIG.CHART_SETTINGS.DEFAULT_WIDTH * (APP_CONFIG.EXPORT_SETTINGS.TIFF_EXPORT_DPI / 96)) : APP_CONFIG.CHART_SETTINGS.DEFAULT_WIDTH * 3); filenameKey = 'CHART_SINGLE_TIFF'; mimeType = 'image/tiff'; ext = 'tiff'; ui_helpers.showToast('TIFF-Export ist experimentell und erzeugt eine PNG-Datei mit .tiff-Endung. Für echte TIFF-Konvertierung bitte externe Tools verwenden.', 'warning', 5000); }
    ...
    async function exportChartsZip(scopeSelector, zipTypeKey, kollektiv, format) {
         ...
         const tableSelectors = [
            scopeSelector + ' table[id^="table-"]', scopeSelector + ' table.publication-table[id]',
            scopeSelector + ' table#auswertung-table', scopeSelector + ' table#daten-table',
            scopeSelector + ' table#bruteforce-results-table', scopeSelector + ' table#praes-as-vs-t2-comp-table'
         ];
         const tableContainerElements = (format === 'png' && APP_CONFIG.EXPORT_SETTINGS.ENABLE_TABLE_PNG_EXPORT) ? document.querySelectorAll(tableSelectors.join(', ')) : [];

         if (chartContainers.length === 0 && tableContainerElements.length === 0) { ui_helpers.showToast('Keine Diagramme oder Tabellen im aktuellen Sichtbereich gefunden.', 'warning'); return; }
         ...
         chartContainers.forEach((svgElement, index) => {
             ...
             if (format === 'png') { filenameKey = 'CHART_SINGLE_PNG'; ext = 'png'; conversionPromise = convertSvgToPngBlob(svgElement, APP_CONFIG.CHART_SETTINGS.DEFAULT_WIDTH * 1.5).catch(e => { console.error(`PNG Konvertierung für ${chartName} fehlgeschlagen:`, e); return null; }); }
             ...
             else if (format === 'tiff') { filenameKey = 'CHART_SINGLE_TIFF'; ext = 'tiff'; conversionPromise = convertSvgToPngBlob(svgElement, APP_CONFIG.EXPORT_SETTINGS.TIFF_EXPORT_DPI ? (APP_CONFIG.CHART_SETTINGS.DEFAULT_WIDTH * (APP_CONFIG.EXPORT_SETTINGS.TIFF_EXPORT_DPI / 96)) : APP_CONFIG.CHART_SETTINGS.DEFAULT_WIDTH * 3).catch(e => { console.error(`TIFF (PNG) Konvertierung für ${chartName} fehlgeschlagen:`, e); return null;}); }
             else { return; }
             const filename = generateFilename(filenameKey, kollektiv, ext, { chartName });
             promises.push(conversionPromise.then(blob => (blob ? { blob, filename } : { error: new Error(`Blob is null for chart ${filename}`), filename })));
         });

         tableContainerElements.forEach((table, index) => {
              if (format !== 'png') return; // Only PNG for tables in ZIP for now
              const tableId = table.id || `exportable-table-${generateUUID()}`; table.id = tableId; // Ensure ID exists
              ...
              promises.push(convertTableToPngBlob(tableId, baseWidth).catch(e => { console.error(`Tabellen-PNG Konvertierung für ${tableName} fehlgeschlagen:`, e); return null; }).then(blob => (blob ? { blob, filename } : { error: new Error(`Table Blob is null for ${filename}`), filename })));
         });
     ...
     async function exportCategoryZip(category, data, bfResults, kollektiv, criteria, logic) {
         ...
         const publicationBfMetric = state.getCurrentPublikationBruteForceMetric();

         const needsStats = ['all-zip', 'csv-zip', 'md-zip', 'html-report', 'publication-draft-md', 'publication-draft-html'].includes(category);
         if(needsStats && data && data.length > 0 && criteria && logic) {
             try {
                statsDataForAllKollektive = statisticsService.calculateAllStatsForPublication(data, criteria, logic, bfResults, publicationBfMetric);
             ...
             if (!statsDataForAllKollektive || !statsDataForAllKollektive[kollektiv]) {
                 ...
                 if (['csv-zip', 'html-report', 'publication-draft-md', 'publication-draft-html'].includes(category)) return;
             }
         }
         ...
         try {
             ...
             if (['all-zip', 'md-zip'].includes(category)) {
                const commonDataForPub = { appName: APP_CONFIG.APP_NAME, appVersion: APP_CONFIG.APP_VERSION, nGesamt: statsDataForAllKollektive?.Gesamt?.deskriptiv?.anzahlPatienten || 0, nDirektOP: statsDataForAllKollektive?.['direkt OP']?.deskriptiv?.anzahlPatienten || 0, nNRCT: statsDataForAllKollektive?.nRCT?.deskriptiv?.anzahlPatienten || 0, significanceLevel: APP_CONFIG.STATISTICAL_CONSTANTS.SIGNIFICANCE_LEVEL, references: APP_CONFIG.REFERENCES_FOR_PUBLICATION, ETHICS_VOTE_SAXONY: { shortCitation: "Ethikvotum Nr. 2023-101 (Sächsische Landesärztekammer)"}, LURZ_SCHAEFER_2025_ADDITIONAL: { studyPeriod: "Januar 2020 und November 2023", mriSystem: "einem 3.0-T System (MAGNETOM Prisma Fit; Siemens Healthineers, Erlangen, Deutschland)", contrastAgent: "Gadoteridol (ProHance; Bracco Imaging, Konstanz, Deutschland)", t2SliceThickness: "2–3 mm", radiologistExperience: ["29", "7", "19"],}};
                const optionsForPub = { currentKollektiv: kollektiv, publicationBfMetric: publicationBfMetric };
                 ...
                 if (category === 'md-zip' || category === 'all-zip') { // Full publication draft only for these specific ZIPs
                    addFile(generateFilename('PUBLIKATION_DRAFT_MD', kollektiv, 'md'), _generatePublicationDraftText('md', data, bfResults, kollektiv, criteria, logic, lang));
                 }
             }
             if (['all-zip', 'publication-draft-html'].includes(category) && data && data.length > 0) {
                addFile(generateFilename('PUBLIKATION_DRAFT_HTML', kollektiv, 'html'), _generatePublicationDraftText('html', data, bfResults, kollektiv, criteria, logic, lang));
             }
             ...
             if (['all-zip', 'html-report'].includes(category) && data && data.length > 0 ) { addFile(generateFilename('COMPREHENSIVE_REPORT_HTML', kollektiv, 'html'), generateComprehensiveReportHTML(data, currentKollektivBfResult, kollektiv, criteria, logic)); }
             ...
             if (['tiff-zip'].includes(category)) { await exportChartsZip('#app-container', 'TIFF_ZIP', kollektiv, 'tiff'); return; }


            if (filesAdded > 0) {
                const zipFilename = generateFilename(`${category.replace('-zip','').toUpperCase()}_PAKET`, kollektiv, 'zip');
                const content = await zip.generateAsync({ type: "blob", compression: "DEFLATE", compressionOptions: { level: 6 } });
                if (downloadFile(content, zipFilename, "application/zip")) ui_helpers.showToast(`${filesAdded} Datei(en) erfolgreich im ${category.toUpperCase().replace('-ZIP','')} ZIP-Paket exportiert.`, 'success');
            } else { ui_helpers.showToast(`Keine Dateien für das ${category.toUpperCase().replace('-ZIP','')} ZIP-Paket gefunden oder generiert.`, 'warning'); }
         } catch (error) { console.error(`Fehler beim Erstellen des ${category.toUpperCase().replace('-ZIP','')} ZIP-Pakets:`, error); ui_helpers.showToast(`Fehler beim Erstellen des ${category.toUpperCase().replace('-ZIP','')} ZIP-Pakets.`, 'danger'); }
     }
     ...
     function exportPraesentationData(actionId, presentationData, kollektiv) {
            ...
            options.kollektivForComparison = presentationData.kollektivForComparison || kollektiv;


            try {
                if (isAsPurView && actionId === 'download-performance-as-pur-csv') {
                     const allStatsData = { Gesamt: statsGesamt, 'direkt OP': statsDirektOP, nRCT: statsNRCT }; const headers = ['Kollektiv', 'N', 'Sens', 'Sens CI Low', 'Sens CI High', 'Spez', 'Spez CI Low', 'Spez CI High', 'PPV', 'PPV CI Low', 'PPV CI High', 'NPV', 'NPV CI Low', 'NPV CI High', 'LR+', 'LR+ CI Low', 'LR+ CI High','LR-', 'LR- CI Low', 'LR- CI High','Acc', 'Acc CI Low', 'Acc CI High', 'BalAcc', 'BalAcc CI Low', 'BalAcc CI High', 'F1', 'F1 CI Low', 'F1 CI High', 'AUC', 'AUC CI Low', 'AUC CI High', 'CI Method (Prop.)', 'CI Method (Effect)']; const fVal = (v, d=1, useStd = false) => formatNumber(v, d, na, useStd);
                     const rows = Object.entries(allStatsData).map(([key, stats]) => { let dN = key; if (!stats || typeof stats.matrix !== 'object') return [getKollektivDisplayName(dN), 0, ...Array(31).fill(na)]; const n = stats.matrix ? (stats.matrix.rp + stats.matrix.fp + stats.matrix.fn + stats.matrix.rn) : 0; const fRowData = (m, metric_k) => { const dig = (metric_k === 'f1' || metric_k === 'auc' || metric_k === 'lr_minus') ? 3 : (metric_k === 'lr_plus' ? 2 : 1) ; return [fVal(m?.value, dig, true), fVal(m?.ci?.lower, dig, true), fVal(m?.ci?.upper, dig, true)]; }; return [ getKollektivDisplayName(dN), n, ...fRowData(stats.sens, 'sens'), ...fRowData(stats.spez, 'spez'), ...fRowData(stats.ppv, 'ppv'), ...fRowData(stats.npv, 'npv'), ...fRowData(stats.lr_plus, 'lr_plus'), ...fRowData(stats.lr_minus, 'lr_minus'), ...fRowData(stats.acc, 'acc'), ...fRowData(stats.balAcc, 'balAcc'), ...fRowData(stats.f1, 'f1'), ...fRowData(stats.auc, 'auc'), stats.sens?.method || na, stats.auc?.method || na ]; });
                ...
                } else if (isAsVsT2View && actionId === 'download-performance-as-vs-t2-csv') { if (!statsAS || !statsT2) { ui_helpers.showToast("Vergleichsdaten für CSV fehlen.", "warning"); return; } const headers = ['Metrik', 'AS (Wert)', 'AS (95% CI Unten)', 'AS (95% CI Oben)', `${options.t2CriteriaLabelShort || 'T2'} (Wert)`, `${options.t2CriteriaLabelShort || 'T2'} (95% CI Unten)`, `${options.t2CriteriaLabelShort || 'T2'} (95% CI Oben)`, 'CI Methode AS (Prop/Effekt)', `CI Methode ${options.t2CriteriaLabelShort || 'T2'} (Prop/Effekt)`]; const fRow = (mKey, nm, isP = true, d = 1) => { const mAS = statsAS[mKey]; const mT2 = statsT2[mKey]; const dig = (mKey === 'auc' || mKey === 'f1' || mKey === 'lr_minus') ? 3 : (mKey === 'lr_plus' ? 2 : 1); const valAS = formatNumber(mAS?.value, dig, na, true); const ciAS_L = formatNumber(mAS?.ci?.lower, dig, na, true); const ciAS_U = formatNumber(mAS?.ci?.upper, dig, na, true); const valT2 = formatNumber(mT2?.value, dig, na, true); const ciT2_L = formatNumber(mT2?.ci?.lower, dig, na, true); const ciT2_U = formatNumber(mT2?.ci?.upper, dig, na, true); return [nm, valAS, ciAS_L, ciAS_U, valT2, ciT2_L, ciT2_U, mAS?.method || na, mT2?.method || na]; }; const rows = [ fRow('sens', 'Sensitivität'), fRow('spez', 'Spezifität'), fRow('ppv', 'PPV'), fRow('npv', 'NPV'), fRow('lr_plus', 'LR+', false, 2), fRow('lr_minus', 'LR-', false, 3), fRow('acc', 'Accuracy'), fRow('balAcc', 'Balanced Accuracy'), fRow('f1', 'F1-Score', false, 3), fRow('auc', 'AUC', false, 3) ]; content = Papa.unparse([headers, ...rows], { delimiter: APP_CONFIG.EXPORT_SETTINGS.CSV_DELIMITER || ";" }); filenameKey = 'PRAES_AS_VS_T2_PERF_CSV'; extension = 'csv'; mimeType = 'text/csv;charset=utf-8;';
                } else if (isAsVsT2View && actionId === 'download-comp-table-as-vs-t2-md') { content = generateMarkdownTableString(presentationData, 'praes_as_vs_t2_comp', options.kollektivForComparison, null, null, options); filenameKey = 'PRAES_AS_VS_T2_COMP_MD'; extension = 'md'; mimeType = 'text/markdown;charset=utf-8;';
                } else if (isAsVsT2View && actionId === 'download-tests-as-vs-t2-md') { content = generateMarkdownTableString(presentationData, 'praes_as_vs_t2_tests', options.kollektivForComparison, null, null, options); filenameKey = 'PRAES_AS_VS_T2_TESTS_MD'; extension = 'md'; mimeType = 'text/markdown;charset=utf-8;';
                }
            } catch(error) {
            ...
            if(content !== null && filenameKey && extension && mimeType) { const filename = generateFilename(filenameKey, options.kollektivForComparison, extension, options); success = downloadFile(content, filename, mimeType); if(success) ui_helpers.showToast(`Präsentationsdaten (${extension}) exportiert: ${filename}`, 'success'); }
