const exportService = (() => {
    ...
    function generateStatistikCSVString(stats, kollektiv, criteria, logic) {
        ...
        const csvData = []; const na = 'N/A'; const fv = (v, d) => formatNumber(v, d, na); const fp = (v, d) => formatPercent(v, d, na);
        const fCI = (o, d, p) => !o || typeof o !== 'object' || o.lower === null || o.upper === null || isNaN(o.lower) || isNaN(o.upper) ? [na, na] : [fv(o.lower, d), fv(o.upper, d)];
        const fPVal = (p) => (p !== null && !isNaN(p)) ? (p < 0.001 ? '<0.001' : fv(p, 3)) : na;
        try {
            ...
            csvData.push(['Metrik (Diagnostik)', 'Methode', 'Wert', '95% CI Lower', '95% CI Upper', 'SE (Bootstrap)', 'CI Methode']); const addPerfRow = (metricKey, metricName, objAS, objT2) => { const isRate = !(metricKey === 'auc' || metricKey === 'f1'); const digits = isRate ? 1 : 3; const ciAS = fCI(objAS?.ci, digits); const ciT2 = fCI(objT2?.ci, digits); const valAS = isRate ? fp(objAS?.value, digits) : fv(objAS?.value, digits); const valT2 = isRate ? fp(objT2?.value, digits) : fv(objT2?.value, digits); csvData.push([metricName, 'AS', valAS, ciAS[0], ciAS[1], fv(objAS?.se, 4), objAS?.method || na]); csvData.push([metricName, 'T2', valT2, ciT2[0], ciT2[1], fv(objT2?.se, 4), objT2?.method || na]); }; const gAS = stats.gueteAS, gT2 = stats.gueteT2; addPerfRow('sens', 'Sensitivität', gAS?.sens, gT2?.sens); addPerfRow('spez', 'Spezifität', gAS?.spez, gT2?.spez); addPerfRow('ppv', 'PPV', gAS?.ppv, gT2?.ppv); addPerfRow('npv', 'NPV', gAS?.npv, gT2?.npv); addPerfRow('acc', 'Accuracy', gAS?.acc, gT2?.acc); addPerfRow('balAcc', 'Balanced Accuracy', gAS?.balAcc, gT2?.balAcc); addPerfRow('f1', 'F1-Score', gAS?.f1, gT2?.f1); addPerfRow('auc', 'AUC', gAS?.auc, gT2?.auc); csvData.push([]);
            csvData.push(['Vergleichstest (AS vs. T2)', 'Test Statistik', 'p-Wert', 'Methode']); const v = stats.vergleichASvsT2; csvData.push(['Accuracy (McNemar)', fv(v?.mcnemar?.statistic, 3), fPVal(v?.mcnemar?.pValue), v?.mcnemar?.method || na]); csvData.push(['AUC (DeLong)', `Z=${fv(v?.delong?.Z, 3)}`, fPVal(v?.delong?.pValue), v?.delong?.method || na]); csvData.push([]);
            csvData.push(['Assoziation mit N-Status', 'Merkmal Key', 'Merkmal Name', 'OR', 'OR CI Lower', 'OR CI Upper', 'RD', 'RD CI Lower', 'RD CI Upper', 'Phi', 'Test Statistik', 'p-Wert', 'Test Methode']); const addAssocRow = (key, name, obj) => { if (!obj) return; const orCI = fCI(obj.or?.ci, 2); const rdCI = fCI(obj.rd?.ci, 3); csvData.push([ key, name, fv(obj.or?.value, 2), orCI[0], orCI[1], fv(obj.rd?.value, 3), rdCI[0], rdCI[1], fv(obj.phi?.value, 2), fv(obj.statistic ?? NaN, 2), fPVal(obj.pValue), obj.testName || na ]); }; const a = stats.assoziation; addAssocRow('as', 'AS Positiv', a?.as); if(a?.size_mwu) { csvData.push(['size_mwu', a.size_mwu.featureName || 'LK Größe MWU', na, na, na, na, na, na, na, fv(a.size_mwu.statistic, 2), fPVal(a.size_mwu.pValue), a.size_mwu.testName || na ]); } ['size', 'form', 'kontur', 'homogenitaet', 'signal'].forEach(fKey => { if(a && a[fKey]) { addAssocRow(fKey, a[fKey].featureName || `T2 ${fKey}`, a[fKey]); } });
    ...
    function _calculateStatsForExport(data, criteria, logic) {
         ...
         try {
            ...
            if(typeof t2CriteriaManager !== 'undefined' && typeof t2CriteriaManager.evaluateDataset === 'function') {
                 evaluatedData = t2CriteriaManager.evaluateDataset(cloneDeep(data), criteria, logic);
            }

            if (!evaluatedData || evaluatedData.length === 0) return null;
            return statisticsService.calculateStatsForExport(evaluatedData, criteria, logic);
         } catch(error) { console.error("Fehler bei der Statistikberechnung für Export:", error); return null; }
     }

    function generateComprehensiveReportHTML(data, bfResults, kollektiv, criteria, logic) {
        try {
            const stats = _calculateStatsForExport(data, criteria, logic);
            if (!data || !stats || !criteria || !logic) return '<html><head><title>Fehler</title></head><body>Fehler: Notwendige Daten für Report fehlen.</body></html>';
            const formatCriteriaFunc = typeof studyT2CriteriaManager !== 'undefined' ? studyT2CriteriaManager.formatCriteriaForDisplay : (c, l) => 'N/A';
            ...
            const chartIdsToCapture = []; if (config.INCLUDE_DESCRIPTIVES_CHARTS) { ['chart-dash-age', 'chart-dash-gender', 'chart-dash-therapy', 'chart-dash-status-n', 'chart-dash-status-as', 'chart-dash-status-t2'].forEach(id => { if(document.getElementById(id)?.querySelector('svg')) chartIdsToCapture.push(id);}); ['chart-stat-age-0', 'chart-stat-gender-0'].forEach(id => { if(document.getElementById(id)?.querySelector('svg')) chartIdsToCapture.push(id);}); } if (config.INCLUDE_AS_VS_T2_COMPARISON_CHART) { const p = ['praes-comp-chart-container', 'stat-comp-bar-0'].find(id => document.getElementById(id)?.querySelector('svg')); if(p) chartIdsToCapture.push(p); } chartIdsToCapture.forEach(id => { const actualId = document.getElementById(id) ? id : (id.endsWith('-0') && document.getElementById(id.replace('-0','')) ? id.replace('-0','') : null); if (actualId) chartSVGs[actualId] = getChartSVG(actualId); });
            ...
            if (config.INCLUDE_DESCRIPTIVES_CHARTS) { html += `<div style="display: flex; flex-wrap: wrap; gap: 10px; margin-top: 1em; justify-content: space-around;">`; ['chart-dash-age', 'chart-stat-age-0'].forEach(cid => { if (chartSVGs[cid]) html += `<div class="chart-container" style="flex: 1 1 45%; min-width: 300px;"><h3>Altersverteilung</h3>${chartSVGs[cid]}</div>`; }); ['chart-dash-gender', 'chart-stat-gender-0'].forEach(cid => { if (chartSVGs[cid]) html += `<div class="chart-container" style="flex: 1 1 45%; min-width: 300px;"><h3>Geschlechterverteilung</h3>${chartSVGs[cid]}</div>`; }); html += `</div>`; }
            const addPerfSection = (reportTitle, statsObj) => { if (!statsObj) return ''; let sHtml = `<h2>${reportTitle}</h2><table><thead><tr><th>Metrik</th><th>Wert (95% CI)</th><th>CI Methode</th></tr></thead><tbody>`; const fCI_val = (m, d=1, p=true) => formatCI(m?.value, m?.ci?.lower, m?.ci?.upper, d, p, '--'); const na_val = '--'; sHtml += `<tr><td>Sensitivität</td><td>${fCI_val(statsObj.sens)}</td><td>${statsObj.sens?.method || na_val}</td></tr>`; sHtml += `<tr><td>Spezifität</td><td>${fCI_val(statsObj.spez)}</td><td>${statsObj.spez?.method || na_val}</td></tr>`; sHtml += `<tr><td>PPV</td><td>${fCI_val(statsObj.ppv)}</td><td>${statsObj.ppv?.method || na_val}</td></tr>`; sHtml += `<tr><td>NPV</td><td>${fCI_val(statsObj.npv)}</td><td>${statsObj.npv?.method || na_val}</td></tr>`; sHtml += `<tr><td>Accuracy</td><td>${fCI_val(statsObj.acc)}</td><td>${statsObj.acc?.method || na_val}</td></tr>`; sHtml += `<tr><td>Balanced Accuracy</td><td>${fCI_val(statsObj.balAcc)}</td><td>${statsObj.balAcc?.method || na_val}</td></tr>`; sHtml += `<tr><td>F1-Score</td><td>${fCI_val(statsObj.f1, 3, false)}</td><td>${statsObj.f1?.method || na_val}</td></tr>`; sHtml += `<tr><td>AUC</td><td>${fCI_val(statsObj.auc, 3, false)}</td><td>${statsObj.auc?.method || na_val}</td></tr>`; sHtml += `</tbody></table>`; return sHtml; };
            if (config.INCLUDE_AS_PERFORMANCE_TABLE && stats?.gueteAS) { html += addPerfSection('Diagnostische Güte: Avocado Sign (vs. N)', stats.gueteAS); }
            if (config.INCLUDE_T2_PERFORMANCE_TABLE && stats?.gueteT2) { html += addPerfSection(`Diagnostische Güte: T2 ('${appliedCriteriaDisplayName}' vs. N)`, stats.gueteT2); }
            if (config.INCLUDE_AS_VS_T2_COMPARISON_TABLE && stats?.vergleichASvsT2) { html += `<h2>Statistischer Vergleich: AS vs. T2 ('${appliedCriteriaDisplayName}')</h2><table><thead><tr><th>Test</th><th>Statistik</th><th>p-Wert</th><th>Methode</th></tr></thead><tbody>`; const v = stats.vergleichASvsT2; const fP_val = (p) => (p !== null && !isNaN(p)) ? (p < 0.001 ? '<0.001' : formatNumber(p, 3, '--')) : '--'; const na_val = '--'; html += `<tr><td>Accuracy (McNemar)</td><td>${formatNumber(v?.mcnemar?.statistic, 3, na_val)} (df=${v?.mcnemar?.df || na_val})</td><td>${fP_val(v?.mcnemar?.pValue)} ${getStatisticalSignificanceSymbol(v?.mcnemar?.pValue)}</td><td>${v?.mcnemar?.method || na_val}</td></tr>`; html += `<tr><td>AUC (DeLong)</td><td>Z=${formatNumber(v?.delong?.Z, 3, na_val)}</td><td>${fP_val(v?.delong?.pValue)} ${getStatisticalSignificanceSymbol(v?.delong?.pValue)}</td><td>${v?.delong?.method || na_val}</td></tr>`; html += `</tbody></table>`; }
            ...
            if (config.INCLUDE_ASSOCIATIONS_TABLE && stats?.assoziation && Object.keys(stats.assoziation).length > 0) { html += `<h2>Assoziation mit N-Status</h2><table><thead><tr><th>Merkmal</th><th>OR (95% CI)</th><th>RD (%) (95% CI)</th><th>Phi</th><th>p-Wert</th><th>Test</th></tr></thead><tbody>`; const a = stats.assoziation; const na_val = '--'; const fP_val = (p) => (p !== null && !isNaN(p)) ? (p < 0.001 ? '<0.001' : formatNumber(p, 3, na_val)) : na_val; const fRow = (nm, obj) => { if (!obj) return ''; const orS = formatCI(obj.or?.value, obj.or?.ci?.lower, obj.or?.ci?.upper, 2, false, na_val); const rdV = formatNumber(obj.rd?.value !== null && !isNaN(obj.rd?.value) ? obj.rd.value * 100 : NaN, 1, na_val); const rdL = formatNumber(obj.rd?.ci?.lower !== null && !isNaN(obj.rd?.ci?.lower) ? obj.rd.ci.lower * 100 : NaN, 1, na_val); const rdU = formatNumber(obj.rd?.ci?.upper !== null && !isNaN(obj.rd?.ci?.upper) ? obj.rd.ci.upper * 100 : NaN, 1, na_val); const rdS = `${rdV} (${rdL} - ${rdU})`; const phiS = formatNumber(obj.phi?.value, 2, na_val); const pS = fP_val(obj.pValue) + ' ' + getStatisticalSignificanceSymbol(obj.pValue); const tN = obj.testName || na_val; return `<tr><td>${nm}</td><td>${orS}</td><td>${rdS}</td><td>${phiS}</td><td>${pS}</td><td>${tN}</td></tr>`; }; html += fRow('AS Positiv', a?.as); if (a?.size_mwu) html += `<tr><td>LK Größe (Median Vgl.)</td><td>${na_val}</td><td>${na_val}</td><td>${na_val}</td><td>${fP_val(a.size_mwu.pValue)} ${getStatisticalSignificanceSymbol(a.size_mwu.pValue)}</td><td>${a.size_mwu.testName || na_val}</td></tr>`; ['size', 'form', 'kontur', 'homogenitaet', 'signal'].forEach(k_val => { if (a && a[k_val]) { const isActive = criteria[k_val]?.active === true; html += fRow(a[k_val].featureName + (isActive ? '' : ' (inaktiv)'), a[k_val]); } }); html += `</tbody></table>`; }
    ...
    function exportTableMarkdown(dataOrStats, tableType, kollektiv, criteria = null, logic = null, options = {}) {
        ...
        if(tableType === 'patienten') { typeKey = 'PATIENTEN_MD'; title = 'Datenliste'; }
    ...
    function exportComprehensiveReportHTML(data, bfResults, kollektiv, criteria, logic) {
        let htmlString = null;
        try { htmlString = generateComprehensiveReportHTML(data, bfResults, kollektiv, criteria, logic); } catch(e) { ui_helpers.showToast("Fehler bei HTML-Report-Erstellung.", "danger"); return; }
    ...
    async function exportChartsZip(scopeSelector, zipTypeKey, kollektiv, format) {
         ...
         const tableSelectors = [
            ...
            scopeSelector + ' table.publication-table'
         ];
         ...
         chartContainers.forEach((container, index) => {
             ...
             const chartId = container.id || `chart_${index + 1}`; const chartName = chartId.replace(/^chart-/, '').replace(/-container$/, '').replace(/-content$/, '').replace(/-[0-9]+$/, ''); let filenameKey, conversionPromise, ext;
         ...
         tableContainers.forEach((table) => {
              ...
              promises.push(convertTableToPngBlob(tableId, baseWidth).catch(e => { console.error(`Tabellen-PNG Konvertierung für ${tableName} fehlgeschlagen:`, e); return null; }).then(blob => (blob ? { blob, filename } : { error: new Error("Table Blob is null for table " + tableName), filename })));
         });
     ...
     async function exportCategoryZip(category, data, bfResults, kollektiv, criteria, logic) {
         ...
         if(needsStats && data && data.length > 0 && criteria && logic) { try { stats = _calculateStatsForExport(data, criteria, logic); } catch(e) { ui_helpers.showToast(`Fehler bei Statistikberechnung für ${category.toUpperCase()} ZIP.`, 'danger'); return; } if (!stats) { ui_helpers.showToast(`Statistikberechnung ergab keine Daten für ${category.toUpperCase()} ZIP.`, 'warning'); return; } }
         ...
         try {
             ...
             if (['all-zip', 'md-zip'].includes(category)) { if (stats?.deskriptiv) addFile(generateFilename('DESKRIPTIV_MD', kollektiv, 'md'), generateMarkdownTableString(stats.deskriptiv, 'deskriptiv', kollektiv)); if (data && data.length > 0) addFile(generateFilename('PATIENTEN_MD', kollektiv, 'md'), generateMarkdownTableString(data, 'patienten', kollektiv)); if (data && data.length > 0) addFile(generateFilename('AUSWERTUNG_MD', kollektiv, 'md'), generateMarkdownTableString(data, 'auswertung', kollektiv, criteria, logic)); }
             ...
             if (['all-zip', 'html'].includes(category) && stats) { addFile(generateFilename('COMPREHENSIVE_REPORT_HTML', kollektiv, 'html'), generateComprehensiveReportHTML(data, bfResults, kollektiv, criteria, logic)); }
     ...
     function exportPraesentationData(actionId, presentationData, kollektiv) {
            ...
            try {
                if (isAsPurView && actionId === 'download-performance-as-pur-csv') {
                     ...
                     const rows = Object.entries(allStatsData).map(([key, stats]) => { let k = key.replace('stats',''); let dN = (k === 'Gesamt') ? 'Gesamt' : (k === 'DirektOP') ? 'direkt OP' : (k === 'NRCT') ? 'nRCT' : k; if (!stats || typeof stats.matrix !== 'object') return [getKollektivDisplayName(dN), 0, ...Array(21).fill(na), na]; const n = stats.matrix ? (stats.matrix.rp + stats.matrix.fp + stats.matrix.fn + stats.matrix.rn) : 0; const fRowData = (m, metricKey) => { const d = (metricKey === 'f1' || metricKey === 'auc') ? 3 : 1; return [fVal(m?.value, d), fVal(m?.ci?.lower, d), fVal(m?.ci?.upper, d)]; }; return [ getKollektivDisplayName(dN), n, ...fRowData(stats.sens, 'sens'), ...fRowData(stats.spez, 'spez'), ...fRowData(stats.ppv, 'ppv'), ...fRowData(stats.npv, 'npv'), ...fRowData(stats.acc, 'acc'), ...fRowData(stats.balAcc, 'balAcc'), ...fRowData(stats.f1, 'f1'), ...fRowData(stats.auc, 'auc'), stats.sens?.method || na ]; });
