const exportService = (() => {
    ...
    function downloadFile(content, filename, mimeType) {
        ...
        try {
            if (content === null || content === undefined) {
                 const errorMsg = langKey === 'de' ? `Export fehlgeschlagen: Keine Daten für ${filename} generiert.` : `Export failed: No data generated for ${filename}.`;
            ...
            if (content instanceof Blob) {
                blob = content;
                if (blob.size === 0) {
                    const emptyFileMsg = langKey === 'de' ? `Export fehlgeschlagen: Leere Datei für ${filename} generiert.` : `Export failed: Empty file generated for ${filename}.`;
                    ui_helpers.showToast(emptyFileMsg, 'warning');
                    return false;
                }
            } else {
                 const stringContent = String(content);
                 if (stringContent.length === 0) {
                    const emptyStringMsg = langKey === 'de' ? `Export fehlgeschlagen: Leere Datei für ${filename} generiert.` : `Export failed: Empty file generated for ${filename}.`;
                    ui_helpers.showToast(emptyStringMsg, 'warning');
                    return false;
                 }
            ...
            const downloadErrorMsg = langKey === 'de' ? `Fehler beim Herunterladen der Datei '${filename}'.` : `Error downloading file '${filename}'.`;
            ui_helpers.showToast(downloadErrorMsg, 'danger');
            return false;
        }
    }
    ...
    async function convertSvgToPngBlob(svgElement, targetWidth = 800) {
        ...
        return new Promise((resolve, reject) => {
            if (!svgElement || typeof svgElement.cloneNode !== 'function') {
                const errorMsg = langKey === 'de' ? "Ungültiges SVG Element für PNG Konvertierung." : "Invalid SVG element for PNG conversion.";
            ...
            try {
                ...
                if (sourceWidth <= 0 || sourceHeight <= 0) {
                    const dimErrorMsg = langKey === 'de' ? "SVG-Dimensionen konnten nicht ermittelt werden." : "SVG dimensions could not be determined.";
                    return reject(new Error(dimErrorMsg));
                }
                ...
                img.onload = () => {
                    ...
                    if (!ctx) {
                        const canvasError = langKey === 'de' ? "Canvas Context nicht verfügbar." : "Canvas context not available.";
                        return reject(new Error(canvasError));
                    }
                    ...
                    canvas.toBlob((blob) => {
                        if (blob) { resolve(blob); }
                            ...
                            const blobError = langKey === 'de' ? "Canvas toBlob fehlgeschlagen." : "Canvas toBlob failed.";
                            reject(new Error(blobError));
                        }
                    }, 'image/png');
                };
                img.onerror = (err) => {
                    const loadError = langKey === 'de' ? "Fehler beim Laden des SVG-Bildes für PNG-Konvertierung." : "Error loading SVG image for PNG conversion.";
                    reject(new Error(loadError));
                };
                img.src = svgDataUrl;
            } catch (error) {
                const conversionError = langKey === 'de' ? `Fehler bei SVG-zu-PNG Konvertierung: ${error.message}` : `Error during SVG to PNG conversion: ${error.message}`;
                reject(new Error(conversionError));
            }
        });
    }
     ...
     async function convertSvgToSvgBlob(svgElement) {
        const langKey = (typeof state !== 'undefined' && typeof state.getCurrentPublikationLang === 'function') ? state.getCurrentPublikationLang() : 'de';
         return new Promise((resolve, reject) => {
             if (!svgElement || typeof svgElement.cloneNode !== 'function') {
                const invalidSvgError = langKey === 'de' ? "Ungültiges SVG Element für SVG Export." : "Invalid SVG element for SVG export.";
                return reject(new Error(invalidSvgError));
             }
              try {
                 const svgClone = svgElement.cloneNode(true);
                 svgClone.setAttribute('xmlns', 'http://www.w3.org/2000/svg');
                ...
                const conversionError = langKey === 'de' ? `Fehler bei SVG-zu-SVG Blob Konvertierung: ${error.message}` : `Error during SVG to SVG blob conversion: ${error.message}`;
                reject(new Error(conversionError));
              }
         });
     }
    ...
    function generateStatistikCSVString(stats, kollektiv, criteria, logic) {
        ...
        const fCICSV = (o, d, p) => !o || typeof o !== 'object' || o.lower === null || o.upper === null || isNaN(o.lower) || !isFinite(o.lower) || isNaN(o.upper) || !isFinite(o.upper) ? [na, na] : [fv(o.lower, d, p), fv(o.upper, d, p)];
        const fPVal = (pv) => (pv !== null && !isNaN(pv) && isFinite(pv)) ? (pv < 0.001 ? (langKey==='de'?'<0,001':'<.001') : formatNumber(pv, 3, na, false, langKey)) : na;

        try {
            csvData.push(['Parameter', 'Wert']); csvData.push(['Kollektiv', getKollektivDisplayName(kollektiv, langKey)]); csvData.push(['Angewandte T2 Logik', UI_TEXTS.t2LogicDisplayNames[logic?.toUpperCase()]?.[langKey] || logic]); csvData.push(['Angewandte T2 Kriterien', formatCriteriaFunc(criteria, logic)]); csvData.push(['Anzahl Patienten', stats.deskriptiv.anzahlPatienten]); csvData.push([]);
            ...
            csvData.push([`Geschlecht ${UI_TEXTS.legendLabels.male[langKey] || 'Männlich'} (n / %)`, `${d.geschlecht?.m ?? 0} / ${fp(d.anzahlPatienten > 0 ? (d.geschlecht?.m ?? 0) / d.anzahlPatienten : NaN, 1)}`]);
            csvData.push([`Geschlecht ${UI_TEXTS.legendLabels.female[langKey] || 'Weiblich'} (n / %)`, `${d.geschlecht?.f ?? 0} / ${fp(d.anzahlPatienten > 0 ? (d.geschlecht?.f ?? 0) / d.anzahlPatienten : NaN, 1)}`]);
            csvData.push([`Therapie ${UI_TEXTS.legendLabels.direktOP[langKey] || 'Direkt OP'} (n / %)`, `${d.therapie?.['direkt OP'] ?? 0} / ${fp(d.anzahlPatienten > 0 ? (d.therapie?.['direkt OP'] ?? 0) / d.anzahlPatienten : NaN, 1)}`]);
            csvData.push([`Therapie ${UI_TEXTS.legendLabels.nRCT[langKey] || 'nRCT'} (n / %)`, `${d.therapie?.nRCT ?? 0} / ${fp(d.anzahlPatienten > 0 ? (d.therapie?.nRCT ?? 0) / d.anzahlPatienten : NaN, 1)}`]);
            csvData.push([`N Status (${UI_TEXTS.legendLabels.nPositive[langKey] || 'N+'}) (n / %)`, `${d.nStatus?.plus ?? 0} / ${fp(d.anzahlPatienten > 0 ? (d.nStatus?.plus ?? 0) / d.anzahlPatienten : NaN, 1)}`]);
            csvData.push([`AS Status (${UI_TEXTS.legendLabels.asPositive[langKey] || 'AS+'}) (n / %)`, `${d.asStatus?.plus ?? 0} / ${fp(d.anzahlPatienten > 0 ? (d.asStatus?.plus ?? 0) / d.anzahlPatienten : NaN, 1)}`]);
            csvData.push([`T2 Status (${UI_TEXTS.legendLabels.t2Positive[langKey] || 'T2+'}) (n / %)`, `${d.t2Status?.plus ?? 0} / ${fp(d.anzahlPatienten > 0 ? (d.t2Status?.plus ?? 0) / d.anzahlPatienten : NaN, 1)}`]);
            const fLKRow = (lk) => [fv(lk?.median, 1), fv(lk?.mean, 1), fv(lk?.sd, 1), fv(lk?.min, 0), fv(lk?.max, 0)];
            csvData.push(['LK N gesamt (Median)', ...fLKRow(d.lkAnzahlen?.n?.total)]); csvData.push([`LK N${UI_TEXTS.legendLabels.nPositive[langKey] || 'N+'} (Median, nur N+ Pat.)`, ...fLKRow(d.lkAnzahlen?.n?.plus)]); csvData.push(['LK AS gesamt (Median)', ...fLKRow(d.lkAnzahlen?.as?.total)]); csvData.push([`LK AS${UI_TEXTS.legendLabels.asPositive[langKey] || 'AS+'} (Median, nur AS+ Pat.)`, ...fLKRow(d.lkAnzahlen?.as?.plus)]); csvData.push(['LK T2 gesamt (Median)', ...fLKRow(d.lkAnzahlen?.t2?.total)]); csvData.push([`LK T2${UI_TEXTS.legendLabels.t2Positive[langKey] || 'T2+'} (Median, nur T2+ Pat.)`, ...fLKRow(d.lkAnzahlen?.t2?.plus)]); csvData.push([]);
            csvData.push(['Metrik (Diagnostik)', 'Methode', 'Wert', '95% CI Lower', '95% CI Upper', 'SE (Bootstrap)', 'CI Methode']); const addPerfRow = (metricKey, metricName, objAS, objT2) => { const isRate = !(metricKey === 'auc' || metricKey === 'f1'); const digits = isRate ? 1 : 3; const useStandard = metricKey === 'auc' || metricKey === 'f1'; const ciAS = fCICSV(objAS?.ci, digits, useStandard); const ciT2 = fCICSV(objT2?.ci, digits, useStandard); const valAS = isRate ? fp(objAS?.value, digits) : fv(objAS?.value, digits, useStandard); const valT2 = isRate ? fp(objT2?.value, digits) : fv(objT2?.value, digits, useStandard); csvData.push([metricName, 'AS', valAS, ciAS[0], ciAS[1], fv(objAS?.se, 4, true), objAS?.method || na]); csvData.push([metricName, 'T2', valT2, ciT2[0], ciT2[1], fv(objT2?.se, 4, true), objT2?.method || na]); }; const gAS = stats.gueteAS, gT2 = stats.gueteT2_angewandt || stats.gueteT2; addPerfRow('sens', UI_TEXTS.statMetrics.sens.name[langKey] || 'Sensitivität', gAS?.sens, gT2?.sens); addPerfRow('spez', UI_TEXTS.statMetrics.spez.name[langKey] || 'Spezifität', gAS?.spez, gT2?.spez); addPerfRow('ppv', UI_TEXTS.statMetrics.ppv.name[langKey] || 'PPV', gAS?.ppv, gT2?.ppv); addPerfRow('npv', UI_TEXTS.statMetrics.npv.name[langKey] || 'NPV', gAS?.npv, gT2?.npv); addPerfRow('acc', UI_TEXTS.statMetrics.acc.name[langKey] || 'Accuracy', gAS?.acc, gT2?.acc); addPerfRow('balAcc', UI_TEXTS.statMetrics.balacc.name[langKey] || 'Balanced Accuracy', gAS?.balAcc, gT2?.balAcc); addPerfRow('f1', UI_TEXTS.statMetrics.f1.name[langKey] || 'F1-Score', gAS?.f1, gT2?.f1); addPerfRow('auc', UI_TEXTS.statMetrics.auc.name[langKey] || 'AUC', gAS?.auc, gT2?.auc); csvData.push([]);
            ...
            csvData.push(['Assoziation mit N-Status', 'Merkmal Key', 'Merkmal Name', 'OR', 'OR CI Lower', 'OR CI Upper', 'RD (%)', 'RD CI Lower (%)', 'RD CI Upper (%)', 'Phi', 'Test Statistik', 'p-Wert', 'Test Methode']); const addAssocRow = (key, name, obj) => { if (!obj) return; const orCI = fCICSV(obj.or?.ci, 2, true); const rdVal = obj.rd?.value !== null && !isNaN(obj.rd?.value) && isFinite(obj.rd.value) ? obj.rd.value * 100 : NaN; const rdCILow = obj.rd?.ci?.lower !== null && !isNaN(obj.rd.ci.lower) && isFinite(obj.rd.ci.lower) ? obj.rd.ci.lower * 100 : NaN; const rdCIUpp = obj.rd?.ci?.upper !== null && !isNaN(obj.rd.ci.upper) && isFinite(obj.rd.ci.upper) ? obj.rd.ci.upper * 100 : NaN; const rdValFormatted = fv(rdVal, 1); const rdCILowFormatted = fv(rdCILow, 1); const rdCIUppFormatted = fv(rdCIUpp, 1); const rdCombined = (rdValFormatted !== na && rdCILowFormatted !== na && rdCIUppFormatted !== na) ? `${rdValFormatted} (${rdCILowFormatted} - ${rdCIUppFormatted})` : rdValFormatted; csvData.push([ key, name, fv(obj.or?.value, 2, true), orCI[0], orCI[1], rdCombined, na, na, fv(obj.phi?.value, 2, true), fv(obj.statistic ?? NaN, 2, true), fPVal(obj.pValue), obj.testName || na ]); }; const a = stats.assoziation_angewandt || stats.assoziation; if (a) { addAssocRow('as', a?.as?.featureName || 'AS Positiv', a?.as); if(a?.size_mwu) { csvData.push(['size_mwu', a.size_mwu.featureName || 'LK Größe MWU', na, na, na, na, na, na, na, `U=${fv(a.size_mwu.statistic, 0, true)}, Z=${fv(a.size_mwu.Z,2,true)}`, fPVal(a.size_mwu.pValue), a.size_mwu.testName || na ]); } ['size', 'form', 'kontur', 'homogenitaet', 'signal'].forEach(fKey => { if(a && a[fKey]) { addAssocRow(fKey, a[fKey].featureName || `T2 ${fKey}`, a[fKey]); } }); }
    ...
    function generateMarkdownTableString(dataOrStats, tableType, kollektiv, criteria = null, logic = null, options = {}) {
        ...
        try {
            ...
            const titleLang = (titleKey, fallbackDe, fallbackEn) => (UI_TEXTS.publicationTableTitles?.[titleKey]?.[langKey] || UI_TEXTS.publicationTableTitles?.[titleKey]?.['de']) || (langKey==='de'?fallbackDe:fallbackEn);
            ...
            console.error(`Fehler in generateMarkdownTableString für ${tableType}:`, error);
    ...
    function generateComprehensiveReportHTML(data, bfResultsForCurrentKollektiv, kollektiv, criteria, logic) {
        ...
        try {
            ...
            const formatCriteriaFunc = typeof studyT2CriteriaManager !== 'undefined' ? (c, l) => studyT2CriteriaManager.formatCriteriaForDisplay(c, l, false, langKey) : (c, l) => 'N/A';
            const config = APP_CONFIG.REPORT_SETTINGS; const kollektivName = getKollektivDisplayName(kollektiv, langKey); const timestamp = new Date().toLocaleString(langKey === 'de' ? 'de-DE' : 'en-US', { dateStyle: 'long', timeStyle: 'medium'}); const criteriaString = formatCriteriaFunc(criteria, logic); const appliedCriteriaDisplayNameBase = UI_TEXTS.kollektivDisplayNames.applied_criteria; const appliedCriteriaDisplayName = (typeof appliedCriteriaDisplayNameBase === 'object' ? appliedCriteriaDisplayNameBase[langKey] : appliedCriteriaDisplayNameBase?.[langKey]) || appliedCriteriaDisplayNameBase?.['de'] || 'Angewandte T2 Kriterien'; let chartSVGs = {};
            ...
            if (config.INCLUDE_DESCRIPTIVES_CHARTS) { chartIdsToCapture.push('chart-stat-age-0', 'chart-stat-gender-0'); } // Assuming these are rendered for currentGlobalKollektiv on Stat-Tab
            if (config.INCLUDE_AS_VS_T2_COMPARISON_CHART && (typeof state !== 'undefined' && state.getCurrentPresentationView && state.getCurrentPresentationView() === 'as-vs-t2') && (typeof state !== 'undefined' && state.getCurrentKollektiv && state.getCurrentKollektiv() === kollektiv)) {
            ...
            html += `<div class="meta-info"><h3>${langKey==='de'?'Analysekonfiguration':'Analysis Configuration'}</h3><ul>`; if (config.INCLUDE_KOLLEKTIV_INFO) html += `<li><strong>${langKey==='de'?'Analysiertes Kollektiv:':'Analyzed Cohort:'}</strong> ${kollektivName} (N=${statsDataForCurrentKollektiv?.deskriptiv?.anzahlPatienten || 0})</li>`; if (config.INCLUDE_T2_CRITERIA) html += `<li><strong>${langKey==='de'?'Angewandte T2-Kriterien':'Applied T2 Criteria'} ('${appliedCriteriaDisplayName}'):</strong> ${langKey==='de'?'Logik:':'Logic:'} ${UI_TEXTS.t2LogicDisplayNames[logic?.toUpperCase()]?.[langKey] || logic?.toUpperCase()}, ${langKey==='de'?'Kriterien:':'Criteria:'} ${criteriaString}</li>`; html += `</ul></div>`;
            if (config.INCLUDE_DESCRIPTIVES_TABLE && statsDataForCurrentKollektiv?.deskriptiv) { html += `<h2>${langKey==='de'?'Deskriptive Statistik':'Descriptive Statistics'}</h2>`; html += `<table><thead><tr><th>${langKey==='de'?'Metrik':'Metric'}</th><th>${langKey==='de'?'Wert (Median)':'Value (Median)'}</th><th>Mean</th><th>SD</th><th>Min</th><th>Max</th></tr></thead><tbody>`; const d = statsDataForCurrentKollektiv.deskriptiv; const na = '--'; const fvRep = (v, dig = 1) => formatNumber(v, dig, na, false, langKey); const fPRep = (v, dig = 1) => formatPercent(v, dig, na, langKey); const addRowHTML = (l, vl=na, m=na, s=na, mn=na, mx=na) => `<tr><td>${l}</td><td>${vl}</td><td>${m}</td><td>${s}</td><td>${mn}</td><td>${mx}</td></tr>`; html += addRowHTML(UI_TEXTS.axisLabels.age[langKey]||'Alter', fvRep(d.alter?.median, 1), fvRep(d.alter?.mean, 1), fvRep(d.alter?.sd, 1), fvRep(d.alter?.min, 0), fvRep(d.alter?.max, 0)); html += addRowHTML(`${UI_TEXTS.legendLabels.gender[langKey] || 'Geschlecht'} ${UI_TEXTS.legendLabels.male[langKey] || 'Männlich'} (n / %)`, `${d.geschlecht?.m ?? 0} / ${fPRep(d.anzahlPatienten > 0 ? (d.geschlecht?.m ?? 0) / d.anzahlPatienten : NaN, 1)}`); html += addRowHTML(`${UI_TEXTS.legendLabels.gender[langKey] || 'Geschlecht'} ${UI_TEXTS.legendLabels.female[langKey] || 'Weiblich'} (n / %)`, `${d.geschlecht?.f ?? 0} / ${fPRep(d.anzahlPatienten > 0 ? (d.geschlecht?.f ?? 0) / d.anzahlPatienten : NaN, 1)}`); html += addRowHTML(`${UI_TEXTS.chartTitles.therapyDistribution[langKey] || 'Therapie'} ${UI_TEXTS.legendLabels.direktOP[langKey] || 'Direkt OP'} (n / %)`, `${d.therapie?.['direkt OP'] ?? 0} / ${fPRep(d.anzahlPatienten > 0 ? (d.therapie?.['direkt OP'] ?? 0) / d.anzahlPatienten : NaN, 1)}`); html += addRowHTML(`${UI_TEXTS.chartTitles.therapyDistribution[langKey] || 'Therapie'} ${UI_TEXTS.legendLabels.nRCT[langKey] || 'nRCT'} (n / %)`, `${d.therapie?.nRCT ?? 0} / ${fPRep(d.anzahlPatienten > 0 ? (d.therapie?.nRCT ?? 0) / d.anzahlPatienten : NaN, 1)}`); html += addRowHTML(`N Status (${UI_TEXTS.legendLabels.nPositive[langKey] || 'N+'}) (n / %)`, `${d.nStatus?.plus ?? 0} / ${fPRep(d.anzahlPatienten > 0 ? (d.nStatus?.plus ?? 0) / d.anzahlPatienten : NaN, 1)}`); html += addRowHTML(`AS Status (${UI_TEXTS.legendLabels.asPositive[langKey] || 'AS+'}) (n / %)`, `${d.asStatus?.plus ?? 0} / ${fPRep(d.anzahlPatienten > 0 ? (d.asStatus?.plus ?? 0) / d.anzahlPatienten : NaN, 1)}`); html += addRowHTML(`T2 Status (${UI_TEXTS.legendLabels.t2Positive[langKey] || 'T2+'}) (n / %)`, `${d.t2Status?.plus ?? 0} / ${fPRep(d.anzahlPatienten > 0 ? (d.t2Status?.plus ?? 0) / d.anzahlPatienten : NaN, 1)}`); const fLKRep = (lk) => `${fvRep(lk?.median,1)} (${fvRep(lk?.min,0)}-${fvRep(lk?.max,0)})`; html += addRowHTML(`${langKey==='de'?'LK N gesamt':'LN N total'} (Median (Min-Max))`, fLKRep(d.lkAnzahlen?.n?.total), fvRep(d.lkAnzahlen?.n?.total?.mean,1), fvRep(d.lkAnzahlen?.n?.total?.sd,1),fvRep(d.lkAnzahlen?.n?.total?.min,0), fvRep(d.lkAnzahlen?.n?.total?.max,0)); html += addRowHTML(`${langKey==='de'?'LK':'LN'} N${UI_TEXTS.legendLabels.nPositive[langKey]||'N+'} (Median (Min-Max), ${langKey==='de'?'nur N+ Pat.':'N+ pts only'})`, fLKRep(d.lkAnzahlen?.n?.plus), fvRep(d.lkAnzahlen?.n?.plus?.mean,1), fvRep(d.lkAnzahlen?.n?.plus?.sd,1),fvRep(d.lkAnzahlen?.n?.plus?.min,0), fvRep(d.lkAnzahlen?.n?.plus?.max,0)); html += addRowHTML(`${langKey==='de'?'LK AS gesamt':'LN AS total'} (Median (Min-Max))`, fLKRep(d.lkAnzahlen?.as?.total), fvRep(d.lkAnzahlen?.as?.total?.mean,1), fvRep(d.lkAnzahlen?.as?.total?.sd,1),fvRep(d.lkAnzahlen?.as?.total?.min,0), fvRep(d.lkAnzahlen?.as?.total?.max,0)); html += addRowHTML(`${langKey==='de'?'LK':'LN'} AS${UI_TEXTS.legendLabels.asPositive[langKey]||'AS+'} (Median (Min-Max), ${langKey==='de'?'nur AS+ Pat.':'AS+ pts only'})`, fLKRep(d.lkAnzahlen?.as?.plus), fvRep(d.lkAnzahlen?.as?.plus?.mean,1), fvRep(d.lkAnzahlen?.as?.plus?.sd,1),fvRep(d.lkAnzahlen?.as?.plus?.min,0), fvRep(d.lkAnzahlen?.as?.plus?.max,0)); html += addRowHTML(`${langKey==='de'?'LK T2 gesamt':'LN T2 total'} (Median (Min-Max))`, fLKRep(d.lkAnzahlen?.t2?.total), fvRep(d.lkAnzahlen?.t2?.total?.mean,1), fvRep(d.lkAnzahlen?.t2?.total?.sd,1),fvRep(d.lkAnzahlen?.t2?.total?.min,0), fvRep(d.lkAnzahlen?.t2?.total?.max,0)); html += addRowHTML(`${langKey==='de'?'LK':'LN'} T2${UI_TEXTS.legendLabels.t2Positive[langKey]||'T2+'} (Median (Min-Max), ${langKey==='de'?'nur T2+ Pat.':'T2+ pts only'})`, fLKRep(d.lkAnzahlen?.t2?.plus), fvRep(d.lkAnzahlen?.t2?.plus?.mean,1), fvRep(d.lkAnzahlen?.t2?.plus?.sd,1),fvRep(d.lkAnzahlen?.t2?.plus?.min,0), fvRep(d.lkAnzahlen?.t2?.plus?.max,0)); html += `</tbody></table>`; }
            if (config.INCLUDE_DESCRIPTIVES_CHARTS) { html += `<div style="display: flex; flex-wrap: wrap; gap: 10px; margin-top: 1em; justify-content: space-around;">`; if (chartSVGs['chart-stat-age-0']) html += `<div class="chart-container" style="flex: 1 1 45%; min-width: 300px;"><h3>${UI_TEXTS.chartTitles.ageDistribution[langKey]||'Altersverteilung'}</h3>${chartSVGs['chart-stat-age-0']}</div>`; if (chartSVGs['chart-stat-gender-0']) html += `<div class="chart-container" style="flex: 1 1 45%; min-width: 300px;"><h3>${UI_TEXTS.chartTitles.genderDistribution[langKey]||'Geschlecht'}</h3>${chartSVGs['chart-stat-gender-0']}</div>`; html += `</div>`; }
            const addPerfSectionHTML = (title, statsObj) => { if (!statsObj) return ''; let sHtml = `<h2>${title}</h2><table><thead><tr><th>${langKey==='de'?'Metrik':'Metric'}</th><th>${langKey==='de'?'Wert (95% CI)':'Value (95% CI)'}</th><th>${langKey==='de'?'CI Methode':'CI Method'}</th></tr></thead><tbody>`; const fCIH = (m, d=1, p=true) => formatCI(m?.value, m?.ci?.lower, m?.ci?.upper, d, p, '--', langKey); const naH = '--'; sHtml += `<tr><td>${TOOLTIP_CONTENT.statMetrics.sens.name[langKey]||'Sensitivität'}</td><td>${fCIH(statsObj.sens)}</td><td>${statsObj.sens?.method || naH}</td></tr>`; sHtml += `<tr><td>${TOOLTIP_CONTENT.statMetrics.spez.name[langKey]||'Spezifität'}</td><td>${fCIH(statsObj.spez)}</td><td>${statsObj.spez?.method || naH}</td></tr>`; sHtml += `<tr><td>${TOOLTIP_CONTENT.statMetrics.ppv.name[langKey]||'PPV'}</td><td>${fCIH(statsObj.ppv)}</td><td>${statsObj.ppv?.method || naH}</td></tr>`; sHtml += `<tr><td>${TOOLTIP_CONTENT.statMetrics.npv.name[langKey]||'NPV'}</td><td>${fCIH(statsObj.npv)}</td><td>${statsObj.npv?.method || naH}</td></tr>`; sHtml += `<tr><td>${TOOLTIP_CONTENT.statMetrics.acc.name[langKey]||'Accuracy'}</td><td>${fCIH(statsObj.acc)}</td><td>${statsObj.acc?.method || naH}</td></tr>`; sHtml += `<tr><td>${TOOLTIP_CONTENT.statMetrics.balacc.name[langKey]||'Balanced Accuracy'}</td><td>${fCIH(statsObj.balAcc)}</td><td>${statsObj.balAcc?.method || naH}</td></tr>`; sHtml += `<tr><td>${TOOLTIP_CONTENT.statMetrics.f1.name[langKey]||'F1-Score'}</td><td>${fCIH(statsObj.f1, 3, false)}</td><td>${statsObj.f1?.method || naH}</td></tr>`; sHtml += `<tr><td>${TOOLTIP_CONTENT.statMetrics.auc.name[langKey]||'AUC'}</td><td>${fCIH(statsObj.auc, 3, false)}</td><td>${statsObj.auc?.method || naH}</td></tr>`; sHtml += `</tbody></table>`; return sHtml; };
            ...
            if (config.INCLUDE_AS_VS_T2_COMPARISON_TABLE && statsDataForCurrentKollektiv?.vergleichASvsT2_angewandt) { html += `<h2>${langKey==='de'?'Statistischer Vergleich: AS vs. T2':'Statistical Comparison: AS vs. T2'} ('${appliedCriteriaDisplayName}')</h2><table><thead><tr><th>${langKey==='de'?'Test':'Test'}</th><th>${langKey==='de'?'Statistik':'Statistic'}</th><th>p-${langKey==='de'?'Wert':'value'}</th><th>${langKey==='de'?'Methode':'Method'}</th></tr></thead><tbody>`; const v = statsDataForCurrentKollektiv.vergleichASvsT2_angewandt; const fPH = (p) => (p !== null && !isNaN(p) && isFinite(p)) ? (p < 0.001 ? (langKey==='de'?'<0,001':'<.001') : formatNumber(p, 3, '--', false, langKey)) : '--'; const naH = '--'; html += `<tr><td>Accuracy (McNemar)</td><td>${formatNumber(v?.mcnemar?.statistic, 3, naH, false, langKey)} (df=${v?.mcnemar?.df || naH})</td><td>${fPH(v?.mcnemar?.pValue)} ${getStatisticalSignificanceSymbol(v?.mcnemar?.pValue)}</td><td>${v?.mcnemar?.method || naH}</td></tr>`; html += `<tr><td>AUC (DeLong)</td><td>Z=${formatNumber(v?.delong?.Z, 3, naH, false, langKey)}</td><td>${fPH(v?.delong?.pValue)} ${getStatisticalSignificanceSymbol(v?.delong?.pValue)}</td><td>${v?.delong?.method || naH}</td></tr>`; html += `</tbody></table>`; }
            ...
            console.error("Fehler in generateComprehensiveReportHTML:", error);
             return `<html><head><title>${langKey==='de'?'Fehler':'Error'}</title></head><body>${langKey==='de'?'Fehler bei der Reporterstellung.':'Error during report generation.'} Detail: ${error.message}</body></html>`;
        }
    }
    ...
    async function convertTableToPngBlob(tableElementId, baseWidth = 800) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (!table || !(table instanceof HTMLTableElement)) {
                const tableNotFoundMsg = langKey === 'de' ? `Tabelle mit ID '${tableElementId}' nicht gefunden.` : `Table with ID '${tableElementId}' not found.`;
                return reject(new Error(tableNotFoundMsg));
            }
            try {
                ...
                if(tableRect.width <= 0 || tableRect.height <= 0) {
                    ...
                    tableRect.height = table.offsetHeight || (baseWidth * 0.5);
                     if(tableRect.width <= 0 || tableRect.height <= 0) {
                        const invalidDimMsg = langKey === 'de' ? `Tabelle '${tableElementId}' hat keine validen Dimensionen.` : `Table '${tableElementId}' has no valid dimensions.`;
                        return reject(new Error(invalidDimMsg));
                     }
                ...
                const ctx = canvas.getContext('2d');
                if (!ctx) {
                    const canvasErrorMsg = langKey === 'de' ? "Canvas Context nicht verfügbar." : "Canvas context not available.";
                    return reject(new Error(canvasErrorMsg));
                }
                ...
                rows.forEach(row => {
                    ...
                    cells.forEach(cell => {
                        ...
                        const cellBCR = cell.getBoundingClientRect(); // Get dimensions from original or visible table cells
                        const cellWidth = cell.offsetWidth; // Use offsetWidth for actual rendered width
                        const cellHeight = cell.offsetHeight; // Use offsetHeight
                ...
                canvas.toBlob((blob) => {
                    if (blob) { resolve(blob); }
                        ...
                        const blobError = langKey === 'de' ? "Canvas toBlob für Tabelle fehlgeschlagen." : "Canvas toBlob for table failed.";
                        reject(new Error(blobError));
                    }
                }, 'image/png');
    ...
    function exportTableMarkdown(dataOrStats, tableType, kollektiv, criteria = null, logic = null, options = {}) {
        ...
        else if(tableType === 'praes_as_vs_t2_perf' || tableType === 'praes_as_vs_t2_comp') { typeKey = 'PRAES_AS_VS_T2_COMP_MD'; title = langKey==='de'?'AS vs T2 Metriken':'AS vs T2 Metrics'; }
    ...
    async function exportSingleChart(chartElementId, format, kollektiv, options = {}) {
         ...
         } catch (error) { ui_helpers.showToast(`${langKey==='de'?"Fehler beim Chart-Export":"Error during chart export"} (${format.toUpperCase()}). ${error.message}`, 'danger'); }
    }
     ...
     async function exportTablePNG(tableElementId, kollektiv, typeKey = 'TABLE_PNG_EXPORT', tableName = 'Tabelle') {
         ...
         } catch(error) { ui_helpers.showToast(`${langKey==='de'?"Fehler beim Tabellen-PNG-Export für":"Error during table PNG export for"} '${tableName}'. ${error.message}`, 'danger'); }
     }
    ...
    async function exportChartsZip(scopeSelector, zipTypeKey, kollektiv, format) {
         ...
         chartContainers.forEach((svgElement, index) => {
             ...
             if (format === 'png') { filenameKey = 'CHART_SINGLE_PNG'; ext = 'png'; conversionPromise = convertSvgToPngBlob(svgElement).catch(e => { console.error(`PNG conversion failed for ${chartId}:`, e); return null; }); }
             else if (format === 'svg') { filenameKey = 'CHART_SINGLE_SVG'; ext = 'svg'; conversionPromise = convertSvgToSvgBlob(svgElement).catch(e => { console.error(`SVG conversion failed for ${chartId}:`, e); return null; }); }
             else { return; }
             const filename = generateFilename(filenameKey, kollektiv, ext, { chartName });
             promises.push(conversionPromise.then(blob => (blob ? { blob, filename } : { error: new Error(`Blob is null for chart ${chartName}`), filename })));
         });
         ...
         tableContainers.forEach((table, index) => {
              ...
              promises.push(convertTableToPngBlob(tableId, baseWidth).catch(e => { console.error(`Table PNG conversion failed for ${tableId}:`, e); return null; }).then(blob => (blob ? { blob, filename } : { error: new Error(`Table Blob is null for ${tableName}`), filename })));
         });

         try {
             const results = await Promise.all(promises);
             results.forEach(result => { if (result && result.blob) { zip.file(result.filename, result.blob); successCount++; } else if (result && result.error) { console.warn(`Skipping file due to error: ${result.filename} - ${result.error.message}`);} });
             if (successCount > 0) {
         ...
         } catch (error) { ui_helpers.showToast(`${langKey==='de'?'Fehler beim Erstellen des':'Error creating the'} ${format.toUpperCase()} ZIPs. ${error.message}`, 'danger'); }
     }
     ...
     async function exportCategoryZip(category, allRawData, bfResults, currentKollektiv, appliedCriteria, appliedLogic) {
         ...
         const currentPubLang = (typeof state !== 'undefined' && typeof state.getCurrentPublikationLang === 'function') ? state.getCurrentPublikationLang() : 'de';
         ...
         try {
             ...
             if (['all-zip', 'md-zip'].includes(category)) {
                 ...
                 if (typeof publicationTextGenerator !== 'undefined' && statsDataForAllKollektive) {
                     const commonDataForPubGen = {
                        ...
                        bruteForceMetricForPublication: (typeof state !== 'undefined' && state.getCurrentPublikationBruteForceMetric) ? state.getCurrentPublikationBruteForceMetric() : PUBLICATION_CONFIG.defaultBruteForceMetricForPublication,
                        references: { lurzSchaefer2025: "Lurz & Schäfer (2025)", koh2008: "Koh et al. (2008)", barbaro2024: "Barbaro et al. (2024)", rutegard2025: "Rutegård et al. (2025)", beetsTan2018ESGAR: "Beets-Tan et al. (2018)"}
                     };
                     PUBLICATION_CONFIG.sections.forEach(mainSection => {
                         mainSection.subSections.forEach(subSection => {
                            ...
                            const typeKey = subSection.id.startsWith('methoden') ? 'PUBLIKATION_METHODEN_MD' : (subSection.id.startsWith('ergebnisse') ? 'PUBLIKATION_ERGEBNISSE_MD' : `PUBLIKATION_${subSection.id.toUpperCase()}_MD`);
             ...
             const currentKollektivBfResults = bfResults ? bfResults[currentKollektiv] : null;
             if (['all-zip'].includes(category) && currentKollektivBfResults) { addFile(generateFilename('BRUTEFORCE_TXT', currentKollektiv, 'txt'), generateBruteForceTXTString(currentKollektivBfResults, currentKollektiv)); }
             if (['all-zip', 'html'].includes(category) && currentKollektivFilteredData && currentKollektivFilteredData.length > 0 ) { addFile(generateFilename('COMPREHENSIVE_REPORT_HTML', currentKollektiv, 'html'), generateComprehensiveReportHTML(allRawData, currentKollektivBfResults, currentKollektiv, appliedCriteria, appliedLogic)); }

             if (['png-zip', 'all-zip'].includes(category)) { await exportChartsZip('#app-container', 'PNG_ZIP', currentKollektiv, 'png'); return; } // exportChartsZip handles its own toast and download
             if (['svg-zip', 'all-zip' && category === 'svg-zip'].includes(category)) { await exportChartsZip('#app-container', 'SVG_ZIP', currentKollektiv, 'svg'); return; } // exportChartsZip handles its own toast and download
            ...
            } else if (!(['png-zip', 'svg-zip'].includes(category) && category !== 'all-zip') ) { // Don't show if it was just chart export as they handle their own
                ui_helpers.showToast(`${langKey==='de'?"Keine Dateien für das":"No files found or generated for the"} ${category.toUpperCase()} ${langKey==='de'?"ZIP-Paket gefunden oder generiert.":"ZIP package."}`, 'warning');
            }
         } catch (error) { ui_helpers.showToast(`${langKey==='de'?"Fehler beim Erstellen des":"Error creating the"} ${category.toUpperCase()} ${langKey==='de'?"ZIP-Pakets.":"ZIP package."} ${error.message}`, 'danger'); }
     }
     ...
     function exportPraesentationData(actionId, presentationData, kollektiv) {
            ...
            try {
                ...
                ui_helpers.showToast(`${langKey==='de'?"Fehler bei Präsentationsexport":"Error during presentation export"} (${actionId}). ${error.message}`, "danger");
                return;
            }
