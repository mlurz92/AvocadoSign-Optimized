const exportService = (() => {
    ...
    async function convertSvgToPngBlob(svgElement, options = {}) {
        return new Promise((resolve, reject) => {
            ...
            try {
                ...
                svgClone.style.backgroundColor = options.backgroundColor || APP_CONFIG.CHART_SETTINGS.PLOT_BACKGROUND_COLOR || '#ffffff';

                const viewBox = svgElement.viewBox?.baseVal;
                let sourceWidth = parseFloat(svgClone.getAttribute('width')) || svgElement.width?.baseVal?.value || viewBox?.width || APP_CONFIG.CHART_SETTINGS.DEFAULT_WIDTH;
                let sourceHeight = parseFloat(svgClone.getAttribute('height')) || svgElement.height?.baseVal?.value || viewBox?.height || APP_CONFIG.CHART_SETTINGS.DEFAULT_HEIGHT;

                if (sourceWidth <= 0 || sourceHeight <= 0) { reject(new Error("SVG-Dimensionen konnten nicht ermittelt werden.")); return;}

                const dpi = options.dpi || APP_CONFIG.DEFAULT_SETTINGS.PNG_EXPORT_DPI;
                const scaleFactor = dpi / 96;
                const targetWidth = options.targetWidth ? options.targetWidth * scaleFactor : sourceWidth * scaleFactor;
                const targetHeight = sourceHeight * (targetWidth / sourceWidth);


                svgClone.setAttribute('width', String(targetWidth / scaleFactor));
                svgClone.setAttribute('height', String(targetHeight / scaleFactor));
                ...
                img.onload = () => {
                    ...
                    ctx.fillStyle = options.backgroundColor || APP_CONFIG.CHART_SETTINGS.PLOT_BACKGROUND_COLOR || '#ffffff';
    ...
    function generateStatistikCSVString(stats, kollektiv, criteria, logic) {
        ...
        const fCI_val = (val, ci, d, isP) => !ci || typeof ci !== 'object' || ci.lower === null || ci.upper === null || isNaN(ci.lower) || isNaN(ci.upper) ? [fv(val,d,true), na, na] : [fv(val, d, true), fv(ci.lower, d, true), fv(ci.upper, d, true)];
        ...
        try {
            ...
            csvData.push(['Geschlecht Männlich (n)', `${d.geschlecht?.m ?? 0}`]); csvData.push(['Geschlecht Männlich (%)', `${fp(d.anzahlPatienten > 0 ? (d.geschlecht?.m ?? 0) / d.anzahlPatienten : NaN, 1)}`]); csvData.push(['Geschlecht Weiblich (n)', `${d.geschlecht?.f ?? 0}`]); csvData.push(['Geschlecht Weiblich (%)', `${fp(d.anzahlPatienten > 0 ? (d.geschlecht?.f ?? 0) / d.anzahlPatienten : NaN, 1)}`]); csvData.push(['Therapie direkt OP (n)', `${d.therapie?.['direkt OP'] ?? 0}`]); csvData.push(['Therapie direkt OP (%)', `${fp(d.anzahlPatienten > 0 ? (d.therapie?.['direkt OP'] ?? 0) / d.anzahlPatienten : NaN, 1)}`]); csvData.push(['Therapie nRCT (n)', `${d.therapie?.nRCT ?? 0}`]); csvData.push(['Therapie nRCT (%)', `${fp(d.anzahlPatienten > 0 ? (d.therapie?.nRCT ?? 0) / d.anzahlPatienten : NaN, 1)}`]); csvData.push(['N Status (+)', `${d.nStatus?.plus ?? 0}`]); csvData.push(['N Status (+ / %)', `${fp(d.anzahlPatienten > 0 ? (d.nStatus?.plus ?? 0) / d.anzahlPatienten : NaN, 1)}`]); csvData.push(['AS Status (+)', `${d.asStatus?.plus ?? 0}`]); csvData.push(['AS Status (+ / %)', `${fp(d.anzahlPatienten > 0 ? (d.asStatus?.plus ?? 0) / d.anzahlPatienten : NaN, 1)}`]); csvData.push(['T2 Status (+)', `${d.t2Status?.plus ?? 0}`]); csvData.push(['T2 Status (+ / %)', `${fp(d.anzahlPatienten > 0 ? (d.t2Status?.plus ?? 0) / d.anzahlPatienten : NaN, 1)}`]); const fLKRow = (lk) => [fv(lk?.median, 1, true), fv(lk?.mean, 1, true), fv(lk?.sd, 1, true), fv(lk?.min, 0, true), fv(lk?.max, 0, true)]; csvData.push(['LK N gesamt (Median)', ...fLKRow(d.lkAnzahlen?.n?.total)]); csvData.push(['LK N+ (Median, nur N+ Pat.)', ...fLKRow(d.lkAnzahlen?.n?.plus)]); csvData.push(['LK AS gesamt (Median)', ...fLKRow(d.lkAnzahlen?.as?.total)]); csvData.push(['LK AS+ (Median, nur AS+ Pat.)', ...fLKRow(d.lkAnzahlen?.as?.plus)]); csvData.push(['LK T2 gesamt (Median)', ...fLKRow(d.lkAnzahlen?.t2?.total)]); csvData.push(['LK T2+ (Median, nur T2+ Pat.)', ...fLKRow(d.lkAnzahlen?.t2?.plus)]); csvData.push([]);
            csvData.push(['Metrik (Diagnostik)', 'Methode', 'Wert', '95% CI Lower', '95% CI Upper', 'SE (Bootstrap)', 'CI Methode', 'LR+ Wert', 'LR+ CI Lower', 'LR+ CI Upper', 'LR- Wert', 'LR- CI Lower', 'LR- CI Upper']); const addPerfRow = (metricKey, metricName, objAS, objT2) => { const isRate = !(metricKey === 'auc' || metricKey === 'f1'); const digits = isRate ? 1 : 3; const ciAS = fCI_val(objAS?.[metricKey]?.value, objAS?.[metricKey]?.ci, digits, isRate); const ciT2 = fCI_val(objT2?.[metricKey]?.value, objT2?.[metricKey]?.ci, digits, isRate); const lrPCI_AS = fCI_val(objAS?.lrPlus?.value, objAS?.lrPlus?.ci, 2, false); const lrNCI_AS = fCI_val(objAS?.lrMinus?.value, objAS?.lrMinus?.ci, 2, false); const lrPCI_T2 = fCI_val(objT2?.lrPlus?.value, objT2?.lrPlus?.ci, 2, false); const lrNCI_T2 = fCI_val(objT2?.lrMinus?.value, objT2?.lrMinus?.ci, 2, false); csvData.push([metricName, 'AS', ciAS[0], ciAS[1], ciAS[2], fv(objAS?.[metricKey]?.se, 4, true), objAS?.[metricKey]?.method || na, metricKey === 'sens' ? lrPCI_AS[0] : na, metricKey === 'sens' ? lrPCI_AS[1] : na, metricKey === 'sens' ? lrPCI_AS[2] : na, metricKey === 'sens' ? lrNCI_AS[0] : na, metricKey === 'sens' ? lrNCI_AS[1] : na, metricKey === 'sens' ? lrNCI_AS[2] : na ]); csvData.push([metricName, 'T2', ciT2[0], ciT2[1], ciT2[2], fv(objT2?.[metricKey]?.se, 4, true), objT2?.[metricKey]?.method || na, metricKey === 'sens' ? lrPCI_T2[0] : na, metricKey === 'sens' ? lrPCI_T2[1] : na, metricKey === 'sens' ? lrPCI_T2[2] : na, metricKey === 'sens' ? lrNCI_T2[0] : na, metricKey === 'sens' ? lrNCI_T2[1] : na, metricKey === 'sens' ? lrNCI_T2[2] : na ]); }; const gAS = stats.gueteAS, gT2 = stats.gueteT2_angewandt; ['sens', 'spez', 'ppv', 'npv', 'acc', 'balAcc', 'f1', 'auc'].forEach(k => addPerfRow(k, k.charAt(0).toUpperCase()+k.slice(1), gAS, gT2)); csvData.push([]);
    ...
    function generateBruteForceTXTString(resultsData) {
        ...
        try {
            ...
            report += ` (Sens: ${formatPercent(bestResult.sens,1)}, Spez: ${formatPercent(bestResult.spez,1)}, PPV: ${formatPercent(bestResult.ppv,1)}, NPV: ${formatPercent(bestResult.npv,1)}, Acc: ${formatPercent(bestResult.acc,1)}, BalAcc: ${formatNumber(bestResult.balAcc,3)}, F1: ${formatNumber(bestResult.f1,3)})\r\n`;
            report += ` (RP: ${bestResult.rp}, FP: ${bestResult.fp}, FN: ${bestResult.fn}, RN: ${bestResult.rn})\r\n\r\n`;

            report += `--- Top ${APP_CONFIG.PERFORMANCE_SETTINGS.MAX_BRUTE_FORCE_REPORTS_IN_MODAL} Ergebnisse (inklusive identischer Werte) ---\r\n`;
            ...
            for (let i = 0; i < results.length; i++) {
                ...
                if (rank > APP_CONFIG.PERFORMANCE_SETTINGS.MAX_BRUTE_FORCE_REPORTS_IN_MODAL && isNewRank) break;
                report += `${String(currentRank).padEnd(4)} | ${formatNumber(result.metricValue, 4, true).padEnd(12)} | ${formatPercent(result.sens,1).padEnd(6)} | ${formatPercent(result.spez,1).padEnd(6)} | ${formatPercent(result.ppv,1).padEnd(6)} | ${formatPercent(result.npv,1).padEnd(6)} | ${formatPercent(result.acc,1).padEnd(6)} | ${formatNumber(result.balAcc,3).padEnd(6)} | ${formatNumber(result.f1,3).padEnd(6)} | ${result.logic.toUpperCase().padEnd(5)} | ${formatCriteriaFunc(result.criteria, result.logic)}\r\n`;
                if (isNewRank || i === 0) { lastMetricValue = result.metricValue; }
                displayedCount++;
                 if (displayedCount >= APP_CONFIG.PERFORMANCE_SETTINGS.MAX_BRUTE_FORCE_REPORTS_IN_MODAL && rank >= APP_CONFIG.PERFORMANCE_SETTINGS.MAX_BRUTE_FORCE_REPORTS_IN_MODAL && isNewRank && i < results.length -1) {
                    report += `... (weitere Ergebnisse mit Rang > ${APP_CONFIG.PERFORMANCE_SETTINGS.MAX_BRUTE_FORCE_REPORTS_IN_MODAL} nicht im Detail aufgeführt) ...\r\n`;
                    break;
                 }
            }
            report += `==================================================\r\n`;
    ...
    function generateBruteForceFullCSVString(resultsData) {
        if (!resultsData || !resultsData.allResults || resultsData.allResults.length === 0) return null;
        try {
            const { allResults, metric } = resultsData;
            const formatCriteriaFunc = typeof studyT2CriteriaManager !== 'undefined' ? studyT2CriteriaManager.formatCriteriaForDisplay : (c, l) => 'Formatierungsfehler';
            const headers = ['TargetMetric', 'MetricValue', 'Sens', 'Spez', 'PPV', 'NPV', 'Acc', 'BalAcc', 'F1', 'RP', 'FP', 'FN', 'RN', 'Logik', 'KriterienString', 'KriterienJSON'];
            const rows = allResults.map(r => [
                metric,
                formatNumber(r.metricValue, 6, 'N/A', true),
                formatNumber(r.sens, 4, 'N/A', true),
                formatNumber(r.spez, 4, 'N/A', true),
                formatNumber(r.ppv, 4, 'N/A', true),
                formatNumber(r.npv, 4, 'N/A', true),
                formatNumber(r.acc, 4, 'N/A', true),
                formatNumber(r.balAcc, 4, 'N/A', true),
                formatNumber(r.f1, 4, 'N/A', true),
                r.rp, r.fp, r.fn, r.rn,
                r.logic,
                formatCriteriaFunc(r.criteria, r.logic),
                JSON.stringify(r.criteria)
            ]);
            return Papa.unparse([headers, ...rows], { delimiter: APP_CONFIG.EXPORT_SETTINGS.CSV_DELIMITER || ";" });
        } catch (error) {
            return null;
        }
    }
    ...
    function generateComprehensiveReportHTML(data, bfResultsForCurrentKollektiv, kollektiv, criteria, logic) {
        try {
            ...
            const addPerfSectionHTML = (title, statsObj) => { if (!statsObj) return ''; let sHtml = `<h2>${title}</h2><table><thead><tr><th>Metrik</th><th>Wert (95% CI)</th><th>CI Methode</th><th>LR+ (95% CI)</th><th>LR- (95% CI)</th></tr></thead><tbody>`; const fCI_local = (m, d=1, p=true) => formatCI(m?.value, m?.ci?.lower, m?.ci?.upper, d, p, '--'); const na = '--'; const lrP_CI = formatCI(statsObj.lrPlus?.value, statsObj.lrPlus?.ci?.lower, statsObj.lrPlus?.ci?.upper, 2, false, na); const lrM_CI = formatCI(statsObj.lrMinus?.value, statsObj.lrMinus?.ci?.lower, statsObj.lrMinus?.ci?.upper, 2, false, na); sHtml += `<tr><td>Sensitivität</td><td>${fCI_local(statsObj.sens)}</td><td>${statsObj.sens?.method || na}</td><td>${lrP_CI}</td><td>${lrM_CI}</td></tr>`; sHtml += `<tr><td>Spezifität</td><td>${fCI_local(statsObj.spez)}</td><td>${statsObj.spez?.method || na}</td><td></td><td></td></tr>`; sHtml += `<tr><td>PPV</td><td>${fCI_local(statsObj.ppv)}</td><td>${statsObj.ppv?.method || na}</td><td></td><td></td></tr>`; sHtml += `<tr><td>NPV</td><td>${fCI_local(statsObj.npv)}</td><td>${statsObj.npv?.method || na}</td><td></td><td></td></tr>`; sHtml += `<tr><td>Accuracy</td><td>${fCI_local(statsObj.acc)}</td><td>${statsObj.acc?.method || na}</td><td></td><td></td></tr>`; sHtml += `<tr><td>Balanced Accuracy</td><td>${fCI_local(statsObj.balAcc)}</td><td>${statsObj.balAcc?.method || na}</td><td></td><td></td></tr>`; sHtml += `<tr><td>F1-Score</td><td>${fCI_local(statsObj.f1, 3, false)}</td><td>${statsObj.f1?.method || na}</td><td></td><td></td></tr>`; sHtml += `<tr><td>AUC</td><td>${fCI_local(statsObj.auc, 3, false)}</td><td>${statsObj.auc?.method || na}</td><td></td><td></td></tr>`; sHtml += `</tbody></table>`; return sHtml; };
    ...
    async function convertTableToPngBlob(tableElementId, baseWidth = 800) {
        ...
        return new Promise((resolve, reject) => {
            ...
            try {
                ...
                tableWrapper.style.width = `${tableRect.width}px`;
    ...
    function exportBruteForceReport(resultsData, exportAllCombinations = false) {
        if (!resultsData || (!resultsData.results && !resultsData.allResults) || (resultsData.results && resultsData.results.length === 0 && !exportAllCombinations) || (resultsData.allResults && resultsData.allResults.length === 0 && exportAllCombinations)) {
             ui_helpers.showToast("Keine Brute-Force Ergebnisse zum Exportieren vorhanden.", "warning");
             return;
        }
        let contentString = null;
        let fileTypeKey = 'BRUTEFORCE_TXT';
        let extension = 'txt';
        let mimeType = 'text/plain;charset=utf-8;';

        try {
            if (exportAllCombinations) {
                contentString = generateBruteForceFullCSVString(resultsData);
                fileTypeKey = 'BRUTEFORCE_FULL_CSV';
                extension = 'csv';
                mimeType = 'text/csv;charset=utf-8;';
            } else {
                contentString = generateBruteForceTXTString(resultsData);
            }
        } catch(e) {
            ui_helpers.showToast(`Fehler bei ${extension.toUpperCase()}-Erstellung.`, "danger");
            return;
        }
        if (contentString === null || contentString.length === 0) {
            ui_helpers.showToast(`${extension.toUpperCase()}-Generierung ergab leere Datei.`, "warning");
            return;
        }
        const filename = generateFilename(fileTypeKey, resultsData.kollektiv, extension);
        if(downloadFile(contentString, filename, mimeType)) {
            ui_helpers.showToast(`Brute-Force ${exportAllCombinations ? 'Rohdaten (CSV)' : 'Bericht (TXT)'} exportiert: ${filename}`, 'success');
        }
    }
    ...
    function exportTableMarkdown(dataOrStats, tableType, kollektiv, criteria = null, logic = null, options = {}) {
        ...
        else if(tableType === 'publikation_methoden' || tableType === 'publikation_ergebnisse') {
            typeKey = 'PUBLIKATION_SECTION_MD';
            title = `Publikation ${options.sectionName || tableType}`;
        }
    ...
    async function exportSingleChart(chartElementId, format, kollektiv, options = {}) {
         ...
         try {
             ...
             const exportOptions = {
                dpi: options.dpi || APP_CONFIG.DEFAULT_SETTINGS.PNG_EXPORT_DPI,
                embedFonts: options.svgEmbedFonts ?? APP_CONFIG.DEFAULT_SETTINGS.SVG_EXPORT_EMBED_FONTS,
                inlineStyles: options.svgInlineStyles ?? APP_CONFIG.DEFAULT_SETTINGS.SVG_EXPORT_INLINE_STYLES,
                backgroundColor: APP_CONFIG.CHART_SETTINGS.PLOT_BACKGROUND_COLOR,
                targetWidth: options.targetWidth // For specific sizing if needed
             };

             if (format === 'png') { ui_helpers.showToast(`Generiere PNG für Chart ${chartName}...`, 'info', 1500); blob = await convertSvgToPngBlob(svgElement, exportOptions); filenameKey = 'CHART_SINGLE_PNG'; mimeType = 'image/png'; ext = 'png'; }
             else if (format === 'svg') { ui_helpers.showToast(`Generiere SVG für Chart ${chartName}...`, 'info', 1500); blob = await convertSvgToSvgBlob(svgElement, exportOptions); filenameKey = 'CHART_SINGLE_SVG'; mimeType = 'image/svg+xml;charset=utf-8'; ext = 'svg'; }
         ...
         } catch (error) { ui_helpers.showToast(`Fehler beim Chart-Export (${format.toUpperCase()}).`, 'danger'); }
    }
     ...
     async function exportTablePNG(tableElementId, kollektiv, typeKey, tableName = 'Tabelle') {
         ...
         } catch(error) { ui_helpers.showToast(`Fehler beim Tabellen-PNG-Export für '${tableName}'.`, 'danger'); }
     }
    ...
    async function exportChartsZip(scopeSelector, zipTypeKey, kollektiv, format) {
         ...
         const exportOptions = {
            dpi: state.getCurrentPngExportDpi ? state.getCurrentPngExportDpi() : APP_CONFIG.DEFAULT_SETTINGS.PNG_EXPORT_DPI,
            svgEmbedFonts: state.getCurrentSvgEmbedFonts ? state.getCurrentSvgEmbedFonts() : APP_CONFIG.DEFAULT_SETTINGS.SVG_EXPORT_EMBED_FONTS,
            svgInlineStyles: state.getCurrentSvgInlineStyles ? state.getCurrentSvgInlineStyles() : APP_CONFIG.DEFAULT_SETTINGS.SVG_EXPORT_INLINE_STYLES
         };
         ...
         chartContainers.forEach((svgElement, index) => {
             ...
             if (format === 'png') { filenameKey = 'CHART_SINGLE_PNG'; ext = 'png'; conversionPromise = convertSvgToPngBlob(svgElement, {dpi: exportOptions.dpi, inlineStyles: exportOptions.svgInlineStyles}).catch(e => { return null; }); }
             else if (format === 'svg') { filenameKey = 'CHART_SINGLE_SVG'; ext = 'svg'; conversionPromise = convertSvgToSvgBlob(svgElement, {embedFonts: exportOptions.svgEmbedFonts, inlineStyles: exportOptions.svgInlineStyles}).catch(e => { return null; }); }
             else { return; }
         ...
         try {
             ...
             results.forEach(result => { if (result && result.blob) { zip.file(result.filename, result.blob); successCount++; } });
             if (successCount > 0) {
         ...
         } catch (error) { ui_helpers.showToast(`Fehler beim Erstellen des ${format.toUpperCase()} ZIPs.`, 'danger'); }
     }

    async function exportPublicationChartsZip(kollektiv) {
        ui_helpers.showToast(`Starte Export der Publikations-Diagramme...`, 'info', 2000);
        if (!window.JSZip) { ui_helpers.showToast("JSZip Bibliothek nicht geladen.", "danger"); return; }
        const zip = new JSZip(); let filesAdded = 0;
        const chartElementsConfig = [
            APP_CONFIG.PUBLICATION_CONFIG.publicationElements.ergebnisse.alterVerteilungChart,
            APP_CONFIG.PUBLICATION_CONFIG.publicationElements.ergebnisse.geschlechtVerteilungChart,
            APP_CONFIG.PUBLICATION_CONFIG.publicationElements.ergebnisse.vergleichPerformanceChartGesamt,
            APP_CONFIG.PUBLICATION_CONFIG.publicationElements.ergebnisse.vergleichPerformanceChartDirektOP,
            APP_CONFIG.PUBLICATION_CONFIG.publicationElements.ergebnisse.vergleichPerformanceChartNRCT
        ];
        ...
        for (const chartConfig of chartElementsConfig) {
            const svgElement = document.getElementById(chartConfig.id)?.querySelector('svg');
            if (svgElement) {
                const chartName = (chartConfig.titleDe || chartConfig.id).replace(/[^a-zA-Z0-9_-]/gi, '_').substring(0, 40);
                try {
                    const pngBlob = await convertSvgToPngBlob(svgElement, exportOptions);
                    if (pngBlob) {
                        zip.file(generateFilename('CHART_SINGLE_PNG', kollektiv, 'png', { chartName: `Pub_${chartName}` }), pngBlob);
                        filesAdded++;
                    }
                    const svgBlob = await convertSvgToSvgBlob(svgElement, exportOptions);
                    if (svgBlob) {
                        zip.file(generateFilename('CHART_SINGLE_SVG', kollektiv, 'svg', { chartName: `Pub_${chartName}` }), svgBlob);
                        filesAdded++;
                    }
                } catch(e) {
                     console.warn(`Fehler beim Export von Publikations-Chart ${chartName}: ${e.message}`);
                }
            }
        }

        if (filesAdded > 0) {
            const zipFilename = generateFilename('PUBLICATION_CHARTS_ZIP', kollektiv, 'zip');
            const content = await zip.generateAsync({ type: "blob", compression: "DEFLATE", compressionOptions: { level: 6 } });
            if (downloadFile(content, zipFilename, "application/zip")) ui_helpers.showToast(`${filesAdded / 2} Publikations-Diagramm(e) (PNG+SVG) erfolgreich exportiert.`, 'success');
        } else {
            ui_helpers.showToast(`Keine Publikations-Diagramme gefunden oder Export fehlgeschlagen.`, 'warning');
        }
    }
     ...
     async function exportCategoryZip(category, data, bfResults, kollektiv, criteria, logic) {
         ...
         const needsStats = ['all-zip', 'csv-zip', 'md-zip', 'html', 'publication-charts-zip'].includes(category);
         if(needsStats && data && data.length > 0 && criteria && logic) {
             ...
             if (!statsDataForAllKollektive || !statsDataForAllKollektive[kollektiv]) {
                 ...
                 if (['csv-zip', 'html'].includes(category)) return;
             }
         }
         ...
         try {
             if (['all-zip', 'csv-zip'].includes(category)) {
                 ...
                 if (currentKollektivBfResult && currentKollektivBfResult.allResults) addFile(generateFilename('BRUTEFORCE_FULL_CSV', kollektiv, 'csv'), generateBruteForceFullCSVString(currentKollektivBfResult));
             }
                 ...
                 if (typeof publicationTextGenerator !== 'undefined' && statsDataForAllKollektive && state) {
                     PUBLICATION_CONFIG.sections.forEach(mainSection => {
                         mainSection.subSections.forEach(subSection => {
                            ...
                            addFile(generateFilename('PUBLIKATION_SECTION_MD', kollektiv, 'md', {sectionName: subSection.id.replace(/^(methoden_|ergebnisse_)/, '')}), mdContent);
                         });
                     });
                 }
             }
             ...
             if (category === 'publication-charts-zip') { await exportPublicationChartsZip(kollektiv); return; }
         ...
         } catch (error) { ui_helpers.showToast(`Fehler beim Erstellen des ${category.toUpperCase()} ZIP-Pakets.`, 'danger'); }
     }
     ...
     function exportPraesentationData(actionId, presentationData, kollektiv) {
            ...
            const { statsAS, statsT2, vergleich, comparisonCriteriaSet, patientCount, statsGesamt, statsDirektOP, statsNRCT, kollektivForComparison, patientCountForComparison } = presentationData;
            ...
            const effectiveKollektiv = isAsPurView ? kollektiv : (kollektivForComparison || kollektiv);


            try {
                if (isAsPurView && actionId === 'download-performance-as-pur-csv') {
                     const allStatsData = { Gesamt: statsGesamt, 'direkt OP': statsDirektOP, nRCT: statsNRCT }; const headers = ['Kollektiv', 'N', 'Sens', 'Sens CI Low', 'Sens CI High', 'Spez', 'Spez CI Low', 'Spez CI High', 'PPV', 'PPV CI Low', 'PPV CI High', 'NPV', 'NPV CI Low', 'NPV CI High', 'Acc', 'Acc CI Low', 'Acc CI High', 'BalAcc', 'BalAcc CI Low', 'BalAcc CI High', 'F1', 'F1 CI Low', 'F1 CI High', 'AUC', 'AUC CI Low', 'AUC CI High', 'CI Method Sens', 'CI Method Spez', 'CI Method PPV', 'CI Method NPV', 'CI Method Acc', 'CI Method BalAcc/AUC/F1']; const fVal = (v, d=1, useStd = false) => formatNumber(v, d, na, useStd);
                     const rows = Object.entries(allStatsData).map(([key, stats]) => { let dN = key; if (!stats || typeof stats.matrix !== 'object') return [getKollektivDisplayName(dN), 0, ...Array(24).fill(na), na]; const n = stats.matrix ? (stats.matrix.rp + stats.matrix.fp + stats.matrix.fn + stats.matrix.rn) : 0; const fRowData = (m, metric_k) => { const dig = (metric_k === 'f1' || metric_k === 'auc' || metric_k === 'balAcc') ? 3 : 1; return [fVal(m?.value, dig, true), fVal(m?.ci?.lower, dig, true), fVal(m?.ci?.upper, dig, true)]; }; return [ getKollektivDisplayName(dN), n, ...fRowData(stats.sens, 'sens'), ...fRowData(stats.spez, 'spez'), ...fRowData(stats.ppv, 'ppv'), ...fRowData(stats.npv, 'npv'), ...fRowData(stats.acc, 'acc'), ...fRowData(stats.balAcc, 'balAcc'), ...fRowData(stats.f1, 'f1'), ...fRowData(stats.auc, 'auc'), stats.sens?.method || na, stats.spez?.method || na, stats.ppv?.method || na, stats.npv?.method || na, stats.acc?.method || na, stats.balAcc?.method || na ]; });
                ...
                } else if (isAsPurView && actionId === 'download-performance-as-pur-md') { options.kollektiv = effectiveKollektiv; content = generateMarkdownTableString(presentationData, 'praes_as_perf', effectiveKollektiv, null, null, options); filenameKey = 'PRAES_AS_PERF_MD'; extension = 'md'; mimeType = 'text/markdown;charset=utf-8;';
                } else if (isAsVsT2View && actionId === 'download-performance-as-vs-t2-csv') { if (!statsAS || !statsT2) { ui_helpers.showToast("Vergleichsdaten für CSV fehlen.", "warning"); return; } const headers = ['Metrik', 'AS (Wert)', 'AS (95% CI Low)', 'AS (95% CI High)', 'AS (CI Methode)', `${options.t2CriteriaLabelShort || 'T2'} (Wert)`, `${options.t2CriteriaLabelShort || 'T2'} (95% CI Low)`, `${options.t2CriteriaLabelShort || 'T2'} (95% CI High)`, `${options.t2CriteriaLabelShort || 'T2'} (CI Methode)`]; const fRow = (mKey, nm, isP = true, d = 1) => { const mAS = statsAS[mKey]; const mT2 = statsT2[mKey]; const dig = (mKey === 'auc' || mKey === 'f1' || mKey === 'balAcc') ? 3 : d; const valAS = formatNumber(mAS?.value, dig, na, true); const ciLowAS = formatNumber(mAS?.ci?.lower, dig, na, true); const ciHighAS = formatNumber(mAS?.ci?.upper, dig, na, true); const valT2 = formatNumber(mT2?.value, dig, na, true); const ciLowT2 = formatNumber(mT2?.ci?.lower, dig, na, true); const ciHighT2 = formatNumber(mT2?.ci?.upper, dig, na, true); return [nm, valAS, ciLowAS, ciHighAS, mAS?.method || na, valT2, ciLowT2, ciHighT2, mT2?.method || na]; }; const rows = [ fRow('sens', 'Sensitivität'), fRow('spez', 'Spezifität'), fRow('ppv', 'PPV'), fRow('npv', 'NPV'), fRow('acc', 'Accuracy'), fRow('balAcc', 'Balanced Accuracy'), fRow('f1', 'F1-Score', false, 3), fRow('auc', 'AUC', false, 3) ]; content = Papa.unparse([headers, ...rows], { delimiter: APP_CONFIG.EXPORT_SETTINGS.CSV_DELIMITER || ";" }); filenameKey = 'PRAES_AS_VS_T2_PERF_CSV'; extension = 'csv'; mimeType = 'text/csv;charset=utf-8;';
                } else if (isAsVsT2View && actionId === 'download-comp-table-as-vs-t2-md') { options.kollektiv = effectiveKollektiv; content = generateMarkdownTableString(presentationData, 'praes_as_vs_t2_comp', effectiveKollektiv, null, null, options); filenameKey = 'PRAES_AS_VS_T2_COMP_MD'; extension = 'md'; mimeType = 'text/markdown;charset=utf-8;';
                } else if (isAsVsT2View && actionId === 'download-tests-as-vs-t2-md') { options.kollektiv = effectiveKollektiv; content = generateMarkdownTableString(presentationData, 'praes_as_vs_t2_tests', effectiveKollektiv, null, null, options); filenameKey = 'PRAES_AS_VS_T2_TESTS_MD'; extension = 'md'; mimeType = 'text/markdown;charset=utf-8;';
                }
            } catch(error) {
            ...
            if(content !== null && filenameKey && extension && mimeType) { const filename = generateFilename(filenameKey, effectiveKollektiv, extension, options); success = downloadFile(content, filename, mimeType); if(success) ui_helpers.showToast(`Präsentationsdaten (${extension}) exportiert: ${filename}`, 'success'); }
