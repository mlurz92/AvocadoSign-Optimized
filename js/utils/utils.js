const utils = (() => {
    ...
    function getKollektivDisplayName(kollektivId) {
        if (!kollektivId) return 'Unbekannt';
        return UI_TEXTS?.kollektivDisplayNames?.[kollektivId] || kollektivId;
    }
    ...
    function formatCI(value, ciLower, ciUpper, digits = 1, isPercent = false, placeholder = '--') {
        ...
        if (formattedLower !== null && formattedUpper !== null) {
            ...
            const ciStr = `(95%-KI: ${lowerStr}\u00A0–\u00A0${upperStr})`;
    ...
    function getPValueText(pValue, lang = 'de', forPublication = false) {
        ...
        let pFormatted;
        if (forPublication) {
            if (pValue < 0.01) pFormatted = formatNumber(pValue, 3, 'N/A', true).substring(1);
            else pFormatted = formatNumber(pValue, 2, 'N/A', true).substring(1);
            if (pFormatted === '.00') pFormatted = '.01';
            return `P = ${pFormatted}`;
        } else {
            pFormatted = formatNumber(pValue, 3, 'N/A', true).replace('.',',');
            return `p = ${pFormatted}`;
        }
    }
