...
const UI_TEXTS = {
    ...
    chartTitles: {
        ...
        statusT2: 'T2-Status (Angew. Kriterien)',
        comparisonBar: 'Vergleichsmetriken: AS vs. {T2Name}',
        rocCurve: 'ROC-Kurve: {Method}',
        asPerformance: 'Performance: Avocado Sign (Kollektiv: {KollektivName})',
    ...
    legendLabels: {
        ...
        benignLN: 'Benigne Lymphknoten',
        malignantLN: 'Maligne Lymphknoten'
    },
    criteriaComparison: {
    ...
    statMetrics: {
        ...
        assoziationStaerkeTexte: {
            ...
            moderat: "moderat",
            schwach: "schwach",
            sehr_schwach: "sehr schwach oder nicht vorhanden",
...
const TOOLTIP_CONTENT = {
    ...
    datenTable: {
        ...
        therapie: "Angewandte Therapie vor der Operation (nRCT: neoadjuvante Radiochemotherapie, direkt OP: keine Vorbehandlung, unbekannt: nicht spezifiziert).",
    ...
    deskriptiveStatistik: {
        ...
        chartGender: { description: "Tortendiagramm der Geschlechterverteilung (m/w/unbekannt) im Kollektiv [KOLLEKTIV]." }
    },
    ...
    assoziationEinzelkriterien: { cardTitle: "Analyse der Assoziation zwischen dem AS-Status bzw. einzelnen T2-Merkmalen (basierend auf den aktuell im Reiter 'Auswertung' definierten Merkmalsausprägungen, auch wenn das Merkmal dort nicht global aktiviert ist) und dem histopathologischen N-Status (+/-) im Kollektiv [KOLLEKTIV]. Angegeben sind Odds Ratio (OR), Risk Difference (RD), Phi-Koeffizient und p-Werte aus geeigneten Tests." },
    ...
    publikationTabTooltips: {
        ...
        methoden_as_definition: {description: "Textvorschlag und relevante Informationen zur Definition und Bewertung des Avocado Signs."},
        methoden_t2_definition: {description: "Textvorschlag und relevante Informationen zur Definition und Bewertung der T2-Kriterien (benutzerdefiniert, Literatur, Brute-Force optimiert)."},
        methoden_referenzstandard: {description: "Textvorschlag und relevante Informationen zum Referenzstandard (Histopathologie)."},
        methoden_statistische_analyse: {description: "Textvorschlag und relevante Informationen zu den statistischen Analysemethoden."},
    ...
    statMetrics: {
        ...
        balAcc: { name: "Balanced Accuracy", description: "Balanced Accuracy ([METHODE] vs. N): Der Mittelwert aus Sensitivität und Spezifität. Geeignet bei unbalancierten Stichproben.<br><i>Formel: (Sensitivität + Spezifität) / 2</i>", interpretation: "Die Balanced Accuracy der Methode [METHODE], die Sensitivität und Spezifität gleich gewichtet, betrug <strong>[WERT]</strong> (95% CI nach [METHOD_CI]: [LOWER] – [UPPER]) im Kollektiv [KOLLEKTIV]."},
        f1: { name: "F1-Score", description: "F1-Score ([METHODE] vs. N): Das harmonische Mittel aus PPV (Precision) und Sensitivität (Recall). Sinnvoll, wenn sowohl falsch positive als auch falsch negative Klassifikationen minimiert werden sollen.<br><i>Formel: 2 * (PPV * Sensitivität) / (PPV + Sensitivität)</i>", interpretation: "Der F1-Score für die Methode [METHODE], der Präzision und Sensitivität kombiniert, beträgt <strong>[WERT]</strong> (95% CI nach [METHOD_CI]: [LOWER] – [UPPER]) im Kollektiv [KOLLEKTIV]."},
        ...
        mcnemar: { name: "McNemar-Test", description: "Prüft auf einen signifikanten Unterschied in den diskordanten Paaren (Fälle, bei denen AS und [T2_SHORT_NAME] unterschiedliche Ergebnisse liefern) bei gepaarten Daten. Geeignet zum Vergleich der Accuracy zweier diagnostischer Tests am selben Kollektiv.<br><i>Nullhypothese: Die Anzahl der Fälle, in denen Test A richtig und Test B falsch liegt, ist gleich der Anzahl der Fälle, in denen Test A falsch und Test B richtig liegt.</i>", interpretation: "Der McNemar-Test ergab einen p-Wert von <strong>[P_WERT] ([SIGNIFIKANZ])</strong>. Dies deutet darauf hin, dass sich die Fehlklassifizierungsraten (bezogen auf Accuracy) von AS und [T2_SHORT_NAME] im Kollektiv [KOLLEKTIV] [SIGNIFIKANZ_TEXT] unterscheiden."},
        delong: { name: "DeLong-Test", description: "Vergleicht zwei AUC-Werte von ROC-Kurven, die auf denselben (gepaarten) Daten basieren, unter Berücksichtigung der Kovarianz. Geeignet zum Vergleich der Gesamt-Diskriminationsfähigkeit zweier diagnostischer Tests.<br><i>Nullhypothese: AUC(AS) = AUC([T2_SHORT_NAME])</i>", interpretation: "Der DeLong-Test ergab einen p-Wert von <strong>[P_WERT] ([SIGNIFIKANZ])</strong>. Dies deutet darauf hin, dass sich die AUC-Werte (bzw. Balanced Accuracies als Maß der Gesamt-Diskriminationsfähigkeit) von AS und [T2_SHORT_NAME] im Kollektiv [KOLLEKTIV] [SIGNIFIKANZ_TEXT] unterscheiden."},
        phi: { name: "Phi-Koeffizient (φ)", description: "Maß für die Stärke und Richtung des Zusammenhangs zwischen zwei binären Variablen (z.B. Vorhandensein von Merkmal [MERKMAL] und N-Status). Wertebereich von -1 (perfekt negativ) bis +1 (perfekt positiv), 0 bedeutet kein Zusammenhang.<br><i>Berechnet als: (RP*RN - FP*FN) / sqrt((RP+FP)*(FN+RN)*(RP+FN)*(FP+RN))</i>", interpretation: "Der Phi-Koeffizient von <strong>[WERT]</strong> deutet auf einen <strong>[STAERKE]</strong> Zusammenhang zwischen dem Merkmal [MERKMAL] und dem N-Status im Kollektiv [KOLLEKTIV] hin."},
        rd: { name: "Risk Difference (RD)", description: "Absolute Differenz in der Wahrscheinlichkeit (Risiko) für N+ zwischen Patienten mit und ohne das Merkmal [MERKMAL]. Gibt an, um wie viele Prozentpunkte das Risiko höher oder niedriger ist.<br><i>Formel: P(N+|Merkmal+) - P(N+|Merkmal-)</i>", interpretation: "Das Risiko für einen N+ Status war um <strong>[WERT]%</strong> absolut [HOEHER_NIEDRIGER] bei Patienten mit dem Merkmal [MERKMAL] verglichen mit Patienten ohne dieses Merkmal (95% CI nach [METHOD_CI]: [LOWER]% – [UPPER]%) im Kollektiv [KOLLEKTIV]."},
        or: { name: "Odds Ratio (OR)", description: "Quotient der Odds für N+ bei Vorhandensein vs. Abwesenheit des Merkmals [MERKMAL]. Eine OR > 1 bedeutet erhöhte Odds bei Vorhandensein des Merkmals, OR < 1 verringerte Odds.<br><i>Formel: (P(N+|Merkmal+) / P(N-|Merkmal+)) / (P(N+|Merkmal-) / P(N-|Merkmal-))</i>", interpretation: "Die Chance (Odds) für einen N+ Status war bei Patienten mit dem Merkmal [MERKMAL] um den Faktor <strong>[WERT]</strong> [FAKTOR_TEXT] im Vergleich zu Patienten ohne dieses Merkmal (95% CI nach [METHOD_CI]: [LOWER] – [UPPER], Test-p=[P_WERT], [SIGNIFIKANZ]) im Kollektiv [KOLLEKTIV]."},
        fisher: { name: "Fisher's Exact Test", description: "Exakter Test zur Prüfung auf einen signifikanten Zusammenhang zwischen zwei kategorialen Variablen (z.B. Merkmal [MERKMAL] vs. N-Status). Wird typischerweise bei kleinen Stichprobengrößen oder wenn erwartete Häufigkeiten in der Kontingenztafel gering sind, verwendet.", interpretation: "Der exakte Test nach Fisher ergab einen p-Wert von <strong>[P_WERT] ([SIGNIFIKANZ])</strong>, was auf einen [SIGNIFIKANZ_TEXT] Zusammenhang zwischen dem Merkmal [MERKMAL] und dem N-Status im Kollektiv [KOLLEKTIV] hindeutet."},
        mannwhitney: { name: "Mann-Whitney-U-Test", description: "Nichtparametrischer Test zum Vergleich der zentralen Tendenz (Median) einer kontinuierlichen oder ordinalen Variable (z.B. '[VARIABLE]') zwischen zwei unabhängigen Gruppen (z.B. N+ vs. N-). Prüft, ob die Werte in einer Gruppe tendenziell höher oder niedriger sind als in der anderen.", interpretation: "Der Mann-Whitney-U-Test ergab einen p-Wert von <strong>[P_WERT] ([SIGNIFIKANZ])</strong>. Dies zeigt einen [SIGNIFIKANZ_TEXT] Unterschied in der Verteilung der Variable '[VARIABLE]' zwischen N+ und N- Patienten im Kollektiv [KOLLEKTIV]."},
        ci95: { name: "95% Konfidenzintervall (CI)", description: "Der Wertebereich, der den wahren (unbekannten) Wert der Population für die berechnete Metrik mit einer Wahrscheinlichkeit von 95% überdeckt.<br><i>Methode für Berechnung: [METHOD_CI]</i>", interpretation: "Basierend auf den Daten liegt der wahre Wert der Metrik mit 95%iger Sicherheit zwischen [LOWER] und [UPPER]."},
        ...
        accComp: { name: "Accuracy Vergleich (ungepaart)", description: "Vergleicht die Accuracy der Methode [METHODE] zwischen zwei unabhängigen Kollektiven ([KOLLEKTIV1] vs. [KOLLEKTIV2]) mittels Fisher's Exact Test auf Basis der 2x2 Tafel der korrekten/falschen Klassifikationen.", interpretation: "Der Unterschied in der Accuracy der Methode [METHODE] zwischen den Kollektiven [KOLLEKTIV1] und [KOLLEKTIV2] ist <strong>[SIGNIFIKANZ_TEXT]</strong> (p=[P_WERT])." },
        aucComp: { name: "AUC Vergleich (ungepaart)", description: "Vergleicht die AUC der Methode [METHODE] zwischen zwei unabhängigen Kollektiven ([KOLLEKTIV1] vs. [KOLLEKTIV2]) mittels eines Z-Tests unter Verwendung der Standardfehler der AUCs.", interpretation: "Der Unterschied in der AUC der Methode [METHODE] zwischen den Kollektiven [KOLLEKTIV1] und [KOLLEKTIV2] ist <strong>[SIGNIFIKANZ_TEXT]</strong> (p=[P_WERT])." },
