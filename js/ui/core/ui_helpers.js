const ui_helpers = (() => {
    ...
    function escapeMarkdown(text) {
        ...
        if (strText.length > 500) {
            strText = strText.substring(0, 497) + "...";
        }
        const map = { '\\': '\\\\', '`': '\\`', '*': '\\*', '_': '\\_', '{': '\\{', '}': '\\}', '[': '\\[', ']': '\\]', '(': '\\(', ')': '\\)', '#': '\\#', '+': '\\+', '-': '\\-', '.': '\\.', '!': '\\!', '|': '\\|' };
        try {
            return strText.replace(/[\\`*_{}[\]()#+\-.!|]/g, match => map[match] || match);
        } catch (e) {
            console.warn("Error in escapeMarkdown, returning original text fragment:", e);
    ...
    function showToast(message, type = 'info', duration = APP_CONFIG.UI_SETTINGS.TOAST_DURATION_MS) {
          ...
          if (processedMessage.length > 300) {
              processedMessage = processedMessage.substring(0, 297) + "...";
          }
          const escapedMessage = escapeMarkdown(processedMessage);


          if (escapedMessage.trim() === '') { console.warn("showToast: Nachricht nach Verarbeitung leer."); return; }
          ...
          toastElement.innerHTML = `<div class="d-flex"><div class="toast-body"><i class="fas ${iconClass} fa-fw me-2"></i> ${escapedMessage}</div><button type="button" class="btn-close me-2 m-auto ${textClass === 'text-white' ? 'btn-close-white' : ''}" data-bs-dismiss="toast" aria-label="Schließen"></button></div>`;
          toastContainer.appendChild(toastElement);

          try {
              const toastInstance = new bootstrap.Toast(toastElement, { delay: (typeof duration === 'number' && duration > 0) ? duration : 5000, autohide: (typeof duration === 'number' && duration > 0) });
    ...
    function toggleAllDetails(tableBodyId, buttonId) {
        ...
        const tooltipKeyBase = (buttonId === 'daten-toggle-details') ? 'datenTable' : 'auswertungTable';
    ...
    function updateStatistikSelectorsUI(layout, kollektiv1, kollektiv2) {
        const toggleBtnEinzel = document.getElementById('statistik-toggle-einzel');
        const toggleBtnVergleich = document.getElementById('statistik-toggle-vergleich');
        ...
        const vsLabel = document.getElementById('statistik-vs-label');
        ...
        if (toggleBtnEinzel) { toggleBtnEinzel.classList.toggle('active', !isVergleich); toggleBtnEinzel.classList.toggle('btn-primary', !isVergleich); toggleBtnEinzel.classList.toggle('btn-outline-primary', isVergleich); toggleBtnEinzel.setAttribute('aria-pressed', String(!isVergleich));}
        if (toggleBtnVergleich) { toggleBtnVergleich.classList.toggle('active', isVergleich); toggleBtnVergleich.classList.toggle('btn-primary', isVergleich); toggleBtnVergleich.classList.toggle('btn-outline-primary', !isVergleich); toggleBtnVergleich.setAttribute('aria-pressed', String(isVergleich));}

        if (container1) container1.style.display = isVergleich ? '' : 'none';
        if (vsLabel) vsLabel.style.display = isVergleich ? '' : 'none';
        if (container2) container2.style.display = isVergleich ? '' : 'none';
    ...
    function updatePublikationSelectorsUI(activeSection, currentLang) {
        const sectionButtonGroup = document.querySelector('.publikation-section-nav .btn-group');
        if (sectionButtonGroup) {
            sectionButtonGroup.querySelectorAll('button[data-section]').forEach(btn => {
                if (btn) {
                    btn.classList.toggle('active', btn.dataset.section === activeSection);
                    btn.classList.toggle('btn-primary', btn.dataset.section === activeSection);
                    btn.classList.toggle('btn-outline-primary', btn.dataset.section !== activeSection);
                }
            });
        }
    ...
    function updateExportButtonStates(activeTabId, hasBruteForceResults, canExportDataDependent) {
        ...
        trySetDisabled('export-statistik-csv', dataDisabled); trySetDisabled('export-deskriptiv-md', dataDisabled); trySetDisabled('export-daten-md', dataDisabled); trySetDisabled('export-auswertung-md', dataDisabled); trySetDisabled('export-filtered-data-csv', dataDisabled); trySetDisabled('export-comprehensive-report-html', dataDisabled);
        trySetDisabled('export-charts-png', dataDisabled); trySetDisabled('export-charts-svg', dataDisabled);
        trySetDisabled('export-all-zip', dataDisabled && bfDisabled); trySetDisabled('export-csv-zip', dataDisabled); trySetDisabled('export-md-zip', dataDisabled); trySetDisabled('export-png-zip', dataDisabled); trySetDisabled('export-svg-zip', dataDisabled);
