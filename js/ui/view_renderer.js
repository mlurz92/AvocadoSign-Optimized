const viewRenderer = (() => {
    ...
    function renderDatenTab(data, sortState) { // Umbenannt von renderPatientenTab
        _renderTabContent('daten-tab', () => { // Angepasst
             if (!data) throw new Error("Daten für Datentabelle nicht verfügbar."); // Angepasst
            ...
            const tableHTML = uiViewLogic.createDatenTableHTML(data, sortState); // Angepasst
            const finalHTML = toggleButtonHTML + `<div class="table-responsive">${tableHTML}</div>`;

            setTimeout(() => {
                 const tableBody = document.getElementById('daten-table-body'); // Angepasst
                 const tableHeader = document.getElementById('daten-table-header'); // Angepasst
    ...
    function renderPublikationTab(processedData, bruteForceResults, currentGlobalKollektiv, appliedCriteria, appliedLogic, publicationLang, activeSectionKey = 'methoden') { // NEU
        _renderTabContent('publikation-tab', () => {
            ...
            const initialHTML = `
                <div class="d-flex justify-content-between align-items-center mb-3 p-2 border-bottom sticky-top bg-light-alpha" id="publikation-tab-header-controls">
                    ${uiComponents.createPublikationTabNavigation(activeSectionKey)}
                    ${uiComponents.createPublikationLanguageSwitcher(publicationLang)}
                </div>
                <div class="tab-content" id="publikationSubNavContent">
                    <div class="tab-pane fade ${activeSectionKey === 'methoden' ? 'show active' : ''}" id="pub-methoden-pane" role="tabpanel">
                        ${uiComponents.createPublikationSectionWrapper('methoden', publicationLang)}
                    </div>
                    <div class="tab-pane fade ${activeSectionKey === 'ergebnisse' ? 'show active' : ''}" id="pub-ergebnisse-pane" role="tabpanel">
                        ${uiComponents.createPublikationSectionWrapper('ergebnisse', publicationLang)}
                    </div>
                </div>
            `;
            // Call the specific renderer function to fill content after initial structure is set
            setTimeout(() => {
                 publicationTabRenderer.renderPublicationTabContent(processedData, bruteForceResults, currentGlobalKollektiv, appliedCriteria, appliedLogic, publicationLang, activeSectionKey);
                 ui_helpers.updatePublikationLangSwitchUI(publicationLang);
            }, 10);
            return initialHTML;
        });
    }
    ...
    return Object.freeze({
        renderDatenTab: renderDatenTab, // Umbenannt
        renderAuswertungTab,
        renderStatistikTab,
