const viewRenderer = (() => {
    ...
    function renderDatenTab(data, sortState) {
        _renderTabContent('daten-tab', () => {
             ...
             const tooltipText = (typeof tooltipTextBase === 'object' ? tooltipTextBase[langKey] : tooltipTextBase) || (langKey === 'de' ? 'Alle Details ein-/ausblenden' : 'Expand/collapse all details');
    ...
    function _renderAuswertungDashboardCharts(stats) {
        ...
        try {
            ...
            chartRenderer.renderPieChart([{label: nPositiveLabel, value: stats.nStatus?.plus ?? 0}, {label: nNegativeLabel, value: stats.nStatus?.minus ?? 0}], ids[3], {...pieOpts, legendItemCount: 2});
            chartRenderer.renderPieChart([{label: asPositiveLabel, value: stats.asStatus?.plus ?? 0}, {label: asNegativeLabel, value: stats.asStatus?.minus ?? 0}], ids[4], {...pieOpts, legendItemCount: 2});
            chartRenderer.renderPieChart([{label: t2PositiveLabel, value: stats.t2Status?.plus ?? 0}, {label: t2NegativeLabel, value: stats.t2Status?.minus ?? 0}], ids[5], {...pieOpts, legendItemCount: 2});
        }
        catch(error) {
     ...
     function _renderCriteriaComparisonTable(containerId, data, kollektiv) {
         ...
         const cardTitle = (typeof cardTitleBase === 'object' ? cardTitleBase[langKey] : cardTitleBase) || (langKey === 'de' ? 'Kriterienvergleich' : 'Criteria Comparison');
         ...
         comparisonSetIds.forEach(setId => {
            let perf = null; let setName = langKey === 'de' ? 'Unbekannt' : 'Unknown'; let setIdUsed = setId;
            try {
                if (setId === APP_CONFIG.SPECIAL_IDS.AVOCADO_SIGN_ID) {
                    ...
                    setName = (typeof asDisplayNameBase === 'object' ? asDisplayNameBase[langKey] : asDisplayNameBase) || asDisplayNameBase.de;
                    ...
                    setName = (typeof appliedDisplayNameBase === 'object' ? appliedDisplayNameBase[langKey] : appliedDisplayNameBase) || appliedDisplayNameBase.de;
                } else {
                    ...
                    if (studySet) {
                        ...
                        setName = (typeof studySetNameBase === 'object' ? studySetNameBase[langKey] : studySetNameBase) || studySet.name;
                    }
    ...
    function renderAuswertungTab(data, currentCriteria, currentLogic, sortState, currentKollektiv, bfWorkerAvailable) {
         _renderTabContent('auswertung-tab', () => {
             const langKey = state.getCurrentPublikationLang() || 'de';
             if (!data || !currentCriteria || !currentLogic) throw new Error(langKey === 'de' ? "Daten oder Kriterien für Auswertungstab nicht verfügbar." : "Data or criteria for evaluation tab not available.");
                 ...
                 if (dashboardContainer) {
                     try {
                         ...
                         if (!stats || stats.anzahlPatienten === 0) {
                             ...
                             const pngTooltip = (typeof pngTooltipBase === 'object' ? pngTooltipBase[langKey] : pngTooltipBase) || (langKey === 'de' ? 'Als PNG' : 'As PNG');
                             const svgTooltipBase = TOOLTIP_CONTENT.exportTab.chartSingleSVG?.description;
                             const svgTooltip = (typeof svgTooltipBase === 'object' ? svgTooltipBase[langKey] : svgTooltipBase) || (langKey === 'de' ? 'Als SVG' : 'As SVG');
                             ...
                             const getChartTitle = (key) => {
                                 ...
                                 return (typeof titleBase === 'object' ? titleBase[langKey] : titleBase) || titleBase?.de || key;
                             };

                             dashboardContainer.innerHTML = `
                                ${uiComponents.createDashboardCard(getChartTitle('ageDistribution'), `<p class="mb-0 small">Median: ${formatNumber(stats.alter?.median, 1, '--', false, langKey)} (${formatNumber(stats.alter?.min, 0, '--', false, langKey)} - ${formatNumber(stats.alter?.max, 0, '--', false, langKey)})</p>`, 'chart-dash-age', '', '', 'p-1', createDlBtns('chart-dash-age'))}
    ...
    function renderStatistikTab(processedDataFull, appliedCriteria, appliedLogic, layout, kollektiv1, kollektiv2, currentGlobalKollektiv) {
        _renderTabContent('statistik-tab', () => {
             const langKey = state.getCurrentPublikationLang() || 'de';
             if (!processedDataFull) throw new Error(langKey === 'de' ? "Statistik-Daten nicht verfügbar." : "Statistics data not available.");
             ...
             if (datasets.length === 0 || datasets.every(d => !Array.isArray(d) || d.length === 0)) { return `<div class="col-12"><div class="alert alert-warning">${langKey === 'de' ? 'Keine Daten für Statistik-Auswahl verfügbar.' : 'No data available for statistics selection.'}</div></div>`; }
             ...
             const pngTT = (typeof pngTTBase === 'object' ? pngTTBase[langKey] : pngTTBase) || (langKey === 'de' ? 'PNG' : 'PNG');
             const svgTTBase = TOOLTIP_CONTENT.exportTab.chartSingleSVG?.description;
             const svgTT = (typeof svgTTBase === 'object' ? svgTTBase[langKey] : svgTTBase) || (langKey === 'de' ? 'SVG' : 'SVG');
             ...
             datasets.forEach((data, i) => {
                 const kollektivName = kollektivDisplayNames[i]; const col = document.createElement('div'); col.className = layout === 'vergleich' ? 'col-xl-6' : 'col-12'; const innerRowId = `inner-stat-row-${i}`; col.innerHTML = `<h4 class="mb-3">${langKey === 'de' ? 'Kollektiv' : 'Cohort'}: ${kollektivName} (N=${data.length})</h4><div class="row g-3" id="${innerRowId}"></div>`; outerRow.appendChild(col); const innerContainer = col.querySelector(`#${innerRowId}`);
                 if (data.length > 0) {
                     let stats = null;
                     try {
                         stats = {
                     ...
                     if (!stats) { innerContainer.innerHTML = `<div class="col-12"><div class="alert alert-danger">${langKey === 'de' ? 'Fehler bei Statistikberechnung.' : 'Error in statistics calculation.'}</div></div>`; return; }
                     ...
                     setTimeout(() => {
                        ...
                        if (ageChartDiv) {
                         ...
                         if (genderChartDiv) {
                            const genderData = [{label: UI_TEXTS.legendLabels.male[langKey], value: stats.deskriptiv.geschlecht?.m ?? 0}, {label: UI_TEXTS.legendLabels.female[langKey], value: stats.deskriptiv.geschlecht?.f ?? 0}]; if(stats.deskriptiv.geschlecht?.unbekannt > 0) genderData.push({label: UI_TEXTS.legendLabels.unknownGender[langKey], value: stats.deskriptiv.geschlecht.unbekannt });
                 ...
                 } else { innerContainer.innerHTML = `<div class="col-12"><div class="alert alert-warning small p-2">${langKey === 'de' ? 'Keine Daten für dieses Kollektiv.' : 'No data for this cohort.'}</div></div>`; }
             });
    ...
    function renderPresentationTab(view, selectedStudyId, currentGlobalKollektiv, processedDataFull, appliedCriteria, appliedLogic) {
        _renderTabContent('praesentation-tab', () => {
            const langKey = state.getCurrentPublikationLang() || 'de';
            if (!processedDataFull) throw new Error(langKey === 'de' ? "Präsentations-Daten nicht verfügbar." : "Presentation data not available.");
                ...
                presentationData.statsCurrentKollektiv = statsCurrent;
                 ...
                 if (filteredData && filteredData.length > 0) {
                    ...
                    if (studySet && evaluatedDataT2) {
                        ...
                        const studySetName = (typeof studySetNameBase === 'object' ? studySetNameBase[langKey] : studySetNameBase) || studySet.name;
            ...
            setTimeout(() => {
                if (view === 'as-pur') {
                     ...
                     if (chartContainer && presentationData?.statsCurrentKollektiv && presentationData.patientCount > 0 && !isNaN(presentationData.statsCurrentKollektiv.sens?.value)) {
                         ...
                         ui_helpers.updateElementHTML(chartContainer.id, `<p class="text-muted small text-center p-3">${langKey === 'de' ? 'Keine Daten für Performance-Chart.' : 'No data for performance chart.'}</p>`);
                     }
                } else if (view === 'as-vs-t2') {
                     const chartContainer = document.getElementById('praes-comp-chart-container');
                     if (chartContainer && presentationData?.statsAS && presentationData?.statsT2 && presentationData.patientCount > 0 && !isNaN(presentationData.statsAS.sens?.value) && !isNaN(presentationData.statsT2.sens?.value)) {
                         ...
                         chartRenderer.renderComparisonBarChart(chartDataComp, 'praes-comp-chart-container', { height: 300, margin: { top: 20, right: 20, bottom: 50, left: 50 } }, presentationData.t2CriteriaLabelShort || 'T2', langKey);
                     } else if (chartContainer) {
                         ui_helpers.updateElementHTML(chartContainer.id, `<p class="text-muted small text-center p-3">${langKey === 'de' ? 'Keine Daten für Vergleichschart.' : 'No data for comparison chart.'}</p>`);
                     }
                }
    ...
    function renderPublikationTab(currentLang, currentSection, currentKollektiv, globalProcessedData, bruteForceResults) {
        _renderTabContent('publikation-tab', () => {
            ...
            const headerHTML = uiComponents.createPublikationTabHeader(); // Should be language-aware internally
