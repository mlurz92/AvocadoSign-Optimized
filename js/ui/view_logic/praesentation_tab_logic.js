const praesentationTabLogic = (() => {
    ...
    function _createPresentationView_ASPUR_HTML(presentationData) {
        ...
        const createPerfTableRow = (stats, kollektivKey) => {
            ...
            if (!stats || typeof stats.matrix !== 'object') {
                const missingDataText = langKey === 'de' ? 'Daten fehlen' : 'Data missing';
                return `<tr><td class="fw-bold" data-tippy-content="${(TOOLTIP_CONTENT.praesentation.asPurPerfTable?.kollektiv?.[langKey] || TOOLTIP_CONTENT.praesentation.asPurPerfTable?.kollektiv?.['de'] || (langKey === 'de' ? 'Kollektiv' : 'Cohort'))}">${kollektivDisplayName} (N=?)</td><td colspan="6" class="text-muted text-center">${missingDataText}</td></tr>`;
            }
            const count = stats.matrix ? (stats.matrix.rp + stats.matrix.fp + stats.matrix.fn + stats.matrix.rn) : 0;
            return `<tr>
                        <td class="fw-bold" data-tippy-content="${(TOOLTIP_CONTENT.praesentation.asPurPerfTable?.kollektiv?.[langKey] || TOOLTIP_CONTENT.praesentation.asPurPerfTable?.kollektiv?.['de'] || (langKey === 'de' ? 'Kollektiv' : 'Cohort'))}">${kollektivDisplayName} (N=${count})</td>
        ...
        const perfCSVTooltip = (typeof perfCSVTooltipBase === 'object' ? perfCSVTooltipBase[langKey] : perfCSVTooltipBase?.[langKey]) || perfCSVTooltipBase?.['de'] || "CSV";
        const perfMDTooltipBase = TOOLTIP_CONTENT.praesentation.downloadPerformanceMD?.description;
        const perfMDTooltip = (typeof perfMDTooltipBase === 'object' ? perfMDTooltipBase[langKey] : perfMDTooltipBase?.[langKey]) || perfMDTooltipBase?.['de'] || "MD";
        const tablePNGTooltipBase = TOOLTIP_CONTENT.praesentation.downloadTablePNG?.description;
        const tablePNGTooltip = (typeof tablePNGTooltipBase === 'object' ? tablePNGTooltipBase[langKey] : tablePNGTooltipBase?.[langKey]) || tablePNGTooltipBase?.['de'] || "Tabelle als PNG";

        const chartPNGTooltip = `${langKey === 'de' ? 'Chart' : 'Chart'} (${currentKollektivName}) PNG`;
        const chartSVGTooltip = `${langKey === 'de' ? 'Chart' : 'Chart'} (${currentKollektivName}) SVG`;
        ...
        const tableHeaderHTML = tooltipKeys.map((key, index) => {
            ...
            let headerLabelText = index === 0 ? (langKey === 'de' ? 'Kollektiv' : 'Cohort') : `${labelKey}.`;
            if (index > 0 && index < tooltipKeys.length) { // Add (95% CI) for metrics except first and last
                headerLabelText += ' (95% CI)';
            }
        ...
        const cardHeaderDe = 'AS Performance vs. N für alle Kollektive';
        const cardHeaderEn = 'AS Performance vs. N for all Cohorts';
        const cardHeaderText = langKey === 'de' ? cardHeaderDe : cardHeaderEn;
        ...
        const chartCardTitleTextDe = `Visualisierung Güte (AS vs. N) - Kollektiv: ${currentKollektivName}`;
        const chartCardTitleTextEn = `Performance Visualization (AS vs. N) - Cohort: ${currentKollektivName}`;
        const chartCardTitle = langKey === 'de' ? chartCardTitleTextDe : chartCardTitleTextEn;

        const chartTippyContentDe = `Balkendiagramm der Gütekriterien für AS vs. N für Kollektiv ${currentKollektivName}.`;
        const chartTippyContentEn = `Bar chart of performance metrics for AS vs. N for cohort ${currentKollektivName}.`;
        const chartTippyContent = (langKey === 'de' ? chartTippyContentDe : chartTippyContentEn);
        const noDataForChartText = langKey === 'de' ? `Keine Daten für Chart (${currentKollektivName}).` : `No data for chart (${currentKollektivName}).`;
                ...
                <div class="card">
                    ...
                    <div class="card-body p-1">
                        <div id="${chartId}" class="praes-chart-container border rounded" style="min-height: 280px;" data-tippy-content="${chartTippyContent}">
                            ${hasDataForCurrent ? '' : `<p class="text-center text-muted p-3">${noDataForChartText}</p>`}
                        </div>
                    </div>
                </div>
            </div>`;

        const mainTitleText = langKey === 'de' ? 'Diagnostische Güte - Avocado Sign' : 'Diagnostic Performance - Avocado Sign';
        return `<div class="row g-3"><div class="col-12"><h3 class="text-center mb-3">${mainTitleText}</h3></div>${tableHTML}${chartHTML}</div>`;
    }
    ...
    function _createPresentationView_ASvsT2_HTML(presentationData, selectedStudyId = null, currentKollektiv = 'Gesamt') {
        ...
        const appliedNameBase = UI_TEXTS.kollektivDisplayNames.applied_criteria;
        const appliedName = (typeof appliedNameBase === 'object' ? appliedNameBase[langKey] : appliedNameBase?.[langKey]) || appliedNameBase?.['de'] || 'Eingestellte T2 Kriterien';
        ...
        if (selectedStudyId && comparisonCriteriaSet) {
            ...
            if (comparisonCriteriaSet.id === 'rutegard_et_al_esgar' && studyInfo?.keyCriteriaSummary) {
                 criteriaHTML = ui_helpers.escapeMarkdown(studyInfo.keyCriteriaSummary);
            } else if (criteriaSourceForFormatting) {
                 criteriaHTML = ui_helpers.escapeMarkdown(studyT2CriteriaManager.formatCriteriaForDisplay(criteriaSourceForFormatting, logicSourceForFormatting, false, langKey));
                 const noActiveText = UI_TEXTS.noActiveCriteria[langKey] || UI_TEXTS.noActiveCriteria.de;
                 if (criteriaHTML === noActiveText && logicSourceForFormatting) criteriaHTML += ` (${langKey === 'de' ? 'Logik' : 'Logic'}: ${UI_TEXTS.t2LogicDisplayNames[logicSourceForFormatting]?.[langKey] || logicSourceForFormatting})`;
                 else if (criteriaHTML !== noActiveText && logicSourceForFormatting) criteriaHTML = `<strong>${langKey === 'de' ? 'Logik' : 'Logic'}:</strong> ${UI_TEXTS.t2LogicDisplayNames[logicSourceForFormatting]?.[langKey] || logicSourceForFormatting}<br><strong>${langKey === 'de' ? 'Regel(n)' : 'Rule(s)'}:</strong> ${criteriaHTML}`;
            }
            const t2BasisInfoTooltips = TOOLTIP_CONTENT.praesentation.t2BasisInfoCard || {};
            const refLabel = langKey === 'de' ? 'Referenz:' : 'Reference:';
            const origCohortLabel = langKey === 'de' ? 'Orig.-Kohorte:' : 'Orig. Cohort:';
            const investigationLabel = langKey === 'de' ? 'Untersuchung:' : 'Investigation:';
            const focusLabel = langKey === 'de' ? 'Studienfokus:' : 'Study Focus:';
            const criteriaLabel = langKey === 'de' ? 'Kriterien:' : 'Criteria:';
            const userDefText = langKey === 'de' ? 'Benutzerdefiniert' : 'User-defined';

            comparisonInfoHTML = `<dl class="row small mb-0">
                                    <dt class="col-sm-4" data-tippy-content="${(t2BasisInfoTooltips.reference?.[langKey] || t2BasisInfoTooltips.reference?.['de'] || refLabel)}">${refLabel}</dt><dd class="col-sm-8">${ui_helpers.escapeMarkdown(studyInfo?.reference || (isApplied ? userDefText : 'N/A'))}</dd>
                                    <dt class="col-sm-4" data-tippy-content="${(t2BasisInfoTooltips.patientCohort?.[langKey] || t2BasisInfoTooltips.patientCohort?.['de'] || origCohortLabel)}">${origCohortLabel}</dt><dd class="col-sm-8">${ui_helpers.escapeMarkdown(studyInfo?.patientCohort || 'N/A')}</dd>
                                    <dt class="col-sm-4" data-tippy-content="${(t2BasisInfoTooltips.investigationType?.[langKey] || t2BasisInfoTooltips.investigationType?.['de'] || investigationLabel)}">${investigationLabel}</dt><dd class="col-sm-8">${ui_helpers.escapeMarkdown(studyInfo?.investigationType || 'N/A')}</dd>
                                    <dt class="col-sm-4" data-tippy-content="${(t2BasisInfoTooltips.focus?.[langKey] || t2BasisInfoTooltips.focus?.['de'] || focusLabel)}">${focusLabel}</dt><dd class="col-sm-8">${ui_helpers.escapeMarkdown(studyInfo?.focus || 'N/A')}</dd>
                                    <dt class="col-sm-4" data-tippy-content="${(t2BasisInfoTooltips.keyCriteriaSummary?.[langKey] || t2BasisInfoTooltips.keyCriteriaSummary?.['de'] || criteriaLabel)}">${criteriaLabel}</dt><dd class="col-sm-8">${criteriaHTML}</dd>
                                </dl>`;
        }
        ...
        const studyOptionsHTML = studySets.map(set => {
            ...
            const setName = (typeof setNameBase === 'object' ? setNameBase[langKey] : setNameBase?.[langKey]) || setNameBase?.['de'] || set.name || set.id;
        ...
        if (canDisplayResults) {
            const fPVal = (r,d=3) => { const p = r?.pValue; return (p !== null && !isNaN(p) && isFinite(p)) ? (p < 0.001 ? (langKey === 'de' ? '&lt;0,001' : '&lt;.001') : formatNumber(p, d, na, false, langKey)) : na; };

            const getTooltipText = (tooltipKey, subKey, defaultValue) => {
                const base = TOOLTIP_CONTENT.praesentation[tooltipKey]?.[subKey];
                return (typeof base === 'object' ? base[langKey] : base?.[langKey]) || base?.['de'] || defaultValue;
            };
            ...
            const compTitleText = langKey === 'de' ? `Stat. Vergleich (AS vs. ${t2ShortNameEffective})` : `Stat. Comparison (AS vs. ${t2ShortNameEffective})`;
            const perfTitleText = langKey === 'de' ? `Vergleich Metriken (AS vs. ${t2ShortNameEffective})` : `Comparison Metrics (AS vs. ${t2ShortNameEffective})`;
            const chartTitleText = langKey === 'de' ? `Vergleichs-Chart (AS vs. ${t2ShortNameEffective})` : `Comparison Chart (AS vs. ${t2ShortNameEffective})`;
            ...
            } else { asValueTooltip = `AS value (vs. N) in cohort ${kollektivName}, incl. 95% CI.`; }
            ...
            } else { t2ValueTooltip = `Value for ${t2ShortNameEffective} (vs. N) in cohort ${kollektivName}, incl. 95% CI.`; }
            ...
            metrics.forEach(key => {
                 ...
                 const tooltipDesc = ui_helpers.getMetricDescriptionHTML(key, (langKey === 'de' ? 'Wert' : 'Value'), langKey);
            ...
            const comparisonTableCardHTML = uiComponents.createStatistikCard(perfTableId+'_card', perfTitleText, comparisonTableHTML, false, 'praesentation.comparisonTableCard', compTableDownloadBtns, perfTableId);

            let testsTableHTML = `<table class="table table-sm table-striped small mb-0" id="${testTableId}"><thead class="small visually-hidden"><tr><th>${langKey === 'de' ? 'Test':'Test'}</th><th>${langKey === 'de' ? 'Statistik':'Statistic'}</th><th>${langKey === 'de' ? 'p-Wert':'p-value'}</th><th>${langKey === 'de' ? 'Methode':'Method'}</th></tr></thead><tbody>`;
            ...
            const testsCardHTML = uiComponents.createStatistikCard(testTableId+'_card', compTitleText, testsTableHTML, false, null, testTableDownloadBtns, testTableId);
            ...
            const loadingChartText = langKey === 'de' ? 'Lade Vergleichschart...' : 'Loading comparison chart...';
            const infoCardTitle = (TOOLTIP_CONTENT.praesentation.t2BasisInfoCard?.title?.[langKey] || TOOLTIP_CONTENT.praesentation.t2BasisInfoCard?.title?.['de'] || (langKey==='de'?'Info T2 Basis':'Info T2 Basis'));
            const infoCardTooltip = (TOOLTIP_CONTENT.praesentation.t2BasisInfoCard?.description?.[langKey] || TOOLTIP_CONTENT.praesentation.t2BasisInfoCard?.description?.['de'] || (langKey==='de'?'Details zur T2 Vergleichsbasis':'Details for T2 comparison basis'));
            ...
            resultsHTML = `
                <div class="row g-3 presentation-comparison-row">
                        ...
                        <div class="card h-100">
                            ...
                            <div class="card-body p-1 d-flex align-items-center justify-content-center">
                                 <div id="${chartContainerId}" class="praes-chart-container w-100" style="min-height: 300px;" data-tippy-content="${chartTippyContent}">
                                     <p class="text-muted small text-center p-3">${loadingChartText}</p>
                                 </div>
                            </div>
                             <div class="card-footer text-end p-1">
                    ...
                    <div class="col-lg-5 col-xl-5 presentation-comparison-col-right d-flex flex-column">
                         <div class="card mb-3 flex-shrink-0 praes-t2-basis-info-card" id="${infoCardId}" data-tippy-content="${infoCardTooltip}">
                            <div class="card-header card-header-sm">${infoCardTitle}</div>
        ...
        const mainTitleText = langKey === 'de' ? 'Vergleich: Avocado Sign vs. T2-Kriterien' : 'Comparison: Avocado Sign vs. T2 Criteria';
        const currentKollektivText = langKey === 'de' ? 'Aktuelles Kollektiv:' : 'Current Cohort:';
        const selectLabelText = langKey === 'de' ? 'T2-Vergleichsbasis:' : 'T2 Comparison Basis:';
        const selectPlaceholderText = langKey === 'de' ? '-- Bitte wählen --' : '-- Please select --';
        const pubCriteriaPlaceholderText = langKey === 'de' ? '--- Publizierte Kriterien ---' : '--- Published Criteria ---';
        ...
        const selectTippy = (typeof selectTippyBase === 'object' ? selectTippyBase[langKey] : selectTippyBase?.[langKey]) || selectTippyBase?.['de'] || '';


        return `<div class="row mb-4"><div class="col-12"><h4 class="text-center mb-1">${mainTitleText}</h4><p class="text-center text-muted small mb-3">${currentKollektivText} <strong>${kollektivName}</strong> (N=${patientCount ?? '?'})</p><div class="row justify-content-center"><div class="col-md-9 col-lg-7" id="praes-study-select-container"><div class="input-group input-group-sm"><label class="input-group-text" for="praes-study-select">${selectLabelText}</label><select class="form-select" id="praes-study-select" data-tippy-content="${selectTippy}"><option value="" ${!selectedStudyId ? 'selected' : ''} disabled>${selectPlaceholderText}</option>${appliedOptionHTML}<option value="" disabled>${pubCriteriaPlaceholderText}</option>${studyOptionsHTML}</select></div><div id="praes-study-description" class="mt-2 small text-muted">${comparisonBasisName === 'N/A' ? noBasisText : `${currentBasisText} <strong>${comparisonBasisName}</strong>`}</div></div></div></div></div><div id="praesentation-as-vs-t2-results">${resultsHTML}</div>`;
    }
    ...
    function createPresentationTabContent(view, presentationData, selectedStudyId = null, currentKollektiv = 'Gesamt') {
        ...
        const viewSelectTippy = (typeof viewSelectTippyBase === 'object' ? viewSelectTippyBase[langKey] : viewSelectTippyBase?.[langKey]) || viewSelectTippyBase?.['de'] || '';
        ...
        const viewSelectionLabel = langKey === 'de' ? 'Präsentationsansicht Auswahl' : 'Presentation View Selection';


        let viewSelectorHTML = `
            <div class="row mb-4">
                <div class="col-12 d-flex justify-content-center">
                    <div class="btn-group btn-group-sm" role="group" aria-label="${viewSelectionLabel}" data-tippy-content="${viewSelectTippy}">
        ...
        if (view === 'as-pur') {
            ...
            const unknownViewText = langKey === 'de' ? 'Unbekannte Ansicht ausgewählt.' : 'Unknown view selected.';
            contentHTML = `<div class="alert alert-warning">${unknownViewText}</div>`;
        }
