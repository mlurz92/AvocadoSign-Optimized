const publikationTabLogic = (() => {
    ...
    function updateDynamicChartsForPublicationTab(sectionId, lang, currentKollektiv) {
        if (!allKollektivStats || !allKollektivStats[currentKollektiv]) {
            ...
            const chartContainerIds = PUBLICATION_CONFIG.publicationElements.ergebnisse;
            Object.values(chartContainerIds).forEach(chartConf => {
                if(typeof chartConf === 'object' && chartConf.id){
                    const el = document.getElementById(chartConf.id.replace('table', 'chart')); // Attempt to find related chart id
                    if(el) ui_helpers.updateElementHTML(el.id, `<p class="text-muted small text-center p-3">Keine Daten für Chart (${getKollektivDisplayName(currentKollektiv)}).</p>`);
                }
            });
            if (sectionId === 'ergebnisse_patientencharakteristika') {
                 ...
                 if (document.getElementById(alterChartId)) ui_helpers.updateElementHTML(alterChartId, `<p class="text-muted small text-center p-3">Keine Daten für Altersverteilung (${getKollektivDisplayName(currentKollektiv)}).</p>`);
                 if (document.getElementById(genderChartId)) ui_helpers.updateElementHTML(genderChartId, `<p class="text-muted small text-center p-3">Keine Daten für Geschlechterverteilung (${getKollektivDisplayName(currentKollektiv)}).</p>`);
            }
            return;
        }
