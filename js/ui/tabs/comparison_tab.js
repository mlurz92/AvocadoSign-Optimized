window.comparisonTab = (() => {
    ...
    function _createASvsT2ComparisonViewHTML(comparisonData, selectedStudyId) {
        ...
        if (selectedStudyId && comparisonCriteriaSet) {
            ...
            // Logic display fix
            const displayLogic = comparisonCriteriaSet.logic === 'KOMBINIERT' ? 'Combined (ESGAR)' : comparisonCriteriaSet.logic;

            comparisonInfoHTML = `<dl class="row small mb-0">
                <dt class="col-sm-4">Reference:</dt>
                <dd class="col-sm-8">${studyInfo?.refKey ? `Published in ${window.studyT2CriteriaManager.getStudyCriteriaSetById(studyInfo.refKey)?.name || studyInfo.refKey}` : (studyInfo?.isDynamic ? 'Data-driven (Brute-Force)' : 'N/A')}</dd>
                
                <dt class="col-sm-4">Basis Cohort:</dt>
                <dd class="col-sm-8">${studyInfo?.patientCohort || `Current: ${displayCohortForComparison} (N=${patientCountForComparison || '?'})`}</dd>
                
                <dt class="col-sm-4">Logic:</dt>
                <dd class="col-sm-8"><strong>${displayLogic}</strong></dd>

                <dt class="col-sm-4">Criteria:</dt>
                <dd class="col-sm-8"><code>${criteriaHTML}</code></dd>
            </dl>`;
        }
        ...
        if (canDisplayResults) {
            ...
            metrics.forEach(metric => {
                ...
                const digits = (metric.key === 'auc') ? 3 : 1;
    ...
    function render(view, comparisonData, selectedStudyIdFromState) {
