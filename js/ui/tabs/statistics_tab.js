window.statisticsTab = (() => {
    ...
    // Helper to calculate extra demographics not covered by the core service
    function calculateExtendedDemographics(data) {
        ...
        data.forEach(p => {
            // T-Stage
            const t = p.tStage || 'Unknown';
            counts.tStage[t] = (counts.tStage[t] || 0) + 1;

            // Location
            const loc = p.tumorLocation || 'Unknown';
            counts.tumorLocation[loc] = (counts.tumorLocation[loc] || 0) + 1;

            // N-Substage
            const sub = p.nSubstage || 'Unknown';
            counts.nSubstage[sub] = (counts.nSubstage[sub] || 0) + 1;
        });

        return counts;
    }

    function createDescriptiveStatsContentHTML(stats, indexSuffix, cohortId, cohortData) {
        ...
        const getTooltip = (key, cId) => {
            let content = tooltips[key]?.description || '';
            return content.replace('[COHORT]', `<strong>${getCohortDisplayName(cId)}</strong>`);
        };

        // Extended Demographics (computed locally on the fly for display)
        const extended = calculateExtendedDemographics(cohortData);
        let extendedRows = '';
        
        if (extended) {
            const formatGroup = (groupObj, label) => {
                const keys = Object.keys(groupObj).sort();
                if (keys.length === 0 || (keys.length === 1 && keys[0] === 'Unknown')) return '';
                const details = keys.filter(k => k !== 'Unknown').map(k => `${k}: ${groupObj[k]}`).join(', ');
                return `<tr><td>${label}</td><td>${details}</td></tr>`;
            };
            
            extendedRows += formatGroup(extended.tStage, 'T-Stage Distribution');
            extendedRows += formatGroup(extended.tumorLocation, 'Tumor Location');
        }
    ...
    function createCriteriaComparisonTableHTML(allStats) {
        ...
        Object.keys(allStats).forEach(cohortId => {
            ...
            if (stats && stats.crossValidation) {
                ...
                const formatMeanSD = (mean, sd, digits = 1) => `${formatNumber(mean, digits)} ± ${formatNumber(sd, digits)}`;
                
                // Construct a performance-like object but with Mean/SD instead of CI values
                const row = {
                    name: `Cohort-Optimised T2 (${cv.folds}-Fold CV)<br><em class="text-muted small fw-normal">(Cohort: ${getCohortDisplayName(cohortId)})</em>`,
                    isPlaceholder: false,
                    auc: { value: cv.meanAUC, customLabel: formatMeanSD(cv.meanAUC, cv.sdAUC, 3) },
                    sens: { value: cv.meanSens, customLabel: formatMeanSD(cv.meanSens, cv.sdSens) },
                    spec: { value: cv.meanSpec, customLabel: formatMeanSD(cv.meanSpec, cv.sdSpec) },
                    ppv: { value: cv.meanPPV, customLabel: formatMeanSD(cv.meanPPV, cv.sdPPV) },
                    npv: { value: cv.meanNPV, customLabel: formatMeanSD(cv.meanNPV, cv.sdNPV) },
                    pValue: undefined, // Statistical comparison not directly applicable in this view
                    isCV: true
                };
                cvRows.push(row);
            }
        });
        addResults(cvRows, 'Internal Validation (Cross-Validation Results)');
        ...
        let tableHtml = `<div class="table-responsive"><table class="table table-sm table-striped small mb-0"><thead><tr>
            ...
            <th data-tippy-content="${getDefinitionTooltip('sens')}">Sens. (95% CI or Mean±SD)</th>
            <th data-tippy-content="${getDefinitionTooltip('spec')}">Spec. (95% CI or Mean±SD)</th>
            <th data-tippy-content="${getDefinitionTooltip('ppv')}">PPV (95% CI or Mean±SD)</th>
            <th data-tippy-content="${getDefinitionTooltip('npv')}">NPV (95% CI or Mean±SD)</th>
            <th data-tippy-content="${getDefinitionTooltip('auc')}">AUC (95% CI or Mean±SD)</th>
        ...
        results.forEach(r => {
            ...
            const formatMetric = (metric, key) => {
                 if (metric && metric.customLabel) {
                     return r.isCV ? `${metric.customLabel}` : metric.customLabel;
                 }
                 const isRate = key !== 'auc';
                 const digits = key === 'auc' ? 3 : 1;
                 return formatCI(metric?.value, metric?.ci?.lower, metric?.ci?.upper, digits, isRate, na_stat);
            };
    ...
    function createValidationStatsHTML(cvStats) {
        ...
        if (cvStats.details && Array.isArray(cvStats.details)) {
            foldsHtml = cvStats.details.map((fold, idx) => `
                <tr>
                    ...
                    <td>${formatPercent(fold.acc, 1)}</td>
                    <td>${formatPercent(fold.sens, 1)}</td>
                    <td>${formatPercent(fold.spec, 1)}</td>
                    <td class="text-muted small">${JSON.stringify(fold.bestConfig?.criteria?.size?.threshold) || '?'}mm</td>
                </tr>
            `).join('');
        }

        const formatMetricRow = (label, mean, sd, tooltipKey, isPercent = false, digits = 3) => {
            const valStr = isPercent ? formatPercent(mean, 1) : formatNumber(mean, digits, na);
            const sdStr = isPercent ? formatPercent(sd, 1) : formatNumber(sd, digits, na);
            return `
                <tr>
                    <td data-tippy-content="${getCVTooltip(tooltipKey)}">
                        ${label} <i class="bi bi-question-circle text-muted ms-1" style="font-size: 0.8em;"></i>
                    </td>
                    <td class="fw-bold">${valStr} ± ${sdStr}</td>
                </tr>
            `;
        };
        ...
        return `
            <div class="row g-2">
                ...
                <div class="col-12">
                     <h6 class="small fw-bold text-uppercase text-muted mb-2">Aggregate Performance (Mean ± SD of ${folds} Folds)</h6>
                     <div class="table-responsive">
                        <table class="table table-sm table-striped small mb-0">
                            ...
                            <tbody>
                                ${formatMetricRow('AUC', cvStats.meanAUC, cvStats.sdAUC, 'meanAUC', false, 3)}
                                ${formatMetricRow('Sensitivity', cvStats.meanSens, cvStats.sdSens, 'meanAUC', true)}
                                ${formatMetricRow('Specificity', cvStats.meanSpec, cvStats.sdSpec, 'meanAUC', true)}
                                ${formatMetricRow('Accuracy', cvStats.meanAcc, cvStats.sdAcc, 'meanAUC', true)}
                                ${formatMetricRow('PPV', cvStats.meanPPV, cvStats.sdPPV, 'meanAUC', true)}
                                ${formatMetricRow('NPV', cvStats.meanNPV, cvStats.sdNPV, 'meanAUC', true)}
                                <tr>
                                    <td data-tippy-content="${getCVTooltip('optimism')}">Optimism (AUC)</td>
                                    <td>${formatNumber(cvStats.optimism, 3, na)} (Apparent AUC: ${formatNumber(cvStats.apparentAUC, 3)})</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                        ...
                        <table class="table table-sm table-bordered table-hover small mb-0 text-center">
                            <thead class="table-light sticky-top">
                                <tr>
                                    ...
                                    <th data-tippy-content="${getCVTooltip('testN')}">N</th>
                                    ...
                                    <th data-tippy-content="${getCVTooltip('cutoff')}">Cut-off</th>
                                </tr>
                            </thead>
    ...
    function render(processedData, appliedCriteria, appliedLogic, layout, cohort1, cohort2, globalCohort) {
        ...
        // Ensure statistics are recalculated
        ...
        cohortIdsToShow.forEach((cohortId, i) => {
            ...
            if (stats && stats.descriptive.patientCount > 0) {
                // Get raw data for this cohort for extended demographic calculation
                ...
                innerContainer.innerHTML += window.uiComponents.createStatisticsCard(`descriptive-stats-${i}`, 'Descriptive Statistics', createDescriptiveStatsContentHTML(stats, i, cohortId, cohortData), true, 'descriptiveStatistics', cohortId);
                ...
                const createAssocTableHTML = (assocStats, appliedCrit) => {
                    ...
                    if (assocStats.as) addRow('as', 'Avocado Sign', assocStats.as);
                    // size_mwu removed from default set as per previous service logic, kept here if present
                ...
                // Added value generalized for all cohorts, check if available first
                const addedValueStats = stats.addedValueAnalysis?.['Rutegard_2025']; // Default to Rutegard for added value showcase
                if (addedValueStats) {
                    const addedValueContent = window.uiComponents.createAddedValueCardHTML(addedValueStats, 'ESGAR 2016');
                ...
                if (stats.crossValidation) {
                    const cvTitle = window.APP_CONFIG.UI_TEXTS.tooltips.crossValidation.cardTitle;
