const ui_helpers = (() => {
        ...
        tableHeader.querySelectorAll('th[data-sort-key]').forEach(th => {
            ...
            if (subSpans.length > 0) {
                subSpans.forEach(span => {
                    ...
                    span.setAttribute('data-tippy-content', `${APP_CONFIG.UI_TEXTS.global.sortByStatus} ${spanLabel}`);
                    if (isActiveSort) {
    ...
    function toggleAllDetails(tableBodyId, buttonId) {
        ...
        if (buttonId === 'daten-toggle-details') {
            buttonText = expand ? APP_CONFIG.UI_TEXTS.datenTable.hideAllDetails : APP_CONFIG.UI_TEXTS.datenTable.showAllDetails;
            tooltipKeyBase = 'datenTable';
            currentTooltipText = expand ? APP_CONFIG.UI_TEXTS.TOOLTIP_CONTENT.datenTable.expandAll.replace('ein-', 'aus-').replace('anzeigen', 'ausblenden') : APP_CONFIG.UI_TEXTS.TOOLTIP_CONTENT.datenTable.expandAll.replace('aus-', 'ein-').replace('ausblenden', 'anzeigen');
        } else if (buttonId === 'auswertung-toggle-details') {
            buttonText = expand ? APP_CONFIG.UI_TEXTS.auswertungTable.hideAllDetails : APP_CONFIG.UI_TEXTS.auswertungTable.showAllDetails;
            tooltipKeyBase = 'auswertungTable';
            currentTooltipText = expand ? APP_CONFIG.UI_TEXTS.TOOLTIP_CONTENT.auswertungTable.expandAll.replace('ein-', 'aus-').replace('anzeigen', 'ausblenden') : APP_CONFIG.UI_TEXTS.TOOLTIP_CONTENT.auswertungTable.expandAll.replace('aus-', 'ein-').replace('ausblenden', 'anzeigen');
        }
        ...
        Object.keys(currentCriteria).forEach(key => {
            ...
            if (checkbox && optionsContainer) {
                ...
                if (key === 'size') {
                    ...
                    const thresholdValue = criterion.threshold ?? APP_CONFIG.T2_CRITERIA_SETTINGS.DEFAULT_CRITERIA.size.threshold;
    ...
    function markCriteriaSavedIndicator(isUnsaved) {
        ...
        const tooltipContent = APP_CONFIG.UI_TEXTS.TOOLTIP_CONTENT.t2CriteriaCard.unsavedIndicator;
    ...
    function updateStatistikSelectorsUI(layout, kollektiv1, kollektiv2) {
        ...
        if (toggleBtn) {
            ...
            updateElementHTML(toggleBtn.id, isVergleich ? `<i class="fas fa-users-cog me-1"></i> ${APP_CONFIG.UI_TEXTS.statistikTab.comparisonActive}` : `<i class="fas fa-user-cog me-1"></i> ${APP_CONFIG.UI_TEXTS.statistikTab.singleViewActive}`);
            if(toggleBtn._tippy) toggleBtn._tippy.setContent(APP_CONFIG.UI_TEXTS.TOOLTIP_CONTENT.statistikToggleVergleich.description);
    ...
    function updateBruteForceUI(state, data = {}, workerAvailable = true, currentKollektiv = null) {
        ...
        const startButtonText = isRunning ? `<span class="spinner-border spinner-border-sm me-1" role="status" aria-hidden="true"></span> ${APP_CONFIG.UI_TEXTS.bruteForceStart.running}` : `<i class="fas fa-cogs me-1"></i> ${workerAvailable ? APP_CONFIG.UI_TEXTS.bruteForceStart.optimizeButton : APP_CONFIG.UI_TEXTS.bruteForceStart.workerMissing}`;
        ...
        switch (state) {
            case 'idle': case 'cancelled': case 'error':
                ...
                if (elements.bestCriteria) updateElementText(elements.bestCriteria.id, APP_CONFIG.UI_TEXTS.bruteForceInfo.bestCriteriaInitial || 'Beste Kriterien: --');
                let statusMsg = '';
                if (state === 'idle') statusMsg = workerAvailable ? APP_CONFIG.UI_TEXTS.bruteForceInfo.ready : APP_CONFIG.UI_TEXTS.bruteForceInfo.workerNotAvailable;
                else if (state === 'cancelled') statusMsg = APP_CONFIG.UI_TEXTS.bruteForceInfo.cancelled;
                else if (state === 'error') statusMsg = `${APP_CONFIG.UI_TEXTS.bruteForceInfo.error}: ${data?.message || APP_CONFIG.UI_TEXTS.global.unknown}.`;
                if (elements.statusText) updateElementText(elements.statusText.id, statusMsg);
                if (bfInfoElement) addOrUpdateTooltip(bfInfoElement, (APP_CONFIG.UI_TEXTS.TOOLTIP_CONTENT.bruteForceInfo.description || '').replace('[KOLLEKTIV_NAME]', `<strong>${getKollektivNameFunc(kollektivToDisplayForInfo)}</strong>`) + ` ${APP_CONFIG.UI_TEXTS.bruteForceInfo.currentStatus}: ${statusMsg}`);
                break;
            case 'start':
                if (elements.progressBar) { elements.progressBar.style.width = '0%'; elements.progressBar.setAttribute('aria-valuenow', '0'); }
                if (elements.progressPercent) updateElementText(elements.progressPercent.id, '0%');
                if (elements.testedCount) updateElementText(elements.testedCount.id, '0');
                if (elements.totalCount) updateElementText(elements.totalCount.id, APP_CONFIG.UI_TEXTS.bruteForceInfo.calculatingTotal || 'berechne...');
                if (elements.metricLabel) updateElementText(elements.metricLabel.id, data?.metric || APP_CONFIG.UI_TEXTS.global.metric);
                if (elements.bestMetric) updateElementText(elements.bestMetric.id, '--');
                if (elements.bestCriteria) updateElementText(elements.bestCriteria.id, APP_CONFIG.UI_TEXTS.bruteForceInfo.bestCriteriaInitial || 'Beste Kriterien: --');
                if (elements.statusText) updateElementText(elements.statusText.id, APP_CONFIG.UI_TEXTS.bruteForceInfo.initializing || 'Initialisiere...');
                if (bfInfoElement) addOrUpdateTooltip(bfInfoElement, (APP_CONFIG.UI_TEXTS.TOOLTIP_CONTENT.bruteForceInfo.description || '').replace('[KOLLEKTIV_NAME]', `<strong>${getKollektivNameFunc(kollektivToDisplayForInfo)}</strong>`) + ` ${APP_CONFIG.UI_TEXTS.bruteForceInfo.status}: ${APP_CONFIG.UI_TEXTS.bruteForceInfo.initializing}...`);
                break;
            case 'started':
                const totalComb = formatNumber(data?.totalCombinations || 0, 0, APP_CONFIG.UI_TEXTS.global.notApplicableShort);
                if (elements.totalCount) updateElementText(elements.totalCount.id, totalComb);
                if (elements.statusText) updateElementText(elements.statusText.id, APP_CONFIG.UI_TEXTS.bruteForceInfo.testing || 'Teste...');
                if (bfInfoElement) addOrUpdateTooltip(bfInfoElement, (APP_CONFIG.UI_TEXTS.TOOLTIP_CONTENT.bruteForceInfo.description || '').replace('[KOLLEKTIV_NAME]', `<strong>${getKollektivNameFunc(kollektivToDisplayForInfo)}</strong>`) + ` ${APP_CONFIG.UI_TEXTS.bruteForceInfo.status}: ${APP_CONFIG.UI_TEXTS.bruteForceInfo.testingCombinations.replace('[TOTAL_COMBINATIONS]', totalComb)}`);
                if (elements.progressContainer) addOrUpdateTooltip(elements.progressContainer, (APP_CONFIG.UI_TEXTS.TOOLTIP_CONTENT.bruteForceProgress.description || '').replace('[TOTAL]', totalComb));
                break;
            case 'progress':
                ...
                if (elements.statusText) updateElementText(elements.statusText.id, APP_CONFIG.UI_TEXTS.bruteForceInfo.running || 'Läuft...');
                if (bfInfoElement) addOrUpdateTooltip(bfInfoElement, (APP_CONFIG.UI_TEXTS.TOOLTIP_CONTENT.bruteForceInfo.description || '').replace('[KOLLEKTIV_NAME]', `<strong>${getKollektivNameFunc(kollektivToDisplayForInfo)}</strong>`) + ` ${APP_CONFIG.UI_TEXTS.bruteForceInfo.status}: ${percentStr} (${testedNum}/${totalNumProg})`);
                if (data?.currentBest && data.currentBest.criteria && isFinite(data.currentBest.metricValue)) {
                    ...
                    if (elements.bestCriteria) updateElementText(elements.bestCriteria.id, `${APP_CONFIG.UI_TEXTS.bruteForceInfo.best}: ${data.currentBest.logic?.toUpperCase()} - ${bestCritStr}`);
                } else {
                    if (elements.bestMetric) updateElementText(elements.bestMetric.id, '--');
                    if (elements.bestCriteria) updateElementText(elements.bestCriteria.id, APP_CONFIG.UI_TEXTS.bruteForceInfo.bestCriteriaInitial || 'Beste Kriterien: --');
                }
            ...
            case 'result':
                ...
                const resultKollektivName = getKollektivNameFunc(data.kollektiv || APP_CONFIG.UI_TEXTS.global.notApplicableShort);
                if (best && best.criteria && isFinite(best.metricValue)) {
                    const metricName = data.metric || APP_CONFIG.UI_TEXTS.global.notApplicableShort;
                    const bestValueStr = formatNumber(best.metricValue, 4);
                    const logicStr = best.logic?.toUpperCase() || APP_CONFIG.UI_TEXTS.global.notApplicableShort;
                    ...
                    if (elements.statusText) updateElementText(elements.statusText.id, APP_CONFIG.UI_TEXTS.bruteForceInfo.finished || 'Fertig.');
                     if (bfInfoElement) addOrUpdateTooltip(bfInfoElement, (APP_CONFIG.UI_TEXTS.TOOLTIP_CONTENT.bruteForceInfo.description || '').replace('[KOLLEKTIV_NAME]', `<strong>${resultKollektivName}</strong>`) + ` ${APP_CONFIG.UI_TEXTS.bruteForceInfo.status}: ${APP_CONFIG.UI_TEXTS.bruteForceInfo.finished}.`);
                     if (elements.resultContainer) addOrUpdateTooltip(elements.resultContainer, (APP_CONFIG.UI_TEXTS.TOOLTIP_CONTENT.bruteForceResult.description || '').replace('[N_GESAMT]', formatNumber(data.nGesamt,0,'?')).replace('[N_PLUS]', formatNumber(data.nPlus,0,'?')).replace('[N_MINUS]', formatNumber(data.nMinus,0,'?')) );
                } else {
                    if (elements.resultContainer) toggleElementClass(elements.resultContainer.id, 'd-none', true);
                    if (elements.statusText) updateElementText(elements.statusText.id, APP_CONFIG.UI_TEXTS.bruteForceInfo.finishedNoValidResult || 'Fertig (kein valides Ergebnis).');
                     if (bfInfoElement) addOrUpdateTooltip(bfInfoElement, (APP_CONFIG.UI_TEXTS.TOOLTIP_CONTENT.bruteForceInfo.description || '').replace('[KOLLEKTIV_NAME]', `<strong>${resultKollektivName}</strong>`) + ` ${APP_CONFIG.UI_TEXTS.bruteForceInfo.status}: ${APP_CONFIG.UI_TEXTS.bruteForceInfo.finishedNoResult}.`);
                }
                break;
        }
    }
    ...
    function updatePublikationUI(currentLang, currentSection, currentBfMetric) {
        ...
        if (langSwitch && langLabel) {
            ...
            langLabel.textContent = APP_CONFIG.UI_TEXTS.publikationTab.spracheSwitchLabel[currentLang];
        }
    ...
    function getMetricInterpretationHTML(key, metricData, methode = '', kollektivName = '', lang = 'de') {
        const interpretationTemplate = APP_CONFIG.UI_TEXTS.TOOLTIP_CONTENT.statMetrics[key]?.interpretation || APP_CONFIG.UI_TEXTS.global.noInterpretationAvailable;
        const data = (typeof metricData === 'object' && metricData !== null) ? metricData : { value: metricData, ci: null, method: null, n_trials: NaN, matrix_components: null };
        const na = '--';
        const digits = (key === 'f1' || key === 'auc') ? 3 : 1;
        const isPercent = !(key === 'f1' || key === 'auc');
        const valueStr = formatNumber(data?.value, digits, na, true, lang);
        const lowerStr = formatNumber(data?.ci?.lower, digits, na, true, lang);
        const upperStr = formatNumber(data?.ci?.upper, digits, na, true, lang);
        const ciMethodStr = data?.method || APP_CONFIG.UI_TEXTS.global.notApplicableShort;
        const bewertungStr = (key === 'auc') ? getAUCBewertung(data?.value) : ((key === 'phi') ? getPhiBewertung(data?.value) : '');
        const kollektivNameToUse = getKollektivDisplayName(kollektivName) || kollektivName || APP_CONFIG.UI_TEXTS.global.unknown;
        ...
        if (data?.n_trials !== undefined && data?.n_trials < ciWarningThreshold && (key === 'sens' || key === 'spez' || key === 'ppv' || key === 'npv' || key === 'acc')) {
            ciWarning = `<hr><i>${APP_CONFIG.UI_TEXTS.global.ciWarningSmallSampleSize.replace('[N_TRIALS]', data.n_trials)}</i>`;
            ...
            if (mc.total < ciWarningThreshold * 2 || mc.rp < ciWarningThreshold/2 || mc.fp < ciWarningThreshold/2 || mc.fn < ciWarningThreshold/2 || mc.rn < ciWarningThreshold/2 ) {
                 ciWarning = `<hr><i>${APP_CONFIG.UI_TEXTS.global.ciWarningConfusionMatrix.replace('[TOTAL]', mc.total)}</i>`;
            }
        }
        ...
        if (lowerStr === na || upperStr === na || ciMethodStr === APP_CONFIG.UI_TEXTS.global.notApplicableShort || !data?.ci) {
             interpretation = interpretation.replace(/\(95%-KI nach .*?: .*? – .*?\)/g, `(${APP_CONFIG.UI_TEXTS.global.noCIDataAvailable})`);
    ...
    function getTestInterpretationHTML(key, testData, kollektivName = '', t2ShortName = 'T2', lang = 'de') {
        const interpretationTemplate = APP_CONFIG.UI_TEXTS.TOOLTIP_CONTENT.statMetrics[key]?.interpretation || APP_CONFIG.UI_TEXTS.global.noInterpretationAvailable;
         if (!testData) return APP_CONFIG.UI_TEXTS.global.noInterpretationAvailable;
        const na = '--';
        const pValue = testData?.pValue;
        const pStr = getPValueText(pValue, lang, true);
        const sigSymbol = getStatisticalSignificanceSymbol(pValue);
        const sigText = getStatisticalSignificanceText(pValue, lang);
        const kollektivNameToUse = getKollektivDisplayName(kollektivName) || kollektivName || APP_CONFIG.UI_TEXTS.global.unknown;
    ...
    function getAssociationInterpretationHTML(key, assocObj, merkmalName, kollektivName, lang = 'de') {
        const interpretationTemplate = APP_CONFIG.UI_TEXTS.TOOLTIP_CONTENT.statMetrics[key]?.interpretation || APP_CONFIG.UI_TEXTS.global.noInterpretationAvailable;
        if (!assocObj) return APP_CONFIG.UI_TEXTS.global.noInterpretationAvailable;
        const na = '--';
        ...
        if(assocObj.matrix && (key === 'or' || key === 'rd' || key === 'phi')) {
            ...
            if (totalInMatrix < ciWarningThreshold * 2 || m.rp < ciWarningThreshold/2 || m.fp < ciWarningThreshold/2 || m.fn < ciWarningThreshold/2 || m.rn < ciWarningThreshold/2) {
                ciWarning = `<hr><i>${APP_CONFIG.UI_TEXTS.global.ciWarningConfusionMatrixForAssoc.replace('[TOTAL]', totalInMatrix)}</i>`;
            }
        }


        if (key === 'or') {
            valueStr = formatNumber(assocObj.or?.value, 2, na, true, lang);
            lowerStr = formatNumber(assocObj.or?.ci?.lower, 2, na, true, lang);
            upperStr = formatNumber(assocObj.or?.ci?.upper, 2, na, true, lang);
            ciMethodStr = assocObj.or?.method || APP_CONFIG.UI_TEXTS.global.notApplicableShort;
            pStr = getPValueText(assozPValue, lang, true);
            sigSymbol = getStatisticalSignificanceSymbol(assozPValue);
            sigText = getStatisticalSignificanceText(assozPValue, lang);
        } else if (key === 'rd') {
            valueStr = formatNumber(assocObj.rd?.value !== null && !isNaN(assocObj.rd?.value) ? assocObj.rd.value * 100 : NaN, 1, na, true, lang);
            lowerStr = formatNumber(assocObj.rd?.ci?.lower !== null && !isNaN(assocObj.rd?.ci?.lower) ? assocObj.rd.ci.lower * 100 : NaN, 1, na, true, lang);
            upperStr = formatNumber(assocObj.rd?.ci?.upper !== null && !isNaN(assocObj.rd?.ci?.upper) ? assocObj.rd.ci.upper * 100 : NaN, 1, na, true, lang);
            ciMethodStr = assocObj.rd?.method || APP_CONFIG.UI_TEXTS.global.notApplicableShort;
        } else if (key === 'phi') {
            valueStr = formatNumber(assocObj.phi?.value, 2, na, true, lang);
            bewertungStr = getPhiBewertung(assocObj.phi?.value);
             ...
             pStr = getPValueText(pVal, lang, true);
             sigSymbol = getStatisticalSignificanceSymbol(pVal);
             sigText = getStatisticalSignificanceText(pVal, lang);
             const templateToUse = APP_CONFIG.UI_TEXTS.TOOLTIP_CONTENT.statMetrics[key]?.interpretation || APP_CONFIG.UI_TEXTS.TOOLTIP_CONTENT.statMetrics.defaultP.interpretation;
        ...
        let interpretation = interpretationTemplate
            ...
            .replace(/\[METHOD_CI\]/g, `<em>${ciMethodStr}</em>`)
            .replace(/\[KOLLEKTIV\]/g, `<strong>${kollektivNameToUse}</strong>`)
            .replace(/\[FAKTOR_TEXT\]/g, assocObj?.or?.value > 1 ? APP_CONFIG.UI_TEXTS.statMetrics.orFaktorTexte.ERHOEHT : (assocObj?.or?.value < 1 ? APP_CONFIG.UI_TEXTS.statMetrics.orFaktorTexte.VERRINGERT : APP_CONFIG.UI_TEXTS.statMetrics.orFaktorTexte.UNVERAENDERT))
            .replace(/\[HOEHER_NIEDRIGER\]/g, assocObj?.rd?.value > 0 ? APP_CONFIG.UI_TEXTS.statMetrics.rdRichtungTexte.HOEHER : (assocObj?.rd?.value < 0 ? APP_CONFIG.UI_TEXTS.statMetrics.rdRichtungTexte.NIEDRIGER : APP_CONFIG.UI_TEXTS.statMetrics.rdRichtungTexte.GLEICH))
         ...
         if (key === 'or' || key === 'rd') {
            if (lowerStr === na || upperStr === na || ciMethodStr === APP_CONFIG.UI_TEXTS.global.notApplicableShort || !assocObj?.[key]?.ci) {
                interpretation = interpretation.replace(/\(95%-KI nach .*?: .*? – .*?\)/g, `(${APP_CONFIG.UI_TEXTS.global.noCIDataAvailable})`);
                interpretation = interpretation.replace(/nach \[METHOD_CI\]:/g, '');
            }
         }
