const uiViewLogic = (() => {
    ...
    function createDatenTableHTML(data, sortState) {
        ...
        if (data.length === 0) {
            tableHTML += `<tr><td colspan="${columns.length}" class="text-center text-muted">Keine Fälle im ausgewählten Kollektiv gefunden.</td></tr>`;
        } else {
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <span>Fallübersicht & Auswertungsergebnisse</span>
        ...
        let tableHTML = `
            <div class="row g-3 p-2">
                <div class="col-md-6">
                    ...
                    <p class="small text-muted mt-1 mb-0"><sup>*</sup> Nur bei N+ Fällen (n=${d.nStatus?.plus ?? 0}); <sup>**</sup> Nur bei AS+ Fällen (n=${d.asStatus?.plus ?? 0}); <sup>***</sup> Nur bei T2+ Fällen (n=${d.t2Status?.plus ?? 0}).</p>
                </div>
    ...
    function _createPresentationView_ASvsT2_HTML(presentationData, selectedStudyId = null, currentKollektiv = 'Gesamt') {
        ...
        if (canDisplayResults) {
            ...
            resultsHTML = `<div class="alert alert-warning">Keine Falldaten für Kollektiv (${kollektivName}) für Vergleich vorhanden.</div>`;
    ...
    function createPublikationTabContent(activeSection, lang, filteredData, appliedCriteria, appliedLogic, currentGlobalKollektiv) {
        if (typeof publikationUIComponents === 'undefined' || typeof publikationUIComponents.createPublikationNav !== 'function') {
            console.error("uiViewLogic: publikationUIComponents.createPublikationNav ist nicht verfügbar.");
        ...
        const contentContainerHTML = `<div id="publikation-section-content-area" class="mt-3">
                                          <p class="text-center p-5 text-muted">Lade Publikationsinhalte...</p>
                                      </div>`;
        return navHTML + contentContainerHTML;
    }
