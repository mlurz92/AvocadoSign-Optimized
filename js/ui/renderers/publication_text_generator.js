const publicationTextGenerator = (() => {
    ...
    function getPValueText(pValue, lang = 'de', placeholder = 'N/A') {
        ...
        return `p = ${fValue(pValue, 3, '', placeholder).replace('.', lang === 'de' ? ',' : '.')}`;
    }
    ...
    function getMethodenStudienanlageText(lang, commonData) {
        ...
        const ethikPlaceholder = "XYZ/2020, Ethikkommission der Sächsischen Landesärztekammer"; // Angepasster Platzhalter
        const originalStudyAuthors = "Lurz & Schäfer (2025)"; // Refers to Lurz_Schaefer_AvocadoSign_2025.pdf
    ...
    function getMethodenMRTProtokollText(lang, commonData) {
         if (lang === 'de') {
            return `
                <p>Alle MRT-Untersuchungen wurden an einem 3.0-Tesla-System (MAGNETOM Prisma Fit; Siemens Healthineers, Erlangen, Deutschland) unter Verwendung von Körper- und Wirbelsäulen-Array-Spulen durchgeführt. Das Bildgebungsprotokoll umfasste hochauflösende T2-gewichtete Turbo-Spin-Echo (TSE)-Sequenzen in sagittaler, axialer und koronarer Ebene sowie eine axiale diffusionsgewichtete Sequenz (DWI). Für die Bewertung des Avocado Signs wurde, wie in der Originalstudie beschrieben, eine kontrastmittelverstärkte axiale T1-gewichtete volumetrische interpolierte Breath-Hold-Sequenz (VIBE) mit Dixon-Fettunterdrückung akquiriert. Die genauen Sequenzparameter sind in der Originalpublikation zum Avocado Sign detailliert (Lurz & Schäfer, 2025).</p>
    ...
    function getMethodenT2DefinitionText(lang, commonData, publicationData, kollektiveData) {
        ...
        const bfMetricForPub = state.getCurrentPublikationBruteForceMetric() || PUBLICATION_CONFIG.defaultBruteForceMetricForPublication;

        const getBFDescription = (kollektivId, langSuffix) => {
            const bfData = kollektiveData?.[kollektivId]?.bruteforce_definition;
            if (bfData && bfData.criteria && bfData.metricName === bfMetricForPub) {
                const criteriaText = studyT2CriteriaManager.formatCriteriaForDisplay(bfData.criteria, bfData.logic);
                const metricValText = fValue(bfData.metricValue, 4);
                if (langSuffix === 'de') return `<li><strong>${getKollektivDisplayName(kollektivId)}:</strong> ${criteriaText} (Ziel: ${bfData.metricName}, Wert: ${metricValText})</li>`;
                return `<li><strong>${getKollektivDisplayName(kollektivId)}:</strong> ${criteriaText} (Target: ${bfData.metricName}, Value: ${metricValText})</li>`;
            }
            const fallbackText = langSuffix === 'de' ? `Keine Optimierungsergebnisse für Zielmetrik ${bfMetricForPub} verfügbar.` : `No optimization results available for target metric ${bfMetricForPub}.`;
            return `<li><strong>${getKollektivDisplayName(kollektivId)}:</strong> ${fallbackText}</li>`;
        };

        let bfCriteriaTextDe = '<ul>';
        bfCriteriaTextDe += getBFDescription('Gesamt', 'de');
        bfCriteriaTextDe += getBFDescription('direkt OP', 'de');
        bfCriteriaTextDe += getBFDescription('nRCT', 'de');
        bfCriteriaTextDe += '</ul>';

        let bfCriteriaTextEn = '<ul>';
        bfCriteriaTextEn += getBFDescription('Gesamt', 'en');
        bfCriteriaTextEn += getBFDescription('direkt OP', 'en');
        bfCriteriaTextEn += getBFDescription('nRCT', 'en');
        bfCriteriaTextEn += '</ul>';

        const kohDesc = studyT2CriteriaManager.getStudyCriteriaSetById('koh_2008_morphology')?.description || (lang === 'de' ? 'Irreguläre Kontur ODER heterogenes Signal' : 'Irregular border OR heterogeneous internal signal');
            ...
            return `
                ...
                <ol>
                    ...
                    <li><strong>Literatur-basierte T2-Kriteriensets:</strong>
                        <ul>
                            <li>Koh et al. (2008): "${kohDesc}". Diese Kriterien wurden für die vorliegende Analyse auf das Gesamtkollektiv angewendet.</li>
                            <li>Barbaro et al. (2024): "${barbaroDesc}". Diese Kriterien wurden auf die nRCT-Kohorte für das Restaging angewendet.</li>
                            <li>ESGAR Konsensus Kriterien (Beets-Tan et al., 2018), evaluiert durch Rutegård et al. (2025): "${esgarDesc}". Diese Kriterien wurden primär auf die Direkt-OP-Kohorte für das Primärstaging angewendet.</li>
                        </ul>
                    ...
                    <li><strong>Brute-Force optimierte T2-Kriterien:</strong> Für jedes Patientenkollektiv (Gesamt, Direkt OP, nRCT) wurden mittels eines integrierten Brute-Force-Algorithmus der Analyseanwendung diejenigen T2-Kriterien (Kombination aus Größe [Bereich ${commonData.t2SizeMin?.toFixed(1)}-${commonData.t2SizeMax?.toFixed(1)}mm, Schritt ${commonData.t2SizeStep?.toFixed(1)}mm], Form, Kontur, Homogenität, Signal und UND/ODER-Logik) identifiziert, welche die Zielmetrik '${bfMetricForPub}' maximieren. Die resultierenden optimierten Kriterien für die jeweilige Kohorte waren:
                        ${bfCriteriaTextDe}
            ...
            return `
                ...
                <ol>
                    ...
                    <li><strong>Literature-based T2 criteria sets:</strong>
                        <ul>
                            <li>Koh et al. (2008): "${kohDesc}". These criteria were applied to the overall cohort for the present analysis.</li>
                            <li>Barbaro et al. (2024): "${barbaroDesc}". These criteria were applied to the nRCT cohort for restaging.</li>
                            <li>ESGAR Consensus Criteria (Beets-Tan et al., 2018), as evaluated by Rutegård et al. (2025): "${esgarDesc}". These criteria were primarily applied to the upfront surgery cohort for primary staging.</li>
                        </ul>
                        A detailed overview of these literature-based criteria and their specific application contexts is provided in Table 2.
                    </li>
                    <li><strong>Brute-force optimized T2 criteria:</strong> For each patient cohort (Overall, Upfront Surgery, nRCT), T2 criteria (combination of size [range ${commonData.t2SizeMin?.toFixed(1)}-${commonData.t2SizeMax?.toFixed(1)}mm, step ${commonData.t2SizeStep?.toFixed(1)}mm], shape, border, homogeneity, signal, and AND/OR logic) that maximize the target metric '${bfMetricForPub}' were identified using an integrated brute-force algorithm of the analysis application. The resulting optimized criteria for each cohort were:
                        ${bfCriteriaTextEn}
    ...
    function getErgebnisseLiteraturT2PerformanceText(lang, publicationData, commonData) {
        ...
        if (lang === 'de') {
            ...
            text += `Für das Kriterienset nach Koh et al. (2008), angewendet auf das Gesamtkollektiv (N=${commonData.nGesamt}), ergab sich eine Sensitivität von ${fCI(kohData?.sens, 1, true, 'de')}, eine Spezifität von ${fCI(kohData?.spez, 1, true, 'de')} und eine AUC von ${fCI(kohData?.auc, 3, false, 'de')}. `;
            text += `Die Kriterien nach Barbaro et al. (2024), angewendet auf die nRCT-Kohorte (N=${commonData.nNRCT}), zeigten eine Sensitivität von ${fCI(barbaroData?.sens, 1, true, 'de')}, eine Spezifität von ${fCI(barbaroData?.spez, 1, true, 'de')} und eine AUC von ${fCI(barbaroData?.auc, 3, false, 'de')}. `;
            text += `Die ESGAR 2016 Kriterien (evaluiert durch Rutegård et al., 2025), angewendet auf die Direkt-OP-Kohorte (N=${commonData.nDirektOP}), erreichten eine Sensitivität von ${fCI(esgarData?.sens, 1, true, 'de')}, eine Spezifität von ${fCI(esgarData?.spez, 1, true, 'de')} und eine AUC von ${fCI(esgarData?.auc, 3, false, 'de')}.</p>`;
        } else {
            ...
            text += `For the criteria set according to Koh et al. (2008), applied to the overall cohort (n=${commonData.nGesamt}), a sensitivity of ${fCI(kohData?.sens, 1, true, 'en')}, a specificity of ${fCI(kohData?.spez, 1, true, 'en')}, and an AUC of ${fCI(kohData?.auc, 3, false, 'en')} were observed. `;
            text += `The criteria by Barbaro et al. (2024), applied to the nRCT cohort (n=${commonData.nNRCT}), showed a sensitivity of ${fCI(barbaroData?.sens, 1, true, 'en')}, a specificity of ${fCI(barbaroData?.spez, 1, true, 'en')}, and an AUC of ${fCI(barbaroData?.auc, 3, false, 'en')}. `;
            text += `The ESGAR 2016 criteria (evaluated by Rutegård et al., 2025), applied to the upfront surgery cohort (n=${commonData.nDirektOP}), achieved a sensitivity of ${fCI(esgarData?.sens, 1, true, 'en')}, a specificity of ${fCI(esgarData?.spez, 1, true, 'en')}, and an AUC of ${fCI(esgarData?.auc, 3, false, 'en')}.</p>`;
        }
        return text;
    }
    ...
    function getErgebnisseOptimierteT2PerformanceText(lang, publicationData, commonData) {
        ...
        const formatBFDef = (def) => def ? studyT2CriteriaManager.formatCriteriaForDisplay(def.criteria, def.logic) : 'N/A';

        if (lang === 'de') {
            return `
                <p>Mittels Brute-Force-Optimierung wurden für jedes Kollektiv T2-Kriteriensets identifiziert, die die Zielmetrik '${bfMetricForPub}' maximieren (detailliert in Tabelle 5). Für das Gesamtkollektiv (N=${commonData.nGesamt}) wurde mit den optimierten Kriterien "${formatBFDef(bfGesamtDef)}" eine Sensitivität von ${fCI(bfGesamtStats?.sens, 1, true, 'de')}, eine Spezifität von ${fCI(bfGesamtStats?.spez, 1, true, 'de')} und eine AUC von ${fCI(bfGesamtStats?.auc, 3, false, 'de')} erreicht (Zielmetrik '${bfMetricForPub}': ${fValue(bfGesamtDef?.metricValue,4)}). Für die Direkt-OP-Gruppe (N=${commonData.nDirektOP}) lauteten die optimierten Kriterien "${formatBFDef(bfDirektOPDef)}" und erzielten eine AUC von ${fCI(bfDirektOPStats?.auc, 3, false, 'de')} (Zielmetrik '${bfMetricForPub}': ${fValue(bfDirektOPDef?.metricValue,4)}). In der nRCT-Gruppe (N=${commonData.nNRCT}) waren die optimierten Kriterien "${formatBFDef(bfNRCTDef)}" mit einer AUC von ${fCI(bfNRCTStats?.auc, 3, false, 'de')} (Zielmetrik '${bfMetricForPub}': ${fValue(bfNRCTDef?.metricValue,4)}).</p>
            `;
        } else {
            return `
                <p>Using brute-force optimization, T2 criteria sets that maximize the target metric '${bfMetricForPub}' were identified for each cohort (detailed in Table 5). For the overall cohort (n=${commonData.nGesamt}), the optimized criteria "${formatBFDef(bfGesamtDef)}" achieved a sensitivity of ${fCI(bfGesamtStats?.sens, 1, true, 'en')}, a specificity of ${fCI(bfGesamtStats?.spez, 1, true, 'en')}, and an AUC of ${fCI(bfGesamtStats?.auc, 3, false, 'en')} (Target metric '${bfMetricForPub}': ${fValue(bfGesamtDef?.metricValue,4)}). For the upfront surgery group (n=${commonData.nDirektOP}), the optimized criteria were "${formatBFDef(bfDirektOPDef)}", achieving an AUC of ${fCI(bfDirektOPStats?.auc, 3, false, 'en')} (Target metric '${bfMetricForPub}': ${fValue(bfDirektOPDef?.metricValue,4)}). In the nRCT group (n=${commonData.nNRCT}), the optimized criteria were "${formatBFDef(bfNRCTDef)}" with an AUC of ${fCI(bfNRCTStats?.auc, 3, false, 'en')} (Target metric '${bfMetricForPub}': ${fValue(bfNRCTDef?.metricValue,4)}).</p>
            `;
        }
    }
    ...
    function getErgebnisseVergleichPerformanceText(lang, publicationData, commonData) {
        ...
        if (lang === 'de') {
            return `
                <p>Im direkten statistischen Vergleich zwischen dem Avocado Sign und den global angewandten T2-Kriterien im Gesamtkollektiv (N=${commonData.nGesamt}) zeigte sich für die Accuracy ein ${pMcNemarGesamt !== null && !isNaN(pMcNemarGesamt) ? ( getStatisticalSignificanceText(pMcNemarGesamt, commonData.significanceLevel) + " Unterschied (McNemar-Test: " + getPValueText(pMcNemarGesamt, 'de') + ")") : "nicht evaluierbarer Unterschied (McNemar-Test: N/A)"}. Für die AUC betrug der Unterschied ${pDeLongGesamt !== null && !isNaN(pDeLongGesamt) ? (getStatisticalSignificanceText(pDeLongGesamt, commonData.significanceLevel) + " (DeLong-Test: " + getPValueText(pDeLongGesamt, 'de') + ")") : "nicht evaluierbar (DeLong-Test: N/A)"}. Die Abbildungen 2a und 2b visualisieren exemplarisch die ROC-Kurven bzw. einen Metrikvergleich für das aktuell im Analyse-Tool ausgewählte Kollektiv (${commonData.currentKollektivName}). Ausführliche paarweise Vergleiche der diagnostischen Güte zwischen dem Avocado Sign und den verschiedenen T2-Kriteriensets (angewandt, Literatur-basiert, optimiert) für alle drei Kollektive (Gesamt, Direkt OP, nRCT) sind in Tabelle 6 dargestellt.</p>
            `;
        } else {
             return `
                <p>In the direct statistical comparison between the Avocado Sign and the globally applied T2 criteria in the overall cohort (n=${commonData.nGesamt}), the difference in accuracy was ${pMcNemarGesamt !== null && !isNaN(pMcNemarGesamt) ? (getStatisticalSignificanceText(pMcNemarGesamt, commonData.significanceLevel) + " (McNemar's test: " + getPValueText(pMcNemarGesamt, 'en') + ")") : "not evaluable (McNemar's test: N/A)"}. For the AUC, the difference was ${pDeLongGesamt !== null && !isNaN(pDeLongGesamt) ? (getStatisticalSignificanceText(pDeLongGesamt, commonData.significanceLevel) + " (DeLong's test: " + getPValueText(pDeLongGesamt, 'en') + ")") : "not evaluable (DeLong's test: N/A)"}. Figures 2a and 2b exemplarily visualize the ROC curves and a metric comparison, respectively, for the cohort currently selected in the analysis tool (${commonData.currentKollektivName}). Detailed pairwise comparisons of diagnostic performance between the Avocado Sign and the various T2 criteria sets (applied, literature-based, optimized) for all three cohorts (Overall, Upfront Surgery, nRCT) are presented in Table 6.</p>
            `;
        }
    }
