const chartRenderer = (() => {
    ...
    const NEW_PRIMARY_COLOR_BLUE = APP_CONFIG.CHART_SETTINGS.NEW_PRIMARY_COLOR_BLUE;
    const NEW_SECONDARY_COLOR_YELLOW_GREEN = APP_CONFIG.CHART_SETTINGS.NEW_SECONDARY_COLOR_YELLOW_GREEN;
    const DEFAULT_COLOR_SCHEME = APP_CONFIG.CHART_SETTINGS.COLOR_SCHEMES?.default || [NEW_PRIMARY_COLOR_BLUE, NEW_SECONDARY_COLOR_YELLOW_GREEN, APP_CONFIG.CHART_SETTINGS.TERTIARY_COLOR_GREEN, '#d62728', '#9467bd', '#8c564b'];
    const PLOT_BACKGROUND_COLOR = APP_CONFIG.CHART_SETTINGS.PLOT_BACKGROUND_COLOR;
    const ANIMATION_DURATION_MS = APP_CONFIG.CHART_SETTINGS.ANIMATION_DURATION_MS;
    const TICK_LABEL_FONT_SIZE = APP_CONFIG.CHART_SETTINGS.TICK_LABEL_FONT_SIZE;
    const AXIS_LABEL_FONT_SIZE = APP_CONFIG.CHART_SETTINGS.AXIS_LABEL_FONT_SIZE;
    const LEGEND_FONT_SIZE = APP_CONFIG.CHART_SETTINGS.LEGEND_FONT_SIZE;
    const TOOLTIP_FONT_SIZE = APP_CONFIG.CHART_SETTINGS.TOOLTIP_FONT_SIZE;
    const ENABLE_GRIDLINES = APP_CONFIG.CHART_SETTINGS.ENABLE_GRIDLINES;
    const LINE_STROKE_WIDTH = APP_CONFIG.CHART_SETTINGS.LINE_STROKE_WIDTH;
    const POINT_RADIUS = APP_CONFIG.CHART_SETTINGS.POINT_RADIUS;
    const TRANSITION_DURATION_MS = APP_CONFIG.UI_SETTINGS.TRANSITION_DURATION_MS;
    const CHART_FONT_FAMILY = APP_CONFIG.CHART_SETTINGS.FONT_FAMILY_CHART;
    ...
    function createSvgContainer(targetElementId, options = {}) {
        ...
        if (innerWidth <= 20 || innerHeight <= 20) {
            container.html(`<p class="text-muted small text-center p-2">${APP_CONFIG.UI_TEXTS.global.chartTooSmall}</p>`);
            return null;
        }
        ...
        const svg = container.append("svg")
            ...
            .style("font-family", CHART_FONT_FAMILY).style("overflow", "visible");
    ...
    function renderAgeDistributionChart(ageData, targetElementId, options = {}, lang = 'de') {
        ...
        if (!Array.isArray(ageData) || ageData.length === 0) { chartArea.append('text').attr('x', innerWidth / 2).attr('y', innerHeight / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text(APP_CONFIG.UI_TEXTS.global.noAgeDataAvailable); return; }
        ...
        svg.append("text").attr("class", "axis-label x-axis-label").attr("text-anchor", "middle").attr("x", margin.left + innerWidth / 2).attr("y", height - 5).style("font-size", AXIS_LABEL_FONT_SIZE).text(APP_CONFIG.UI_TEXTS.axisLabels.age);
        ...
        svg.append("text").attr("class", "axis-label y-axis-label").attr("text-anchor", "middle").attr("transform", `translate(${margin.left / 2 - 5}, ${margin.top + innerHeight / 2}) rotate(-90)`).style("font-size", AXIS_LABEL_FONT_SIZE).text(APP_CONFIG.UI_TEXTS.axisLabels.patientCount);
        ...
        chartArea.selectAll(".bar").data(bins).join("rect").attr("class", "bar").attr("x", d => x(d.x0) + 1).attr("y", y(0)).attr("width", d => Math.max(0, x(d.x1) - x(d.x0) - 1)).attr("height", 0).style("fill", barColor).style("opacity", 0.8).attr("rx", 1).attr("ry", 1)
            .on("mouseover", (event, d) => { tooltip.transition().duration(50).style("opacity", .95); tooltip.html(`${APP_CONFIG.UI_TEXTS.axisLabels.age}: ${formatNumber(d.x0, 0)}-${formatNumber(d.x1, 0)}<br>${APP_CONFIG.UI_TEXTS.axisLabels.count}: ${d.length}`).style("left", (event.pageX + 10) + "px").style("top", (event.pageY - 15) + "px"); d3.select(event.currentTarget).style("opacity", 1).style("stroke", "#333").style("stroke-width", 0.5); })
    ...
    function renderPieChart(data, targetElementId, options = {}) {
        ...
        if (validData.length === 0 || totalValue <= 0) { chartArea.append('text').attr('x', innerWidth / 2).attr('y', innerHeight / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text(APP_CONFIG.UI_TEXTS.global.noData); return; }
        ...
        if (outerRadius <= innerRadius || outerRadius <= 0) { chartArea.append('text').attr('x', plottingWidth / 2).attr('y', plottingHeight / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text(APP_CONFIG.UI_TEXTS.global.chartTooSmall); return; }
        ...
        if (setupOptions.legendBelow && legendSpaceY > 0 && validData.length > 0) {
            ...
            const legendGroup = svg.append("g").attr("class", "legend pie-legend").attr("transform", `translate(${margin.left}, ${margin.top + plottingHeight + 15})`).attr("font-family", CHART_FONT_FAMILY).attr("font-size", LEGEND_FONT_SIZE).attr("text-anchor", "start");
    ...
    function renderComparisonBarChart(chartData, targetElementId, options = {}, t2Label = APP_CONFIG.UI_TEXTS.legendLabels.currentT2) {
         ...
         if (!Array.isArray(chartData) || chartData.length === 0) { chartArea.append('text').attr('x', innerWidth / 2).attr('y', innerHeight / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text(APP_CONFIG.UI_TEXTS.global.noComparisonData); return; }
         ...
         svg.append("text").attr("class", "axis-label y-axis-label").attr("text-anchor", "middle").attr("transform", `translate(${margin.left / 2}, ${margin.top + innerHeight / 2}) rotate(-90)`).style("font-size", AXIS_LABEL_FONT_SIZE).text(APP_CONFIG.UI_TEXTS.axisLabels.metricValue);
         ...
         const legendGroup = svg.append("g").attr("class", "legend bar-legend").attr("font-family", CHART_FONT_FAMILY).attr("font-size", LEGEND_FONT_SIZE).attr("text-anchor", "start"); const legendItems = legendGroup.selectAll("g.legend-item").data(subgroups).join("g").attr("class", "legend-item").attr("data-subgroup", d => d);
    ...
    function renderASPerformanceChart(targetElementId, performanceData, options = {}, kollektivName = '') {
        ...
        const containerSetup = createSvgContainer(targetElementId, setupOptions); if (!containerSetup) return;
        const { svg, chartArea, innerWidth, innerHeight, width, height, margin } = containerSetup; const tooltip = createTooltip();
        const metrics = [APP_CONFIG.UI_TEXTS.t2MetricsOverview.sensShort, APP_CONFIG.UI_TEXTS.t2MetricsOverview.spezShort, APP_CONFIG.UI_TEXTS.t2MetricsOverview.ppvShort, APP_CONFIG.UI_TEXTS.t2MetricsOverview.npvShort, APP_CONFIG.UI_TEXTS.t2MetricsOverview.accShort, APP_CONFIG.UI_TEXTS.t2MetricsOverview.aucShort];
        const metricKeys = ['sens', 'spez', 'ppv', 'npv', 'acc', 'auc']; // Actual data keys
        const cohortKey = 'overall'; const cohortPerf = performanceData ? performanceData[cohortKey] : null;
        if (!cohortPerf) { chartArea.append('text').attr('x', innerWidth / 2).attr('y', innerHeight / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text(APP_CONFIG.UI_TEXTS.global.performanceDataNotAvailable); return; }
        
        const chartData = metricKeys.map((key, index) => { 
            let value = NaN;
            if (cohortPerf[key + 'Val'] !== undefined) value = cohortPerf[key + 'Val']; 
            else if (cohortPerf[key] !== undefined && cohortPerf[key] !== null) value = cohortPerf[key]?.value ?? NaN; 
            return { metric: metrics[index], value: value, originalKey: key }; // Store original key for tooltip
        }).filter(d => d.value !== undefined && !isNaN(d.value));

        if (chartData.length === 0) { chartArea.append('text').attr('x', innerWidth / 2).attr('y', innerHeight / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text(APP_CONFIG.UI_TEXTS.global.incompleteData); return; }
        ...
        const tickCountY = 5; chartArea.append("g").attr("class", "y-axis axis").call(d3.axisLeft(y).ticks(tickCountY, "%").tickSizeOuter(0)).selectAll("text").style("font-size", TICK_LABEL_FONT_SIZE);
        svg.append("text").attr("class", "axis-label y-axis-label").attr("text-anchor", "middle").attr("transform", `translate(${margin.left / 2 - 10}, ${margin.top + innerHeight / 2}) rotate(-90)`).style("font-size", AXIS_LABEL_FONT_SIZE).text(APP_CONFIG.UI_TEXTS.axisLabels.diagnosticQualityAS);
        ...
        chartArea.selectAll(".bar").data(chartData).join("rect").attr("class", "bar").attr("x", d => x(d.metric)).attr("y", y(0)).attr("width", x.bandwidth()).attr("height", 0).attr("fill", APP_CONFIG.CHART_SETTINGS.AS_COLOR || NEW_PRIMARY_COLOR_BLUE).style("opacity", 0.9).attr("rx", 1).attr("ry", 1)
           .on("mouseover", function(event, d) { tooltip.transition().duration(50).style("opacity", .95); const digits = (d.originalKey === 'auc' || d.originalKey === 'f1') ? 3 : 1; const isPercent = !(d.originalKey === 'auc' || d.originalKey === 'f1'); const formattedValue = isPercent ? formatPercent(d.value, digits) : formatNumber(d.value, digits); const metricName = APP_CONFIG.UI_TEXTS.TOOLTIP_CONTENT.statMetrics[d.originalKey]?.name || d.metric; tooltip.html(`<strong>${metricName}:</strong> ${formattedValue}`).style("left", (event.pageX + 10) + "px").style("top", (event.pageY - 15) + "px"); d3.select(this).style("opacity", 1).style("stroke", "#333").style("stroke-width", 1); })
           .on("mouseout", function(event, d) { tooltip.transition().duration(200).style("opacity", 0); d3.select(this).style("opacity", 0.9).style("stroke", "none"); })
           .transition().duration(ANIMATION_DURATION_MS).ease(d3.easeCubicOut).attr("y", d => y(d.value ?? 0)).attr("height", d => Math.max(0, innerHeight - y(d.value ?? 0)));
        svg.append("text").attr("x", width / 2).attr("y", margin.top - 10).attr("text-anchor", "middle").style("font-size", "12px").style("font-weight", "bold").text(`${APP_CONFIG.UI_TEXTS.chartTitles.asPerformanceForCollective} ${kollektivName}`);
    }
    ...
    function renderROCCurve(rocData, targetElementId, options = {}) {
        ...
        if (validRocData.length < 2) { chartArea.append('text').attr('x', innerWidth / 2).attr('y', innerHeight / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text(APP_CONFIG.UI_TEXTS.global.notEnoughDataForROC); return; }
        ...
        svg.append("text").attr("class", "axis-label x-axis-label").attr("text-anchor", "middle").attr("x", margin.left + innerWidth / 2).attr("y", height - 5).style("font-size", AXIS_LABEL_FONT_SIZE).text(APP_CONFIG.UI_TEXTS.axisLabels.oneMinusSpecificity);
        ...
        svg.append("text").attr("class", "axis-label y-axis-label").attr("text-anchor", "middle").attr("transform", `translate(${margin.left / 2 - 10}, ${margin.top + innerHeight / 2}) rotate(-90)`).style("font-size", AXIS_LABEL_FONT_SIZE).text(APP_CONFIG.UI_TEXTS.axisLabels.sensitivity);
