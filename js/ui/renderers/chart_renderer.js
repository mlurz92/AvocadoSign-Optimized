const chartRenderer = (() => {
    ...
    function _getLang() {
        return (typeof state !== 'undefined' && state.getCurrentPublikationLang && state.getActiveTabId && state.getActiveTabId() === 'publikation-tab') ? state.getCurrentPublikationLang() : 'de';
    }

    function _getText(textObj, lang) {
        if (typeof textObj === 'string') return textObj;
        if (textObj && typeof textObj === 'object' && textObj[lang]) return textObj[lang];
        if (textObj && typeof textObj === 'object' && textObj['de']) return textObj['de']; // Fallback to German
        return 'N/A Text';
    }
    ...
    function createSvgContainer(targetElementId, options = {}) {
        ...
        if (innerWidth <= 20 || innerHeight <= 20) {
            container.html(`<p class="text-muted small text-center p-2">${_getText(UI_TEXTS.publikationTab.publicationMisc.noValidData, _getLang())}</p>`);
    ...
    function renderAgeDistributionChart(ageData, targetElementId, options = {}) {
        ...
        if (!Array.isArray(ageData) || ageData.length === 0) { chartArea.append('text').attr('x', innerWidth / 2).attr('y', innerHeight / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text(_getText(UI_TEXTS.publikationTab.publicationMisc.noData, lang)); return; }
        ...
        const xAxisLabelText = (options.xAxisLabelKey && UI_TEXTS.axisLabels[options.xAxisLabelKey]) ? _getText(UI_TEXTS.axisLabels[options.xAxisLabelKey], lang) : _getText(UI_TEXTS.axisLabels.age, lang);
        ...
        const yAxisLabelText = (options.yAxisLabelKey && UI_TEXTS.axisLabels[options.yAxisLabelKey]) ? _getText(UI_TEXTS.axisLabels[options.yAxisLabelKey], lang) : _getText(UI_TEXTS.axisLabels.patientCount, lang);
        ...
        chartArea.selectAll(".bar").data(bins).join("rect").attr("class", "bar").attr("x", d => x(d.x0) + 1).attr("y", y(0)).attr("width", d => Math.max(0, x(d.x1) - x(d.x0) - 1)).attr("height", 0).style("fill", barColor).style("opacity", 0.8).attr("rx", 1).attr("ry", 1)
            .on("mouseover", (event, d) => { tooltip.transition().duration(50).style("opacity", .95); tooltip.html(`${xAxisLabelText}: ${formatNumber(d.x0, 0)}-${formatNumber(d.x1, 0)}<br>${yAxisLabelText}: ${d.length}`).style("left", (event.pageX + 10) + "px").style("top", (event.pageY - 15) + "px"); d3.select(event.currentTarget).style("opacity", 1).style("stroke", "#333").style("stroke-width", 0.5); })
    ...
    function renderPieChart(data, targetElementId, options = {}) {
        ...
        if (validData.length === 0 || totalValue <= 0) { chartArea.append('text').attr('x', innerWidth / 2).attr('y', innerHeight / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text(_getText(UI_TEXTS.publikationTab.publicationMisc.noData, lang)); return; }
        ...
        if (outerRadius <= innerRadius || outerRadius <= 0) { chartArea.append('text').attr('x', plottingWidth / 2).attr('y', plottingHeight / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text(_getText(UI_TEXTS.publikationTab.publicationMisc.noValidData, lang)); return; }
    ...
    function renderComparisonBarChart(chartData, targetElementId, options = {}, t2Label = 'T2') {
        ...
        if (!Array.isArray(chartData) || chartData.length === 0) { chartArea.append('text').attr('x', innerWidth / 2).attr('y', innerHeight / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text(_getText(UI_TEXTS.publikationTab.publicationMisc.noData, lang)); return; }
        ...
        const getLegendLabel = (key) => {
            if (key === 'AS') return _getText(UI_TEXTS.legendLabels.avocadoSign, lang);
            if (key === 'T2') return t2Label; // t2Label is already passed sprachabhängig
            return key;
        };
        ...
        const yAxisLabelText = (options.yAxisLabelKey && UI_TEXTS.axisLabels[options.yAxisLabelKey]) ? _getText(UI_TEXTS.axisLabels[options.yAxisLabelKey], lang) : _getText(UI_TEXTS.axisLabels.metricValue, lang);
        ...
        metricGroup.selectAll("rect").data(d => subgroups.map(key => ({key: key, value: d[key], metric: d.metric}))).join("rect").attr("class", d => `bar bar-${d.key}`).attr("x", d => x1(d.key)).attr("y", y(0)).attr("width", x1.bandwidth()).attr("height", 0).attr("fill", d => color(d.key)).style("opacity", 0.9).attr("rx", 1).attr("ry", 1)
            .on("mouseover", function(event, d) { tooltip.transition().duration(50).style("opacity", .95); const displayName = getLegendLabel(d.key); const digits = (d.metric === 'AUC' || d.metric === 'F1') ? 3 : 1; const isPercent = !(d.metric === 'AUC' || d.metric === 'F1'); const formattedValue = isPercent ? formatPercent(d.value, digits) : formatNumber(d.value, digits); tooltip.html(`<strong>${d.metric} (${displayName}):</strong> ${formattedValue}`).style("left", (event.pageX + 10) + "px").style("top", (event.pageY - 15) + "px"); d3.select(this).style("opacity", 1).style("stroke", "#333").style("stroke-width", 1); })
    ...
    function renderASPerformanceChart(targetElementId, performanceData, options = {}, kollektivName = '') {
        ...
        if (!cohortPerf) { chartArea.append('text').attr('x', innerWidth / 2).attr('y', innerHeight / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text(_getText(UI_TEXTS.publikationTab.publicationMisc.noData, lang)); return; }
        ...
        if (chartData.length === 0) { chartArea.append('text').attr('x', innerWidth / 2).attr('y', innerHeight / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text(_getText(UI_TEXTS.publikationTab.publicationMisc.noValidData, lang)); return; }
        ...
        const yAxisLabelText = (options.yAxisLabelKey && UI_TEXTS.axisLabels[options.yAxisLabelKey]) ? _getText(UI_TEXTS.axisLabels[options.yAxisLabelKey], lang) : `${_getText(UI_TEXTS.axisLabels.metricValue, lang)} (AS)`;
        ...
        const chartTitleText = options.chartTitle || _getText(UI_TEXTS.chartTitles.asPerformance, lang).replace('{Kollektiv}', kollektivName);
    ...
    function renderROCCurve(rocData, targetElementId, options = {}) {
        ...
        if (validRocData.length < 2) { chartArea.append('text').attr('x', innerWidth / 2).attr('y', innerHeight / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text(_getText(UI_TEXTS.publikationTab.publicationMisc.noData, lang) + ' (ROC)'); return; }
        ...
        const xAxisLabelText = (options.xAxisLabelKey && UI_TEXTS.axisLabels[options.xAxisLabelKey]) ? _getText(UI_TEXTS.axisLabels[options.xAxisLabelKey], lang) : _getText(UI_TEXTS.axisLabels.oneMinusSpecificity, lang);
        svg.append("text").attr("class", "axis-label x-axis-label").attr("text-anchor", "middle").attr("x", margin.left + innerWidth / 2).attr("y", height - 5).style("font-size", AXIS_LABEL_FONT_SIZE).text(xAxisLabelText);
        ...
        const yAxisLabelText = (options.yAxisLabelKey && UI_TEXTS.axisLabels[options.yAxisLabelKey]) ? _getText(UI_TEXTS.axisLabels[options.yAxisLabelKey], lang) : _getText(UI_TEXTS.axisLabels.sensitivity, lang);
        svg.append("text").attr("class", "axis-label y-axis-label").attr("text-anchor", "middle").attr("transform", `translate(${margin.left / 2 - 10}, ${margin.top + innerHeight / 2}) rotate(-90)`).style("font-size", AXIS_LABEL_FONT_SIZE).text(yAxisLabelText);
        ...
        if (options.showPoints) { chartArea.selectAll(".roc-point").data(validRocData.filter((d, i) => i > 0 && i < validRocData.length - 1)).join("circle").attr("class", "roc-point").attr("cx", d => xScale(d.fpr)).attr("cy", d => yScale(d.tpr)).attr("r", POINT_RADIUS / 1.5).attr("fill", lineColor).style("opacity", 0)
         ...
         if (options.aucValue !== undefined && !isNaN(options.aucValue)) {
             const aucLabelText = _getText(UI_TEXTS.statMetrics.auc.name, lang) || 'AUC';
             const aucText = `${aucLabelText}: ${formatCI(options.aucValue, options.aucCI?.lower, options.aucCI?.upper, 3, false, '--', lang)}`;
             chartArea.append("text").attr("class", "auc-label").attr("x", innerWidth - 10).attr("y", innerHeight - 10).attr("text-anchor", "end").style("font-size", AXIS_LABEL_FONT_SIZE).style("font-weight", "bold").text(aucText);
         }
    }
