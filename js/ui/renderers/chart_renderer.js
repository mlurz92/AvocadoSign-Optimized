const chartRenderer = (() => {
    ...
    function createSvgContainer(targetElementId, options = {}) {
        ...
        if (innerWidth <= 20 || innerHeight <= 20) {
            ...
            const errorText = langKey === 'de' ? 'Diagrammgröße ungültig/zu klein.' : 'Chart size invalid/too small.';
            container.html(`<p class="text-muted small text-center p-2">${errorText}</p>`);
    ...
    function renderAgeDistributionChart(ageData, targetElementId, options = {}, lang = 'de') {
        ...
        if (!Array.isArray(ageData) || ageData.length === 0) {
            const noDataText = langKey === 'de' ? 'Keine Altersdaten verfügbar.' : 'No age data available.';
            chartArea.append('text').attr('x', innerWidth / 2).attr('y', innerHeight / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text(noDataText); return;
        }
        ...
        svg.append("text").attr("class", "axis-label x-axis-label").attr("text-anchor", "middle").attr("x", margin.left + innerWidth / 2).attr("y", height - 5).style("font-size", AXIS_LABEL_FONT_SIZE).text(UI_TEXTS.axisLabels.age[langKey] || UI_TEXTS.axisLabels.age.de);
        ...
        svg.append("text").attr("class", "axis-label y-axis-label").attr("text-anchor", "middle").attr("transform", `translate(${margin.left / 2 - 5}, ${margin.top + innerHeight / 2}) rotate(-90)`).style("font-size", AXIS_LABEL_FONT_SIZE).text(UI_TEXTS.axisLabels.patientCount[langKey] || UI_TEXTS.axisLabels.patientCount.de);
        ...
        chartArea.selectAll(".bar").data(bins).join("rect").attr("class", "bar").attr("x", d => x(d.x0) + 1).attr("y", y(0)).attr("width", d => Math.max(0, x(d.x1) - x(d.x0) - 1)).attr("height", 0).style("fill", barColor).style("opacity", 0.8).attr("rx", 1).attr("ry", 1)
            .on("mouseover", (event, d) => { tooltip.transition().duration(50).style("opacity", .95); const ageRangeText = langKey === 'de' ? 'Alter' : 'Age'; const countText = langKey === 'de' ? 'Anzahl' : 'Count'; tooltip.html(`${ageRangeText}: ${formatNumber(d.x0, 0, '--', false, langKey)}-${formatNumber(d.x1, 0, '--', false, langKey)}<br>${countText}: ${d.length}`).style("left", (event.pageX + 10) + "px").style("top", (event.pageY - 15) + "px"); d3.select(event.currentTarget).style("opacity", 1).style("stroke", "#333").style("stroke-width", 0.5); })
    ...
    function renderPieChart(data, targetElementId, options = {}, lang = 'de') {
        ...
        if (validData.length === 0 || totalValue <= 0) {
            const noDataText = langKey === 'de' ? 'Keine Daten.' : 'No data.';
            chartArea.append('text').attr('x', innerWidth / 2).attr('y', innerHeight / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text(noDataText); return;
        }
        ...
        if (outerRadius <= innerRadius || outerRadius <= 0) {
            const tooSmallText = langKey === 'de' ? 'Zu klein' : 'Too small';
            chartArea.append('text').attr('x', plottingWidth / 2).attr('y', plottingHeight / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text(tooSmallText); return;
        }
        ...
        arcs.append("path").style("fill", d => color(d.data.label)).style("stroke", PLOT_BACKGROUND_COLOR).style("stroke-width", "1.5px").style("opacity", 0.85)
            .on("mouseover", function(event, d) { tooltip.transition().duration(50).style("opacity", .95); tooltip.html(`<strong>${d.data.label}:</strong> ${formatNumber(d.data.value, 0, '--', false, langKey)} (${formatPercent(d.data.value / totalValue, 1, '--%', langKey)})`).style("left", (event.pageX + 10) + "px").style("top", (event.pageY - 15) + "px"); d3.select(this).transition().duration(100).style("opacity", 1).attr("transform", "scale(1.03)"); })
        ...
        if (setupOptions.legendBelow && legendSpaceY > 0 && validData.length > 0) {
            ...
            legendItems.each(function(d, i) {
                ...
                item.append("text").attr("x", 14).attr("y", 5).attr("dy", "0.35em").text(`${d.label} (${formatNumber(d.value, 0, '--', false, langKey)})`);
    ...
    function renderComparisonBarChart(chartData, targetElementId, options = {}, t2Label = 'T2', lang = 'de') {
         ...
         if (!Array.isArray(chartData) || chartData.length === 0) {
             const noDataText = langKey === 'de' ? 'Keine Vergleichsdaten.' : 'No comparison data.';
             chartArea.append('text').attr('x', innerWidth / 2).attr('y', innerHeight / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text(noDataText); return;
        }
         const groups = chartData.map(d => d.metric); const subgroups = Object.keys(chartData[0]).filter(key => key !== 'metric');
         const subgroupDisplayNames = { 'AS': (UI_TEXTS.legendLabels.avocadoSign[langKey] || UI_TEXTS.legendLabels.avocadoSign.de), 'T2': t2Label };
         ...
         svg.append("text").attr("class", "axis-label y-axis-label").attr("text-anchor", "middle").attr("transform", `translate(${margin.left / 2}, ${margin.top + innerHeight / 2}) rotate(-90)`).style("font-size", AXIS_LABEL_FONT_SIZE).text(UI_TEXTS.axisLabels.metricValue[langKey] || UI_TEXTS.axisLabels.metricValue.de);
         ...
         metricGroup.selectAll("rect").data(d => subgroups.map(key => ({key: key, value: d[key], metric: d.metric}))).join("rect").attr("class", d => `bar bar-${d.key}`).attr("x", d => x1(d.key)).attr("y", y(0)).attr("width", x1.bandwidth()).attr("height", 0).attr("fill", d => color(d.key)).style("opacity", 0.9).attr("rx", 1).attr("ry", 1)
            .on("mouseover", function(event, d) { tooltip.transition().duration(50).style("opacity", .95); const displayName = subgroupDisplayNames[d.key] || d.key; const digits = (d.metric === 'AUC' || d.metric === 'F1') ? 3 : 1; const isPercent = !(d.metric === 'AUC' || d.metric === 'F1'); const formattedValue = isPercent ? formatPercent(d.value, digits, '--', langKey) : formatNumber(d.value, digits, '--', false, langKey); tooltip.html(`<strong>${d.metric} (${displayName}):</strong> ${formattedValue}`).style("left", (event.pageX + 10) + "px").style("top", (event.pageY - 15) + "px"); d3.select(this).style("opacity", 1).style("stroke", "#333").style("stroke-width", 1); })
    ...
    function renderASPerformanceChart(targetElementId, performanceData, options = {}, kollektivName = '') {
        ...
        if (!cohortPerf) {
            const noDataText = langKey === 'de' ? 'Performance-Daten nicht verfügbar.' : 'Performance data not available.';
            chartArea.append('text').attr('x', innerWidth / 2).attr('y', innerHeight / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text(noDataText); return;
        }
        const chartData = metrics.map(metric => { const keyLower = metric.toLowerCase(); let value = NaN; if (cohortPerf[keyLower + 'Val'] !== undefined && cohortPerf[keyLower + 'Val'] !== null) value = cohortPerf[keyLower + 'Val']; else if (cohortPerf[metric] !== undefined && cohortPerf[metric] !== null) value = cohortPerf[metric]?.value ?? NaN; return { metric: metric, value: value }; }).filter(d => d.value !== undefined && !isNaN(d.value) && isFinite(d.value));
        if (chartData.length === 0) {
            const incompleteDataText = langKey === 'de' ? 'Unvollständige Daten.' : 'Incomplete data.';
            chartArea.append('text').attr('x', innerWidth / 2).attr('y', innerHeight / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text(incompleteDataText); return;
        }
        ...
        const tickCountY = 5; chartArea.append("g").attr("class", "y-axis axis").call(d3.axisLeft(y).ticks(tickCountY, "%").tickSizeOuter(0)).selectAll("text").style("font-size", TICK_LABEL_FONT_SIZE);
        const yAxisLabelText = langKey === 'de' ? 'Diagnostische Güte (AS)' : 'Diagnostic Performance (AS)';
        ...
        chartArea.selectAll(".bar").data(chartData).join("rect").attr("class", "bar").attr("x", d => x(d.metric)).attr("y", y(0)).attr("width", x.bandwidth()).attr("height", 0).attr("fill", NEW_PRIMARY_COLOR_BLUE).style("opacity", 0.9).attr("rx", 1).attr("ry", 1)
           .on("mouseover", function(event, d) { tooltip.transition().duration(50).style("opacity", .95); const digits = (d.metric === 'AUC' || d.metric === 'F1') ? 3 : 1; const isPercent = !(d.metric === 'AUC' || d.metric === 'F1'); const formattedValue = isPercent ? formatPercent(d.value, digits, '--', langKey) : formatNumber(d.value, digits, '--', false, langKey); const metricName = TOOLTIP_CONTENT.statMetrics[d.metric.toLowerCase()]?.name?.[langKey] || TOOLTIP_CONTENT.statMetrics[d.metric.toLowerCase()]?.name?.['de'] || d.metric; tooltip.html(`<strong>${metricName}:</strong> ${formattedValue}`).style("left", (event.pageX + 10) + "px").style("top", (event.pageY - 15) + "px"); d3.select(this).style("opacity", 1).style("stroke", "#333").style("stroke-width", 1); })
           .on("mouseout", function(event, d) { tooltip.transition().duration(200).style("opacity", 0); d3.select(this).style("opacity", 0.9).style("stroke", "none"); })
           .transition().duration(ANIMATION_DURATION_MS).ease(d3.easeCubicOut).attr("y", d => y(d.value ?? 0)).attr("height", d => Math.max(0, innerHeight - y(d.value ?? 0)));

        const chartTitleTextDe = `AS Performance für Kollektiv: ${kollektivName}`;
        const chartTitleTextEn = `AS Performance for Cohort: ${kollektivName}`;
        svg.append("text").attr("x", width / 2).attr("y", margin.top - 10).attr("text-anchor", "middle").style("font-size", "12px").style("font-weight", "bold").text(langKey === 'de' ? chartTitleTextDe : chartTitleTextEn);
    }
    ...
    function renderROCCurve(rocData, targetElementId, options = {}) {
        ...
        if (!validRocData.find(p => p.fpr === 0 && p.tpr === 0)) { validRocData.unshift({ fpr: 0, tpr: 0, threshold: Infinity }); }
        if (!validRocData.find(p => p.fpr === 1 && p.tpr === 1)) { validRocData.push({ fpr: 1, tpr: 1, threshold: -Infinity }); }
        validRocData.sort((a,b) => a.fpr - b.fpr || a.tpr - b.tpr); // Ensure correct order for line path

        if (validRocData.length < 2) {
            const noDataText = langKey === 'de' ? 'Nicht genügend Daten für ROC.' : 'Not enough data for ROC.';
            chartArea.append('text').attr('x', innerWidth / 2).attr('y', innerHeight / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text(noDataText); return;
        }
        ...
        svg.append("text").attr("class", "axis-label x-axis-label").attr("text-anchor", "middle").attr("x", margin.left + innerWidth / 2).attr("y", height - 5).style("font-size", AXIS_LABEL_FONT_SIZE).text(UI_TEXTS.axisLabels.oneMinusSpecificity[langKey] || UI_TEXTS.axisLabels.oneMinusSpecificity.de);
        ...
        svg.append("text").attr("class", "axis-label y-axis-label").attr("text-anchor", "middle").attr("transform", `translate(${margin.left / 2 - 10}, ${margin.top + innerHeight / 2}) rotate(-90)`).style("font-size", AXIS_LABEL_FONT_SIZE).text(UI_TEXTS.axisLabels.sensitivity[langKey] || UI_TEXTS.axisLabels.sensitivity.de);
        ...
        if (options.showPoints) { chartArea.selectAll(".roc-point").data(validRocData.filter((d, i) => i > 0 && i < validRocData.length - 1)).join("circle").attr("class", "roc-point").attr("cx", d => xScale(d.fpr)).attr("cy", d => yScale(d.tpr)).attr("r", POINT_RADIUS / 1.5).attr("fill", lineColor).style("opacity", 0)
            .on("mouseover", function(event, d) { tooltip.transition().duration(50).style("opacity", .95); const threshText = (d.threshold && isFinite(d.threshold)) ? `<br>Threshold: ${formatNumber(d.threshold, 2, '--', false, langKey)}` : ''; tooltip.html(`FPR: ${formatNumber(d.fpr, 2, '--', false, langKey)}<br>TPR: ${formatNumber(d.tpr, 2, '--', false, langKey)}${threshText}`).style("left", (event.pageX + 10) + "px").style("top", (event.pageY - 15) + "px"); d3.select(this).attr("r", POINT_RADIUS); })
         ...
         if (options.aucValue !== undefined && !isNaN(options.aucValue)) { const aucText = `AUC: ${formatCI(options.aucValue, options.aucCI?.lower, options.aucCI?.upper, 3, false, '--', langKey)}`; chartArea.append("text").attr("class", "auc-label").attr("x", innerWidth - 10).attr("y", innerHeight - 10).attr("text-anchor", "end").style("font-size", AXIS_LABEL_FONT_SIZE).style("font-weight", "bold").text(aucText); }
    }
