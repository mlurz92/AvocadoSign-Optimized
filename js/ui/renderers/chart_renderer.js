const chartRenderer = (() => {
    ...
    function createSvgContainer(targetElementId, options = {}) {
        ...
        if (innerWidth <= 20 || innerHeight <= 20) {
            const noDataMessage = options.noDataMessage || 'Diagrammgröße ungültig/zu klein.';
            container.html(`<p class="text-muted small text-center p-2">${noDataMessage}</p>`);
    ...
    function renderAgeDistributionChart(ageData, targetElementId, options = {}) {
        ...
        const noDataMessage = options.noDataMessage || (lang === 'en' ? 'No age data available.' : 'Keine Altersdaten verfügbar.');
        const xAxisLabelText = options.xAxisLabel || (lang === 'en' ? 'Age (Years)' : 'Alter (Jahre)');
        const yAxisLabelText = options.yAxisLabel || (lang === 'en' ? 'Number of Patients' : 'Anzahl Patienten');
        const tooltipLabelAgeText = options.tooltipLabelAge || (lang === 'en' ? 'Age' : 'Alter');
        const tooltipLabelCountText = options.tooltipLabelCount || (lang === 'en' ? 'Count' : 'Anzahl');


        if (!Array.isArray(ageData) || ageData.length === 0) { chartArea.append('text').attr('x', innerWidth / 2).attr('y', innerHeight / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text(noDataMessage); return; }
        ...
        svg.append("text").attr("class", "axis-label x-axis-label").attr("text-anchor", "middle").attr("x", margin.left + innerWidth / 2).attr("y", height - 5).style("font-size", AXIS_LABEL_FONT_SIZE).text(xAxisLabelText);
        ...
        svg.append("text").attr("class", "axis-label y-axis-label").attr("text-anchor", "middle").attr("transform", `translate(${margin.left / 2 - 5}, ${margin.top + innerHeight / 2}) rotate(-90)`).style("font-size", AXIS_LABEL_FONT_SIZE).text(yAxisLabelText);
        ...
        chartArea.selectAll(".bar").data(bins).join("rect").attr("class", "bar").attr("x", d => x(d.x0) + 1).attr("y", y(0)).attr("width", d => Math.max(0, x(d.x1) - x(d.x0) - 1)).attr("height", 0).style("fill", barColor).style("opacity", 0.8).attr("rx", 1).attr("ry", 1)
            .on("mouseover", (event, d) => { tooltip.transition().duration(50).style("opacity", .95); tooltip.html(`${tooltipLabelAgeText}: ${formatNumber(d.x0, 0, '--', false, lang)}-${formatNumber(d.x1, 0, '--', false, lang)}<br>${tooltipLabelCountText}: ${d.length}`).style("left", (event.pageX + 10) + "px").style("top", (event.pageY - 15) + "px"); d3.select(event.currentTarget).style("opacity", 1).style("stroke", "#333").style("stroke-width", 0.5); })
    ...
    function renderPieChart(data, targetElementId, options = {}) {
        ...
        const lang = options.lang || 'de';
        const noDataMessage = options.noDataMessage || (lang === 'en' ? 'No data.' : 'Keine Daten.');
        ...
        if (validData.length === 0 || totalValue <= 0) { chartArea.append('text').attr('x', innerWidth / 2).attr('y', innerHeight / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text(noDataMessage); return; }
        ...
        if (outerRadius <= innerRadius || outerRadius <= 0) { chartArea.append('text').attr('x', plottingWidth / 2).attr('y', plottingHeight / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text(options.tooSmallMessage || (lang === 'en' ? 'Too small' : 'Zu klein')); return; }
        ...
        arcs.append("path").style("fill", d => color(d.data.label)).style("stroke", PLOT_BACKGROUND_COLOR).style("stroke-width", "1.5px").style("opacity", 0.85)
            .on("mouseover", function(event, d) { tooltip.transition().duration(50).style("opacity", .95); tooltip.html(`<strong>${d.data.label}:</strong> ${formatNumber(d.data.value, 0, '--', false, lang)} (${formatPercent(d.data.value / totalValue, 0, '--%', lang)})`).style("left", (event.pageX + 10) + "px").style("top", (event.pageY - 15) + "px"); d3.select(this).transition().duration(100).style("opacity", 1).attr("transform", "scale(1.03)"); })
        ...
        arcs.filter(d_2 => (d_2.endAngle - d_2.startAngle) / (2 * Math.PI) >= labelThreshold).append("text").attr("transform", d_2 => `translate(${labelArcGenerator.centroid(d_2)})`).attr("dy", "0.35em").style("text-anchor", "middle").style("font-size", labelFontSize).style("fill", "#ffffff").style("pointer-events", "none").style("opacity", 0).text(d_2 => formatPercent(d_2.data.value / totalValue, 0, '--%', lang)).transition().duration(ANIMATION_DURATION_MS).delay(ANIMATION_DURATION_MS / 2).style("opacity", 1);
        if (setupOptions.legendBelow && legendSpaceY > 0 && validData.length > 0) {
            ...
            legendItems.each(function(d_3, i_1) {
                const item = d3.select(this);
                item.append("rect").attr("x", 0).attr("y", 0).attr("width", 10).attr("height", 10).attr("fill", color(d_3.label));
                item.append("text").attr("x", 14).attr("y", 5).attr("dy", "0.35em").text(`${d_3.label} (${formatNumber(d_3.value, 0, '--', false, lang)})`);
                const itemWidth = this.getBBox().width + 15;
                if (i_1 > 0 && currentX + itemWidth > legendMaxWidth) { currentX = 0; currentY += legendItemHeight; }
    ...
    function renderComparisonBarChart(chartData, targetElementId, options = {}, t2LabelFallback = 'T2') {
         ...
         const lang = options.lang || 'de';
         const noDataMessage = options.noDataMessage || (lang === 'en' ? 'No comparison data.' : 'Keine Vergleichsdaten.');
         const yAxisLabelText = options.yAxisLabel || (lang === 'en' ? 'Metric Value' : 'Wert');


         if (!Array.isArray(chartData) || chartData.length === 0) { chartArea.append('text').attr('x', innerWidth / 2).attr('y', innerHeight / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text(noDataMessage); return; }
         const groups = chartData.map(d => d.metric); const subgroups = Object.keys(chartData[0]).filter(key => key !== 'metric');

         const legendLabelsOption = options.legendLabels || {};
         const subgroupDisplayNames = {
             'AS': legendLabelsOption.AS || (lang === 'en' ? 'Avocado Sign (AS)' : 'Avocado Sign (AS)'),
             'T2': legendLabelsOption.T2 || t2LabelFallback
         };
         ...
         svg.append("text").attr("class", "axis-label y-axis-label").attr("text-anchor", "middle").attr("transform", `translate(${margin.left / 2}, ${margin.top + innerHeight / 2}) rotate(-90)`).style("font-size", AXIS_LABEL_FONT_SIZE).text(yAxisLabelText);
         ...
         const metricGroup = chartArea.selectAll(".metric-group").data(chartData).join("g").attr("class", "metric-group").attr("transform", d_1 => `translate(${x0(d_1.metric)},0)`);
         metricGroup.selectAll("rect").data(d_2 => subgroups.map(key => ({key: key, value: d_2[key], metric: d_2.metric}))).join("rect").attr("class", d_2 => `bar bar-${d_2.key}`).attr("x", d_2 => x1(d_2.key)).attr("y", y(0)).attr("width", x1.bandwidth()).attr("height", 0).attr("fill", d_2 => color(d_2.key)).style("opacity", 0.9).attr("rx", 1).attr("ry", 1)
            .on("mouseover", function(event, d_3) { tooltip.transition().duration(50).style("opacity", .95); const displayName = subgroupDisplayNames[d_3.key] || d_3.key; const digits = (d_3.metric === 'AUC' || d_3.metric === 'F1') ? 3 : 1; const isPercent = !(d_3.metric === 'AUC' || d_3.metric === 'F1'); const formattedValue = isPercent ? formatPercent(d_3.value, digits, '--%', lang) : formatNumber(d_3.value, digits, '--', false, lang); tooltip.html(`<strong>${d_3.metric} (${displayName}):</strong> ${formattedValue}`).style("left", (event.pageX + 10) + "px").style("top", (event.pageY - 15) + "px"); d3.select(this).style("opacity", 1).style("stroke", "#333").style("stroke-width", 1); })
            .on("mouseout", function(event, d_4) { tooltip.transition().duration(200).style("opacity", 0); d3.select(this).style("opacity", 0.9).style("stroke", "none"); })
            .transition().duration(ANIMATION_DURATION_MS).ease(d3.easeCubicOut).attr("y", d_5 => y(d_5.value ?? 0)).attr("height", d_5 => Math.max(0, innerHeight - y(d_5.value ?? 0)));
         ...
         let totalLegendWidth = 0; const legendSpacings = []; legendItems.append("rect").attr("x", 0).attr("y", -5).attr("width", 10).attr("height", 10).attr("fill", color); legendItems.append("text").attr("x", 14).attr("y", 0).attr("dy", "0.35em").text(d_6 => subgroupDisplayNames[d_6] || d_6).each(function() { const itemWidth = this.getBBox().width + 25; legendSpacings.push(itemWidth); totalLegendWidth += itemWidth; });
         const legendStartX = margin.left + Math.max(0, (innerWidth - totalLegendWidth + 10) / 2); let currentX = legendStartX; legendItems.attr("transform", (d_7, i) => { const tx = currentX; currentX += legendSpacings[i]; return `translate(${tx}, ${height - margin.bottom + 30})`; });
    }
    ...
    function renderASPerformanceChart(targetElementId, performanceData, options = {}, kollektivName = '') {
        ...
        const { svg, chartArea, innerWidth, innerHeight, width, height, margin } = containerSetup; const tooltip = createTooltip();
        const lang = options.lang || 'de';
        const noDataMessage = options.noDataMessage || (lang === 'en' ? 'Performance data not available.' : 'Performance-Daten nicht verfügbar.');
        const incompleteDataMessage = options.incompleteDataMessage || (lang === 'en' ? 'Incomplete data.' : 'Unvollständige Daten.');
        const yAxisLabelText = options.yAxisLabel || (lang === 'en' ? 'Diagnostic Performance (AS)' : 'Diagnostische Güte (AS)');
        const chartTitleTemplate = options.chartTitleTemplate || (lang === 'en' ? 'AS Performance for Cohort: {KOLLEKTIV_NAME}' : 'AS Performance für Kollektiv: {KOLLEKTIV_NAME}');
        ...
        if (!cohortPerf) { chartArea.append('text').attr('x', innerWidth / 2).attr('y', innerHeight / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text(noDataMessage); return; }
        ...
        if (chartData.length === 0) { chartArea.append('text').attr('x', innerWidth / 2).attr('y', innerHeight / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text(incompleteDataMessage); return; }
        ...
        const tickCountY = 5; chartArea.append("g").attr("class", "y-axis axis").call(d3.axisLeft(y).ticks(tickCountY, "%").tickSizeOuter(0)).selectAll("text").style("font-size", TICK_LABEL_FONT_SIZE);
        svg.append("text").attr("class", "axis-label y-axis-label").attr("text-anchor", "middle").attr("transform", `translate(${margin.left / 2 - 10}, ${margin.top + innerHeight / 2}) rotate(-90)`).style("font-size", AXIS_LABEL_FONT_SIZE).text(yAxisLabelText);
        ...
        if (ENABLE_GRIDLINES) { chartArea.append("g").attr("class", "grid").call(d3.axisLeft(y).ticks(tickCountY).tickSize(-innerWidth).tickFormat("")); }
        chartArea.selectAll(".bar").data(chartData).join("rect").attr("class", "bar").attr("x", d_1 => x(d_1.metric)).attr("y", y(0)).attr("width", x.bandwidth()).attr("height", 0).attr("fill", NEW_PRIMARY_COLOR_BLUE).style("opacity", 0.9).attr("rx", 1).attr("ry", 1)
           .on("mouseover", function(event, d_2) { tooltip.transition().duration(50).style("opacity", .95); const digits = (d_2.metric === 'AUC' || d_2.metric === 'F1') ? 3 : 1; const isPercent = !(d_2.metric === 'AUC' || d_2.metric === 'F1'); const formattedValue = isPercent ? formatPercent(d_2.value, digits, '--%', lang) : formatNumber(d_2.value, digits, '--', false, lang); const metricName = (getTooltipContent(lang).statMetrics[d_2.metric.toLowerCase()]?.name || d_2.metric); tooltip.html(`<strong>${metricName}:</strong> ${formattedValue}`).style("left", (event.pageX + 10) + "px").style("top", (event.pageY - 15) + "px"); d3.select(this).style("opacity", 1).style("stroke", "#333").style("stroke-width", 1); })
           .on("mouseout", function(event, d_3) { tooltip.transition().duration(200).style("opacity", 0); d3.select(this).style("opacity", 0.9).style("stroke", "none"); })
           .transition().duration(ANIMATION_DURATION_MS).ease(d3.easeCubicOut).attr("y", d_4 => y(d_4.value ?? 0)).attr("height", d_4 => Math.max(0, innerHeight - y(d_4.value ?? 0)));
        svg.append("text").attr("x", width / 2).attr("y", margin.top - 10).attr("text-anchor", "middle").style("font-size", "12px").style("font-weight", "bold").text(chartTitleTemplate.replace('{KOLLEKTIV_NAME}', kollektivName));
    }
    ...
    function renderROCCurve(rocData, targetElementId, options = {}) {
        ...
        const lang = options.lang || 'de';
        const noDataMessage = options.noDataMessage || (lang === 'en' ? 'Not enough data for ROC.' : 'Nicht genügend Daten für ROC.');
        const xAxisLabelText = options.xAxisLabel || (lang === 'en' ? '1 - Specificity (FPR)' : '1 - Spezifität (FP Rate)');
        const yAxisLabelText = options.yAxisLabel || (lang === 'en' ? 'Sensitivity (TPR)' : 'Sensitivität (RP Rate)');
        const tooltipLabels = options.tooltipLabels || { fpr: (lang === 'en' ? 'FPR' : 'FPR'), tpr: (lang === 'en' ? 'TPR' : 'RP Rate'), threshold: (lang === 'en' ? 'Threshold' : 'Schwellenwert') };
        const aucLabelPrefixText = options.aucLabelPrefix || (lang === 'en' ? 'AUC: ' : 'AUC: ');
        ...
        if (validRocData.length < 2) { chartArea.append('text').attr('x', innerWidth / 2).attr('y', innerHeight / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text(noDataMessage); return; }
        ...
        const rocLine = d3.line().x(d_1 => xScale(d_1.fpr)).y(d_1 => yScale(d_1.tpr)).curve(d3.curveLinear);
        ...
        if (options.showPoints) { chartArea.selectAll(".roc-point").data(validRocData.filter((d_2, i) => i > 0 && i < validRocData.length - 1)).join("circle").attr("class", "roc-point").attr("cx", d_2 => xScale(d_2.fpr)).attr("cy", d_2 => yScale(d_2.tpr)).attr("r", POINT_RADIUS / 1.5).attr("fill", lineColor).style("opacity", 0)
            .on("mouseover", function(event, d_3) { tooltip.transition().duration(50).style("opacity", .95); const threshText = (d_3.threshold && isFinite(d_3.threshold)) ? `<br>${tooltipLabels.threshold}: ${formatNumber(d_3.threshold, 2, '--', false, lang)}` : ''; tooltip.html(`${tooltipLabels.fpr}: ${formatNumber(d_3.fpr, 2, '--', false, lang)}<br>${tooltipLabels.tpr}: ${formatNumber(d_3.tpr, 2, '--', false, lang)}${threshText}`).style("left", (event.pageX + 10) + "px").style("top", (event.pageY - 15) + "px"); d3.select(this).attr("r", POINT_RADIUS); })
            .on("mouseout", function(event, d_4) { tooltip.transition().duration(200).style("opacity", 0); d3.select(this).attr("r", POINT_RADIUS / 1.5); })
            .transition().delay(ANIMATION_DURATION_MS * 1.5).duration(ANIMATION_DURATION_MS / 2).style("opacity", 0.7); }
         if (options.aucValue !== undefined && !isNaN(options.aucValue)) { const aucText = `${aucLabelPrefixText}${formatCI(options.aucValue, options.aucCI?.lower, options.aucCI?.upper, 3, false, '--', lang)}`; chartArea.append("text").attr("class", "auc-label").attr("x", innerWidth - 10).attr("y", innerHeight - 10).attr("text-anchor", "end").style("font-size", AXIS_LABEL_FONT_SIZE).style("font-weight", "bold").text(aucText); }
    }
