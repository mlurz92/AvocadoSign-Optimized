const chartRenderer = (() => {
    ...
    function createSvgContainer(targetElementId, options = {}) {
        ...
        if (innerWidth <= 20 || innerHeight <= 20) {
            const errorMsg = options.lang === 'en' ? 'Chart size invalid/too small.' : 'Diagrammgröße ungültig/zu klein.';
    ...
    function renderAgeDistributionChart(ageData, targetElementId, options = {}) {
        ...
        const noDataText = UI_TEXTS?.axisLabels?.[lang]?.noAgeData || UI_TEXTS?.axisLabels?.de?.noAgeData || 'Keine Altersdaten verfügbar.';
        if (!Array.isArray(ageData) || ageData.length === 0) { chartArea.append('text').attr('x', innerWidth / 2).attr('y', innerHeight / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text(noDataText); return; }
        ...
        const xAxisLabelText = options.xAxisLabel || UI_TEXTS?.axisLabels?.[lang]?.age || UI_TEXTS?.axisLabels?.de?.age || 'Alter (Jahre)';
        ...
        const yAxisLabelText = options.yAxisLabel || UI_TEXTS?.axisLabels?.[lang]?.patientCount || UI_TEXTS?.axisLabels?.de?.patientCount || 'Anzahl Patienten';
        ...
        const tooltipAgeLabel = options.tooltipLabels?.age || (lang === 'en' ? 'Age' : 'Alter');
        const tooltipCountLabel = options.tooltipLabels?.count || (lang === 'en' ? 'Count' : 'Anzahl');
        ...
        chartArea.selectAll(".bar").data(bins).join("rect").attr("class", "bar").attr("x", d => x(d.x0) + 1).attr("y", y(0)).attr("width", d => Math.max(0, x(d.x1) - x(d.x0) - 1)).attr("height", 0).style("fill", barColor).style("opacity", 0.8).attr("rx", 1).attr("ry", 1)
            .on("mouseover", (event, d) => { tooltip.transition().duration(50).style("opacity", .95); tooltip.html(`${tooltipAgeLabel}: ${formatNumber(d.x0, 0, '--', false, lang)}-${formatNumber(d.x1, 0, '--', false, lang)}<br>${tooltipCountLabel}: ${d.length}`).style("left", (event.pageX + 10) + "px").style("top", (event.pageY - 15) + "px"); d3.select(event.currentTarget).style("opacity", 1).style("stroke", "#333").style("stroke-width", 0.5); })
        ...
        if(options.chartTitle) {
            svg.append("text").attr("x", width / 2).attr("y", margin.top - 10).attr("text-anchor", "middle").style("font-size", "12px").style("font-weight", "bold").text(options.chartTitle);
        }
    }
    ...
    function renderPieChart(data, targetElementId, options = {}) {
        ...
        const containerSetup = createSvgContainer(targetElementId, setupOptions); if (!containerSetup) return;
        const { svg, chartArea, innerWidth, innerHeight, width, height, margin, legendSpaceY } = containerSetup; const tooltip = createTooltip(); const colorScheme = DEFAULT_COLOR_SCHEME;
        ...
        const noDataText = UI_TEXTS?.axisLabels?.[lang]?.noData || UI_TEXTS?.axisLabels?.de?.noData || 'Keine Daten.';
        if (validData.length === 0 || totalValue <= 0) { chartArea.append('text').attr('x', innerWidth / 2).attr('y', innerHeight / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text(noDataText); return; }
        ...
        const tooSmallText = UI_TEXTS?.axisLabels?.[lang]?.chartTooSmall || UI_TEXTS?.axisLabels?.de?.chartTooSmall || 'Zu klein';
        if (outerRadius <= innerRadius || outerRadius <= 0) { chartArea.append('text').attr('x', plottingWidth / 2).attr('y', plottingHeight / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text(tooSmallText); return; }
        ...
        if (setupOptions.legendBelow && legendSpaceY > 0 && validData.length > 0) {
            ...
            legendItems.each(function(d, i) {
                ...
                item.append("text").attr("x", 14).attr("y", 5).attr("dy", "0.35em").text(`${d.label} (${formatNumber(d.value, 0, '--', false, lang)})`);
    ...
    function renderComparisonBarChart(chartData, targetElementId, options = {}, t2Label = 'T2') {
         ...
         const noDataText = UI_TEXTS?.axisLabels?.[lang]?.noComparisonData || UI_TEXTS?.axisLabels?.de?.noComparisonData || 'Keine Vergleichsdaten.';
         if (!Array.isArray(chartData) || chartData.length === 0) { chartArea.append('text').attr('x', innerWidth / 2).attr('y', innerHeight / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text(noDataText); return; }

         const groups = chartData.map(d => d.metric); const subgroups = Object.keys(chartData[0]).filter(key => key !== 'metric');
         const defaultSubgroupDisplayNames = {
            'AS': UI_TEXTS?.legendLabels?.[lang]?.avocadoSign || UI_TEXTS?.legendLabels?.de?.avocadoSign || 'Avocado Sign (AS)',
            'T2': options.t2LabelForLegend || t2Label
         };
         const subgroupDisplayNames = options.subgroupDisplayNames || defaultSubgroupDisplayNames;
         ...
         const yAxisLabelText = options.yAxisLabel || UI_TEXTS?.axisLabels?.[lang]?.metricValue || UI_TEXTS?.axisLabels?.de?.metricValue || 'Wert';
         ...
         metricGroup.selectAll("rect").data(d => subgroups.map(key => ({key: key, value: d[key], metric: d.metric}))).join("rect").attr("class", d => `bar bar-${d.key}`).attr("x", d => x1(d.key)).attr("y", y(0)).attr("width", x1.bandwidth()).attr("height", 0).attr("fill", d => color(d.key)).style("opacity", 0.9).attr("rx", 1).attr("ry", 1)
            .on("mouseover", function(event, d) { tooltip.transition().duration(50).style("opacity", .95); const displayName = subgroupDisplayNames[d.key] || d.key; const digits = (d.metric === 'AUC' || d.metric === 'F1') ? 3 : 1; const isPercent = !(d.metric === 'AUC' || d.metric === 'F1'); const formattedValue = isPercent ? formatPercent(d.value, digits, '--%', lang) : formatNumber(d.value, digits, '--', false, lang); tooltip.html(`<strong>${d.metric} (${displayName}):</strong> ${formattedValue}`).style("left", (event.pageX + 10) + "px").style("top", (event.pageY - 15) + "px"); d3.select(this).style("opacity", 1).style("stroke", "#333").style("stroke-width", 1); })
        ...
        if(options.chartTitle) {
            svg.append("text").attr("x", width / 2).attr("y", margin.top - 5).attr("text-anchor", "middle").style("font-size", "12px").style("font-weight", "bold").text(options.chartTitle);
        }
    }
    ...
    function renderASPerformanceChart(targetElementId, performanceData, options = {}, kollektivName = '') {
        ...
        const noDataText = UI_TEXTS?.axisLabels?.[lang]?.noPerformanceData || UI_TEXTS?.axisLabels?.de?.noPerformanceData || 'Performance-Daten nicht verfügbar.';
        if (!cohortPerf) { chartArea.append('text').attr('x', innerWidth / 2).attr('y', innerHeight / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text(noDataText); return; }
        ...
        const incompleteDataText = UI_TEXTS?.axisLabels?.[lang]?.incompleteData || UI_TEXTS?.axisLabels?.de?.incompleteData || 'Unvollständige Daten.';
        if (chartData.length === 0) { chartArea.append('text').attr('x', innerWidth / 2).attr('y', innerHeight / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text(incompleteDataText); return; }
        ...
        if (ENABLE_GRIDLINES) { chartArea.append("g").attr("class", "grid").call(d3.axisLeft(y).ticks(tickCountY).tickSize(-innerWidth).tickFormat("")); }
        chartArea.selectAll(".bar").data(chartData).join("rect").attr("class", "bar").attr("x", d => x(d.metric)).attr("y", y(0)).attr("width", x.bandwidth()).attr("height", 0).attr("fill", NEW_PRIMARY_COLOR_BLUE).style("opacity", 0.9).attr("rx", 1).attr("ry", 1)
           .on("mouseover", function(event, d) { tooltip.transition().duration(50).style("opacity", .95); const digits = (d.metric === 'AUC' || d.metric === 'F1') ? 3 : 1; const isPercent = !(d.metric === 'AUC' || d.metric === 'F1'); const formattedValue = isPercent ? formatPercent(d.value, digits, '--%', lang) : formatNumber(d.value, digits, '--', false, lang); const metricName = TOOLTIP_CONTENT.statMetrics[lang]?.[d.metric.toLowerCase()]?.name || TOOLTIP_CONTENT.statMetrics.de[d.metric.toLowerCase()]?.name || d.metric; tooltip.html(`<strong>${metricName}:</strong> ${formattedValue}`).style("left", (event.pageX + 10) + "px").style("top", (event.pageY - 15) + "px"); d3.select(this).style("opacity", 1).style("stroke", "#333").style("stroke-width", 1); })
           .on("mouseout", function(event, d) { tooltip.transition().duration(200).style("opacity", 0); d3.select(this).style("opacity", 0.9).style("stroke", "none"); })
           .transition().duration(ANIMATION_DURATION_MS).ease(d3.easeCubicOut).attr("y", d => y(d.value ?? 0)).attr("height", d => Math.max(0, innerHeight - y(d.value ?? 0)));

        const chartTitleText = options.chartTitle || (lang === 'en' ? `AS Performance for Cohort: ${kollektivName}` : `AS Performance für Kollektiv: ${kollektivName}`);
        svg.append("text").attr("x", width / 2).attr("y", margin.top - 10).attr("text-anchor", "middle").style("font-size", "12px").style("font-weight", "bold").text(chartTitleText);
    }
    ...
    function renderROCCurve(rocData, targetElementId, options = {}) {
        ...
        const noDataText = UI_TEXTS?.axisLabels?.[lang]?.noROCData || UI_TEXTS?.axisLabels?.de?.noROCData || 'Nicht genügend Daten für ROC.';
        if (validRocData.length < 2) { chartArea.append('text').attr('x', innerWidth / 2).attr('y', innerHeight / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text(noDataText); return; }
        ...
        const xAxisLabelText = options.xAxisLabel || UI_TEXTS?.axisLabels?.[lang]?.oneMinusSpecificity || UI_TEXTS?.axisLabels?.de?.oneMinusSpecificity || '1 - Spezifität (FP Rate)';
        svg.append("text").attr("class", "axis-label x-axis-label").attr("text-anchor", "middle").attr("x", margin.left + innerWidth / 2).attr("y", height - 5).style("font-size", AXIS_LABEL_FONT_SIZE).text(xAxisLabelText);
        ...
        const yAxisLabelText = options.yAxisLabel || UI_TEXTS?.axisLabels?.[lang]?.sensitivity || UI_TEXTS?.axisLabels?.de?.sensitivity || 'Sensitivität (RP Rate)';
        svg.append("text").attr("class", "axis-label y-axis-label").attr("text-anchor", "middle").attr("transform", `translate(${margin.left / 2 - 10}, ${margin.top + innerHeight / 2}) rotate(-90)`).style("font-size", AXIS_LABEL_FONT_SIZE).text(yAxisLabelText);
        ...
        const tooltipFPLabel = options.tooltipLabels?.fpr || 'FPR';
        const tooltipTPLabel = options.tooltipLabels?.tpr || 'TPR';
        const tooltipThreshLabel = options.tooltipLabels?.threshold || (lang === 'en' ? 'Threshold' : 'Schwellenwert');
        ...
        if (options.showPoints) { chartArea.selectAll(".roc-point").data(validRocData.filter((d, i) => i > 0 && i < validRocData.length - 1)).join("circle").attr("class", "roc-point").attr("cx", d => xScale(d.fpr)).attr("cy", d => yScale(d.tpr)).attr("r", POINT_RADIUS / 1.5).attr("fill", lineColor).style("opacity", 0)
            .on("mouseover", function(event, d) { tooltip.transition().duration(50).style("opacity", .95); const threshText = (d.threshold && isFinite(d.threshold)) ? `<br>${tooltipThreshLabel}: ${formatNumber(d.threshold, 2, '--', false, lang)}` : ''; tooltip.html(`${tooltipFPLabel}: ${formatNumber(d.fpr, 2, '--', false, lang)}<br>${tooltipTPLabel}: ${formatNumber(d.tpr, 2, '--', false, lang)}${threshText}`).style("left", (event.pageX + 10) + "px").style("top", (event.pageY - 15) + "px"); d3.select(this).attr("r", POINT_RADIUS); })
