const chartRenderer = (() => {
    ...
    function createSvgContainer(targetElementId, options = {}) {
        ...
        if (container.empty() || !container.node()) {
            ...
            if(document.getElementById(targetElementId)) {
                 ui_helpers.updateElementHTML(targetElementId, '<p class="text-muted small text-center p-2">Chart Container Fehler.</p>');
            }
            return null;
        }
        container.selectAll("svg").remove();
        container.html(''); // Clear previous content, including placeholders
        ...
        const legendHeightPerItem = 18;
        const legendPadding = 15;
        const estimatedLegendHeight = options.legendBelow && legendItemCount > 0 ? Math.max(25, legendItemCount * legendHeightPerItem + legendPadding) : 0;


        if (options.useCompactMargins && options.legendBelow) {
             height = Math.max(height, (options.height || defaultHeight) + estimatedLegendHeight - (APP_CONFIG.CHART_SETTINGS.DEFAULT_MARGIN.bottom - APP_CONFIG.CHART_SETTINGS.COMPACT_PIE_MARGIN.bottom));
        }
        ...
        svg.append("defs").append("style").text(`
            .axis path, .axis line { fill: none; stroke: ${APP_CONFIG.CHART_SETTINGS.GRIDLINE_COLOR}; shape-rendering: crispEdges; stroke-width: 1px; }
            .axis text { font-size: ${TICK_LABEL_FONT_SIZE}; fill: ${APP_CONFIG.CHART_SETTINGS.AXIS_LABEL_FONT_SIZE}; }
            .axis-label { font-size: ${AXIS_LABEL_FONT_SIZE}; fill: ${APP_CONFIG.CHART_SETTINGS.AXIS_LABEL_FONT_SIZE}; text-anchor: middle; }
            .grid .tick { stroke: ${APP_CONFIG.CHART_SETTINGS.GRIDLINE_COLOR}; stroke-opacity: 0.7; }
            .grid path { stroke-width: 0; }
            .legend text { font-size: ${LEGEND_FONT_SIZE}; fill: ${APP_CONFIG.CHART_SETTINGS.AXIS_LABEL_FONT_SIZE};}
            .reference-line { stroke: #adb5bd; stroke-width: 1px; stroke-dasharray: "3 3"; }
        `);
    ...
    function renderAgeDistributionChart(ageData, targetElementId, options = {}) {
        ...
        chartArea.append("g").attr("class", "x-axis axis").attr("transform", `translate(0,${innerHeight})`).call(d3.axisBottom(x).ticks(tickCountX).tickSizeOuter(0).tickFormat(d3.format("d")));
        svg.append("text").attr("class", "axis-label x-axis-label").attr("text-anchor", "middle").attr("x", margin.left + innerWidth / 2).attr("y", height - margin.bottom + (options.useCompactMargins ? 18 : 30)).style("font-size", AXIS_LABEL_FONT_SIZE).text(UI_TEXTS.axisLabels.age);
        ...
        chartArea.append("g").attr("class", "y-axis axis").call(d3.axisLeft(y).ticks(tickCountY).tickSizeOuter(0).tickFormat(d3.format("d")));
        svg.append("text").attr("class", "axis-label y-axis-label").attr("text-anchor", "middle").attr("transform", `translate(${margin.left / 2 - (options.useCompactMargins ? 5 : 15)}, ${margin.top + innerHeight / 2}) rotate(-90)`).style("font-size", AXIS_LABEL_FONT_SIZE).text(UI_TEXTS.axisLabels.patientCount);
    ...
    function renderPieChart(data, targetElementId, options = {}) {
        ...
        if (validData.length === 0 || totalValue <= 0) { chartArea.append('text').attr('x', innerWidth / 2).attr('y', (innerHeight + (options.legendBelow ? 0 : margin.top + margin.bottom ) / 2 ) / 2).attr('text-anchor', 'middle').attr('class', 'text-muted small').text('Keine Daten.'); return; }
        ...
        if (setupOptions.legendBelow && legendSpaceY > 0 && validData.length > 0) {
            const legendItemHeight = 18; const legendMaxWidth = innerWidth; const legendItemPadding = 15;
            const legendGroup = svg.append("g").attr("class", "legend pie-legend").attr("transform", `translate(${margin.left}, ${margin.top + plottingHeight + (options.useCompactMargins ? 10 : 20)})`).attr("font-family", "sans-serif").attr("font-size", LEGEND_FONT_SIZE).attr("text-anchor", "start");
            let currentX = 0; let currentY = 0;
            const legendItems = legendGroup.selectAll("g").data(validData).join("g").attr("class", "legend-item");
            legendItems.each(function(d, i) {
                const item = d3.select(this);
                item.append("rect").attr("x", 0).attr("y", -5).attr("width", 10).attr("height", 10).attr("fill", color(d.label));
                item.append("text").attr("x", 14).attr("y", 0).attr("dy", "0.32em").text(`${d.label} (${formatNumber(d.value, 0)})`);
                const itemWidth = this.getBBox().width + legendItemPadding;
    ...
    function renderComparisonBarChart(chartData, targetElementId, options = {}, t2Label = 'T2') {
         ...
         const x0 = d3.scaleBand().domain(groups).range([0, innerWidth]).paddingInner(0.35); const x1 = d3.scaleBand().domain(subgroups).range([0, x0.bandwidth()]).padding(0.15); const y = d3.scaleLinear().domain([0, 1.0]).nice().range([innerHeight, 0]); const color = d3.scaleOrdinal().domain(subgroups).range([NEW_PRIMARY_COLOR_BLUE, NEW_SECONDARY_COLOR_YELLOW_GREEN, APP_CONFIG.CHART_SETTINGS.TERTIARY_COLOR_GREEN].slice(0, subgroups.length));
         const tickCountY = Math.min(5, Math.floor(innerHeight/30)); chartArea.append("g").attr("class", "y-axis axis").call(d3.axisLeft(y).ticks(tickCountY, "%").tickSizeOuter(0));
         ...
         chartArea.append("g").attr("class", "x-axis axis").attr("transform", `translate(0,${innerHeight})`).call(d3.axisBottom(x0).tickSizeOuter(0)).selectAll(".tick text").style("text-anchor", "middle");
         ...
         let totalLegendWidth = 0; const legendSpacings = []; const legendItemPadding = 25;
         legendItems.append("rect").attr("x", 0).attr("y", -5).attr("width", 10).attr("height", 10).attr("fill", d => color(d));
         legendItems.append("text").attr("x", 14).attr("y", 0).attr("dy", "0.32em").text(d => subgroupDisplayNames[d] || d).each(function() { const itemWidth = this.getBBox().width + legendItemPadding; legendSpacings.push(itemWidth); totalLegendWidth += itemWidth; });
         const legendStartX = margin.left + Math.max(0, (innerWidth - totalLegendWidth + legendItemPadding) / 2); let currentX = legendStartX; legendItems.attr("transform", (d, i) => { const tx = currentX; currentX += legendSpacings[i]; return `translate(${tx}, ${height - margin.bottom + 30})`; });
    }
    ...
    function renderASPerformanceChart(targetElementId, performanceData, options = {}, kollektivName = '') {
        ...
        chartArea.append("g").attr("class", "x-axis axis").attr("transform", `translate(0,${innerHeight})`).call(d3.axisBottom(x).tickSizeOuter(0)).selectAll(".tick text").style("text-anchor", "middle");
        if (ENABLE_GRIDLINES) { chartArea.append("g").attr("class", "grid").call(d3.axisLeft(y).ticks(tickCountY).tickSize(-innerWidth).tickFormat("")); }
        chartArea.selectAll(".bar").data(chartData).join("rect").attr("class", "bar").attr("x", d => x(d.metric)).attr("y", y(0)).attr("width", x.bandwidth()).attr("height", 0).attr("fill", NEW_PRIMARY_COLOR_BLUE).style("opacity", 0.9).attr("rx", 1).attr("ry", 1)
           ...
           .on("mouseout", function(event, d) { tooltip.transition().duration(200).style("opacity", 0); d3.select(this).style("opacity", 0.9).style("stroke", "none"); })
           .transition().duration(ANIMATION_DURATION_MS).ease(d3.easeCubicOut).attr("y", d => y(d.value ?? 0)).attr("height", d => Math.max(0, innerHeight - y(d.value ?? 0)));
        svg.append("text").attr("x", width / 2).attr("y", margin.top - 10).attr("text-anchor", "middle").style("font-size", "12px").style("font-weight", "bold").text(UI_TEXTS.chartTitles.asPerformance.replace('{KollektivName}', kollektivName));
    }
    ...
    function renderROCCurve(rocData, targetElementId, options = {}) {
        ...
        const tickCount = Math.min(5, Math.floor(innerWidth/40));
        chartArea.append("g").attr("class", "x-axis axis").attr("transform", `translate(0,${innerHeight})`).call(d3.axisBottom(xScale).ticks(tickCount).tickSizeOuter(0).tickFormat(d3.format(".1f")));
        svg.append("text").attr("class", "axis-label x-axis-label").attr("text-anchor", "middle").attr("x", margin.left + innerWidth / 2).attr("y", height - margin.bottom + 30).style("font-size", AXIS_LABEL_FONT_SIZE).text(UI_TEXTS.axisLabels.oneMinusSpecificity);
        chartArea.append("g").attr("class", "y-axis axis").call(d3.axisLeft(yScale).ticks(tickCount).tickSizeOuter(0).tickFormat(d3.format(".1f")));
        ...
        if (options.showPoints) { chartArea.selectAll(".roc-point").data(validRocData.filter((d, i) => i > 0 && i < validRocData.length - 1 && d.threshold !== undefined)).join("circle").attr("class", "roc-point").attr("cx", d => xScale(d.fpr)).attr("cy", d => yScale(d.tpr)).attr("r", POINT_RADIUS / 1.5).attr("fill", lineColor).style("opacity", 0)
         ...
         if (options.aucValue !== undefined && options.aucValue.value !== undefined && !isNaN(options.aucValue.value)) { const aucText = `AUC: ${formatCI(options.aucValue, 3, false, 'de')}`; chartArea.append("text").attr("class", "auc-label").attr("x", innerWidth - 10).attr("y", innerHeight - 10).attr("text-anchor", "end").style("font-size", AXIS_LABEL_FONT_SIZE).style("font-weight", "bold").text(aucText); }
    }
