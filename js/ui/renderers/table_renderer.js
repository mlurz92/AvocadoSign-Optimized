const tableRenderer = (() => {
    ...
    function _createDatenDetailRowContent(patient) {
         ...
         patient.lymphknoten_t2.forEach((lk, index) => {
            ...
            content += `<div class="sub-row-item border rounded mb-1 p-1 w-100 align-items-center small">
                           ...
                           <span class="me-2 text-nowrap" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="${sizeTooltip}">${sizeIcon}${groesseText !== 'N/A' ? groesseText + 'mm' : groesseText}</span>
                           <span class="me-2 text-nowrap" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="${formTooltip}">${formIcon}${formText}</span>
                           <span class="me-2 text-nowrap" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="${konturTooltip}">${konturIcon}${konturText}</span>
                           <span class="me-2 text-nowrap" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="${homogenitaetTooltip}">${homogenitaetIcon}${homogenitaetText}</span>
                           <span class="me-2 text-nowrap" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="${signalTooltip}">${signalIcon}${signalText}</span>
                        </div>`;
         });
         return content;
    }
    ...
    function createDatenTableRow(patient) {
        ...
        return `
            <tr id="${rowId}" ${hasT2Nodes ? `class="clickable-row"` : ''} ${hasT2Nodes ? `data-bs-toggle="collapse"` : ''} data-bs-target="#${detailRowId}" aria-expanded="false" aria-controls="${detailRowId}">
                <td data-label="Nr." data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="${tooltipNr}">${patient.nr}</td>
                <td data-label="Name" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="${tooltipName}">${patient.name || naPlaceholder}</td>
                <td data-label="Vorname" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="${tooltipVorname}">${patient.vorname || naPlaceholder}</td>
                <td data-label="Geschlecht" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="${tooltipGeschlecht}">${geschlechtText}</td>
                <td data-label="Alter" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="${tooltipAlter}">${formatNumber(patient.alter, 0, naPlaceholder)}</td>
                <td data-label="Therapie" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="${tooltipTherapie}">${therapieText}</td>
                <td data-label="N/AS/T2" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="${tooltipStatus}">
                    <span class="status-${patient.n === '+' ? 'plus' : (patient.n === '-' ? 'minus' : 'unknown')}" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Pathologie N-Status (Goldstandard)">${patient.n ?? '?'}</span> /
                    <span class="status-${patient.as === '+' ? 'plus' : (patient.as === '-' ? 'minus' : 'unknown')}" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Avocado Sign Status (Vorhersage)">${patient.as ?? '?'}</span> /
                    <span class="status-${t2StatusIcon}" id="status-t2-pat-${patient.nr}" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="T2 Status (Vorhersage basierend auf aktuell angewendeten Kriterien)">${t2StatusText}</span>
                </td>
                <td data-label="Bemerkung" class="text-truncate" style="max-width: 150px;" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-html="true" data-bs-title="${tooltipBemerkung}">${bemerkungText || naPlaceholder}</td>
                 <td class="text-center p-1" style="width: 30px;" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="${tooltipExpand}">
    ...
    function _createAuswertungDetailRowContent(patient, appliedCriteria, appliedLogic) {
        ...
        const naPlaceholder = '--';
        const tooltipTitle = `Zeigt die Bewertung jedes einzelnen T2-Lymphknotens basierend auf den aktuell angewendeten Kriterien. Erfüllte Kriterien, die zur Positiv-Bewertung beitragen, sind hervorgehoben.`;

        let content = `<h6 class="w-100 mb-2 ps-1" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-html="true" data-bs-title="${tooltipTitle}">T2 LK Bewertung (Logik: ${UI_TEXTS.t2LogicDisplayNames[appliedLogic] || appliedLogic || 'N/A'}, Kriterien: ${criteriaFormatted || 'N/A'})</h6>`;
        ...
        patient.lymphknoten_t2_bewertet.forEach((lk, index) => {
            ...
            const badgeTooltip = lk.isPositive ? "Positiv bewertet gemäß aktueller T2-Kriterien" : "Negativ bewertet gemäß aktueller T2-Kriterien";
            let itemContent = `<strong class="me-2">LK ${index + 1}: ${lk.isPositive ? `<span class="badge bg-danger text-white ms-1" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="${badgeTooltip}">Pos.</span>` : `<span class="badge bg-success text-white ms-1" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="${badgeTooltip}">Neg.</span>`}</strong>`;
            ...
            const formatCriterionCheck = (key, iconType, valueText, checkResultForLK) => {
                ...
                return `<span class="me-2 text-nowrap ${hlClass}" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-html="true" data-bs-title="${tooltip}">${icon} ${text}</span>`;
            };
        ...
        return `
            <tr id="${rowId}" ${hasBewerteteNodes ? `class="clickable-row"` : ''} ${hasBewerteteNodes ? `data-bs-toggle="collapse"` : ''} data-bs-target="#${detailRowId}" aria-expanded="false" aria-controls="${detailRowId}">
                ...
                <td data-label="N/AS/T2" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="${tooltipStatus}">
                    <span class="status-${patient.n === '+' ? 'plus' : (patient.n === '-' ? 'minus' : 'unknown')}" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Pathologie N-Status (Goldstandard)">${patient.n ?? '?'}</span> /
                    <span class="status-${patient.as === '+' ? 'plus' : (patient.as === '-' ? 'minus' : 'unknown')}" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Avocado Sign Status (Vorhersage)">${patient.as ?? '?'}</span> /
                    <span class="status-${t2StatusIcon}" id="status-t2-ausw-${patient.nr}" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="T2 Status (Vorhersage basierend auf aktuell angewendeten Kriterien)">${t2StatusText}</span>
                </td>
                <td data-label="N+/N ges." class="text-center" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="${tooltipNCounts}">${nCountsText}</td>
                <td data-label="AS+/AS ges." class="text-center" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="${tooltipASCounts}">${asCountsText}</td>
                <td data-label="T2+/T2 ges." class="text-center" id="t2-counts-${patient.nr}" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="${tooltipT2Counts}">${t2CountsText}</td>
                 <td class="text-center p-1" style="width: 30px;" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="${tooltipExpand}">
