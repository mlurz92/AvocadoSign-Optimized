const auswertungTabRenderer = (() => {
    ...
    function _createDashboardHTML(currentKollektiv) {
        ...
        chartPlaceholders.forEach(chart => {
            const tooltipText = TOOLTIP_CONTENT.deskriptiveStatistik[chart.tooltipKey]?.description.replace('[KOLLEKTIV]', kollektivName) || chart.title;
            dashboardHtml += `
    ...
    function _createT2KriterienDefinitionHTML(currentCriteria, currentLogic) {
        if (typeof uiComponents === 'undefined' || typeof t2CriteriaManager === 'undefined' || typeof TOOLTIP_CONTENT === 'undefined') {
            return '<p class="text-danger">Fehler: UI Komponenten oder Kriterien-Manager nicht geladen.</p>';
        }
        const criteriaDisplay = t2CriteriaManager.getCriteriaDisplayValues(currentCriteria);
                        ...
                        <div class="card-body">
                            <div class="row g-3">
                                <div class="col-md-6 col-lg-3 criteria-group">
                                    ...
                                    ${uiComponents.createRangeSlider('size', criteriaDisplay.size.threshold, APP_CONFIG.T2_CRITERIA_SETTINGS.SIZE_RANGE, currentCriteria.size.active, TOOLTIP_CONTENT.t2Size.description)}
                                </div>
                                <div class="col-md-6 col-lg-2 criteria-group">
                                    ...
                                    ${uiComponents.createRadioOptions('form', APP_CONFIG.T2_CRITERIA_SETTINGS.FORM_VALUES, criteriaDisplay.form.value, currentCriteria.form.active, TOOLTIP_CONTENT.t2Form.description)}
                                </div>
                                <div class="col-md-6 col-lg-2 criteria-group">
                                    ${uiComponents.createCriteriaToggle('kontur', currentCriteria.kontur.active, TOOLTIP_CONTENT.t2Kontur.description)}
                                    ${uiComponents.createRadioOptions('kontur', APP_CONFIG.T2_CRITERIA_SETTINGS.KONTUR_VALUES, criteriaDisplay.kontur.value, currentCriteria.kontur.active, TOOLTIP_CONTENT.t2Kontur.description)}
                                </div>
                                <div class="col-md-6 col-lg-2 criteria-group">
                                    ...
                                    ${uiComponents.createRadioOptions('homogenitaet', APP_CONFIG.T2_CRITERIA_SETTINGS.HOMOGENITAET_VALUES, criteriaDisplay.homogenitaet.value, currentCriteria.homogenitaet.active, TOOLTIP_CONTENT.t2Homogenitaet.description)}
                                </div>
                                <div class="col-md-6 col-lg-3 criteria-group">
                                    ${uiComponents.createCriteriaToggle('signal', currentCriteria.signal.active, TOOLTIP_CONTENT.t2Signal.description)}
                                    ${uiComponents.createRadioOptions('signal', APP_CONFIG.T2_CRITERIA_SETTINGS.SIGNAL_VALUES, criteriaDisplay.signal.value, currentCriteria.signal.active, TOOLTIP_CONTENT.t2Signal.description)}
                                </div>
                            </div>
                            <hr class="my-3">
    ...
    function _createT2MetricsOverviewHTML(currentKollektiv) {
        const kollektivName = getKollektivDisplayName(currentKollektiv);
        const cardTitle = TOOLTIP_CONTENT.t2MetricsOverview.cardTitle.replace('[KOLLEKTIV]', kollektivName);
    ...
    function _createBruteForcePanelHTML(isWorkerAvailable, currentKollektiv) {
        const kollektivName = getKollektivDisplayName(currentKollektiv);
        const defaultMetric = stateManager.getBruteForceMetric() || APP_CONFIG.DEFAULT_SETTINGS.BRUTE_FORCE_METRIC;
        ...
        let html = `<h3 class="mb-3">Brute-Force Optimierung für T2-Kriterien</h3>
                    <div class="card mb-4">
                        <div class="card-body">
                            ...
                            <div id="brute-force-status-info" class="mt-3 small text-muted" data-tippy-content="${TOOLTIP_CONTENT.bruteForceInfo.description}">
                                Status: Bereit für Kollektiv <strong>${kollektivName}</strong>.
                            </div>
                            <div id="brute-force-progress-container" class="mt-2" data-tippy-content="${TOOLTIP_CONTENT.bruteForceProgress.description}">
        ...
        lymphknotenT2BewertetArray.forEach((lkBewertet, index) => {
            ...
            criteriaKeys.forEach(key => {
                if (currentCriteria[key]?.active) {
                    ...
                    const criterionMet = evaluationDetails[key]?.metOverall;
                    let icon = uiComponents.getIconForT2Feature(key, lk[key], criterionMet);
                    let textClass = '';
    ...
    function renderAuswertungTab(data, currentT2Criteria, currentT2Logic, sortState, currentKollektiv, isBruteForceWorkerAvailable) {
        if (typeof tableRenderer === 'undefined' || typeof uiComponents === 'undefined' || typeof TOOLTIP_CONTENT === 'undefined' || typeof APP_CONFIG === 'undefined' || typeof stateManager === 'undefined') {
        ...
        html += `<div class="d-flex justify-content-end mb-2">
                    <button class="btn btn-sm btn-outline-secondary" id="auswertung-toggle-details" data-tippy-content="${TOOLTIP_CONTENT.auswertungTable.expandAll.description}">
        ...
        const headers = [
            ...
            { key: 'n_as_t2', label: 'N | AS | T2', sortable: true, subSortKeys: ['n', 'as', 't2'], tooltip: TOOLTIP_CONTENT.auswertungTable.n_as_t2.description, class: 'text-center' },
            { key: 'n_counts', label: 'N LK (+/ges)', sortable: true, tooltip: TOOLTIP_CONTENT.auswertungTable.n_counts.description, class: 'text-center' },
            { key: 'as_counts', label: 'AS LK (+/ges)', sortable: true, tooltip: TOOLTIP_CONTENT.auswertungTable.as_counts.description, class: 'text-center' },
            { key: 't2_counts', label: 'T2 LK (+/ges)', sortable: true, tooltip: TOOLTIP_CONTENT.auswertungTable.t2_counts.description, class: 'text-center' }
        ];

        html += tableRenderer.createSortableTableHeaders(headers, sortState);
